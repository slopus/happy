...
export function connectRoutes(app: Fastify) {
    ...
    app.get('/v1/connect/github/callback', {
        ...
        if (!tokenData) {
            ...
            return reply.redirect('http://192.144.133.93:3000?error=invalid_state');
        }
        ...
        if (!clientId || !clientSecret) {
            return reply.redirect('http://192.144.133.93:3000?error=server_config');
        }
        ...
        try {
            ...
            if (tokenResponseData.error) {
                return reply.redirect(`http://192.144.133.93:3000?error=${encodeURIComponent(tokenResponseData.error)}`);
            }
            ...
            if (!userResponse.ok) {
                return reply.redirect('http://192.144.133.93:3000?error=github_user_fetch_failed');
            }
            ...
            return reply.redirect(`http://192.144.133.93:3000?github=connected&user=${encodeURIComponent(userData.login)}`);

        } catch (error) {
            log({ module: 'github-oauth' }, `Error in GitHub GET callback: ${error}`);
            return reply.redirect('http://192.144.133.93:3000?error=server_error');
        }
    });
