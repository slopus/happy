...
class Sync {
     ...
     */
    async runBashCommand(sessionId: string, command: string, prefix: string) {
        const session = storage.getState().sessions[sessionId];
        const cwd = session?.metadata?.path;

        // Echo the command as a local user message so the chat shows what ran,
        // carrying back the prefix the user actually typed rather than a fixed one.
        ...
        this.enqueueMessages(sessionId, [{
            ...
            role: 'user',
            isSidechain: false,
            content: { type: 'text', text: `${prefix}${command}` },
        }]);
