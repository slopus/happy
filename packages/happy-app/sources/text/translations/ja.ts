/**
 * Japanese translations for the Paws app
 * Values can be:
...
export const ja: TranslationStructure = {
    ...
    deviceEnvironmentDashboard: {
        ...
        scan: '再確認',
        scanning: '確認中…',
        fleet: ({ total, online }: { total: number; online: number }) => `デバイス ${total} 台 · オンライン ${online} 台`,
        lastChecked: ({ time }: { time: string }) => `確認日時 ${time}`,
        notChecked: '未確認',
        offline: 'オフライン · 前回の結果',
        online: 'オンライン',
        latest: '最新です',
        available: '更新あり',
        update: '更新',
        updateType: ({ count }: { count: number }) => `このツールを更新 · ${count} 台`,
        updateAll: ({ count }: { count: number }) => `更新可能な項目を更新 · ${count}`,
        install: 'インストール',
        login: 'ログイン',
        handle: '対処',
        retry: '再試行',
        ...
        failed: '更新失敗',
        uncertain: '結果未確認',
        unresolvedHint: 'リモートでの更新がまだ実行中の可能性があります。このデバイスでの更新は一時停止しています。バージョンの確認だけでは処理の終了を確認できません。続行する前に対象デバイスで完了を確認してください。',
        changed: '環境が変わりました · 内容を再確認してください',
        pendingLogin: 'インストール済み · ログインが必要',
        pendingPair: 'App と CLI の組み合わせに問題があります',
        usable: 'インストール済み',
        sourceUnknown: '最新バージョンを取得できません',
        ahead: '配布元より新しいバージョン',
        certificateOptional: '一時トンネル利用可 · アカウントへのログインは任意',
        updatesSummary: ({ count, machines }: { count: number; machines: number }) => `${machines} 台のデバイスで ${count} 件の更新が可能`,
        attentionSummary: ({ count }: { count: number }) => `${count} 件は個別のインストール・ログイン・対処が必要`,
        allCurrent: 'オンラインのデバイスに更新はありません',
        batchProgress: ({ done, total }: { done: number; total: number }) => `${total} 件を更新中 · ${done} 件完了`,
        batchResult: ({ succeeded, failed, uncertain, skipped }: { succeeded: number; failed: number; uncertain: number; skipped: number }) => `完了 ${succeeded} 件 · 失敗 ${failed} 件 · 未確認 ${uncertain} 件 · スキップ ${skipped} 件`,
        background: 'このパネルを閉じても、後で進行状況を確認できます。',
        verifyHint: '更新後は自動で検証し、オフラインのデバイスはスキップします。',
        horizontalHint: '左右にスワイプしてデバイスを比較',
        target: ({ version }: { version: string }) => `${version} に更新可能`,
        actionConfirmTitle: 'このデバイスで続行しますか？',
        actionConfirm: ({ machine, component, action, version }: { machine: string; component: string; action: string; version: string }) => `${machine}: ${component}（${version}）— ${action}。ログインや設定時に対象デバイスでウィンドウが開く場合があります。`,
        manualHint: '対象デバイスで以下のコマンドを実行し、再確認してください。',
        selectedAction: ({ machine, component, action }: { machine: string; component: string; action: string }) => `${machine} · ${component} · ${action}`,
        localSource: '対象バージョンは、このデバイスのインストール元に基づきます。',
    },
    deviceEnvironment: {
        ...
        egoAppMissing: "Ego Lite アプリがありません。このマシンにインストールして初期設定を完了してください。",
        egoUnsupported: "Ego Lite アプリの確認には macOS が必要です。このプラットフォームでは CLI のバージョンのみを表示します。",
        pawsUnsupported: "Paws CLI の同期には Apple Silicon Mac と確認済みのグローバル npm インストールが必要です。",
        componentUnsupported: "このプラットフォームでは、このコンポーネントの確認はサポートされていません。",
        cloudflaredFormulaUnavailable: "cloudflared のフォーミュラを取得できません。このマシンの Homebrew を確認してください。",
        cloudflaredHomebrewMissing: "Homebrew がないため、cloudflared の最新バージョンを確認できません。",
        componentVersionSourceMismatch: "バージョンの取得元が一致しません。このコンポーネントのインストールを確認して、もう一度スキャンしてください。",
        title: "デバイス環境",
        subtitle: "各マシンの開発ツールを確認し、バージョンを揃えます",
        fleetReady: ({ ready, total }: { ready: number; total: number }) => `${ready}/${total} 台が目標バージョンです`,
        scanAll: "すべてのマシンをスキャン",
        scanning: "マシンをスキャン中…",
        previewAlignment: "同期内容をプレビュー",
        previewing: "プレビューを準備中…",
        confirmTitle: "GitHub CLI を同期しますか？",
        confirmMessage: ({ actions }: { actions: string }) => `各マシンで実行する操作を確認してください。\n\n${actions}\n\n表示されたインストールと更新のみを実行します。各マシンで個別にログインする必要があります。`,
        confirmAction: "同期を適用",
        applying: "同期を適用中…",
        completed: "完了",
        githubCli: "GitHub CLI",
        daemonOnline: "Daemon 接続済み",
        daemonOffline: "Daemon オフラインのためスキップ",
        offlineRecovery: "このマシンの接続を復旧するか、SSH で状態を確認してから、もう一度スキャンしてください。",
        versionInstalled: ({ version }: { version: string }) => `インストール済み: ${version}`,
        versionTarget: ({ version }: { version: string }) => `目標: ${version}`,
        authReady: "GitHub 認証済み",
        authMissing: "GitHub へのログインが必要です",
        authUnknown: "GitHub の認証状態は不明です",
        actionNone: "バージョン変更なし",
        actionInstall: ({ version }: { version: string }) => `gh ${version} をインストール`,
        actionUpgrade: ({ from, version }: { from: string; version: string }) => `gh を ${from} → ${version} に更新`,
        actionManualRepair: "手動での修復が必要です。自動変更は行いません",
        repairWithSsh: "SSH でこのマシンに接続して、設定または修復を完了してください。",
        repairLocally: "このマシンでターミナルを開き、設定または修復を完了してください。",
        scanAgain: "もう一度スキャンしてマシンの状態を更新してください",
        stateUnknown: "状態は不明です。もう一度スキャンしてください",
        timeoutRecovery: "Homebrew または現在の操作が完了するまで待ち、再試行の前にもう一度スキャンしてください。",
        partialFailure: "対応が必要なマシンがあります。以下の結果を確認してください。",
        versionSourceMismatch: "バージョンの取得元が一致しません。該当マシンの Homebrew 情報を更新して、もう一度スキャンしてください。",
        homebrewMissing: "Homebrew がありません。同期の前にこのマシンにインストールしてください。",
        unsupportedMachine: "自動同期には、Homebrew がインストールされた Apple Silicon Mac が必要です。",
        operationInProgress: "このマシンで別の操作が実行中です。完了を待って、もう一度スキャンしてください。",
        planExpired: "プレビューの期限が切れたか、マシンの状態が変わりました。スキャンとプレビューをやり直してください。",
        formulaUnavailable: "gh のフォーミュラを取得できません。このマシンの Homebrew を確認してください。",
        versionAhead: "インストール済みのバージョンは目標より新しいものです。手動で確認してください。ダウングレードは行いません。",
        alignmentFailed: "同期が完了しませんでした。このマシンを確認して、もう一度スキャンしてください。",
        notInstalled: "未インストール",
        previewRequired: "プレビューで実行内容を確認してください",
        emptyFleet: "登録されたマシンはまだありません。マシンを接続して開始してください。",
        authenticationNote: "バージョンと GitHub の認証は個別に確認します。各マシンで gh auth login を実行してログインしてください。",
        confirmPawsMessage: ({ actions }: { actions: string }) => `各マシンの Paws CLI 操作を確認してください。\n\n${actions}\n\n表示されたグローバル npm のインストールと更新のみを実行します。確認済みの npm 所有権が必要です。`, pawsTimeoutRecovery: 'npm または現在の操作が完了するまで待ってから、もう一度スキャンしてください。', componentTimeoutRecovery: '現在の操作が完了するまで待ってから、もう一度スキャンしてください。', pawsOwnershipMismatch: 'Paws CLI の所有権を確認できませんでした。同期前にグローバル npm プレフィックスを確認してください。', inspectOnly: '確認のみ',
        developmentEnvironmentHealth: '開発環境の状態', componentReadyCount: ({ ready, total }: { ready: number; total: number }) => `${ready}/${total} 件のチェックが準備完了`, componentReady: '準備完了', componentWarning: '要対応', componentUnknown: '不明', pawsCli: 'Paws CLI', egoBrowser: 'Ego Lite と CLI', cloudflareWrangler: 'Cloudflare Wrangler', cloudflared: 'cloudflared', versionLatest: ({ version }: { version: string }) => `最新: ${version}`, pawsOwnershipVerified: 'グローバル npm パッケージを確認済み', pawsOwnershipUnverified: 'Paws CLI はこのグローバル npm インストールに属していません', egoAppVersion: ({ version }: { version: string }) => `Ego Lite: ${version}`, egoCliVersion: ({ version }: { version: string }) => `Ego CLI: ${version}`, egoChromiumVersion: ({ version }: { version: string }) => `Chromium: ${version}`, egoNodeVersion: ({ version }: { version: string }) => `Node: ${version}`, egoPaired: 'Ego Lite と CLI のバージョンが一致しています', egoMismatch: 'Ego Lite と CLI のバージョンが一致しません', cloudflareAuthReady: 'Cloudflare 認証済み', cloudflareAuthMissing: 'Cloudflare へのログインが必要です', cloudflareAuthUnknown: 'Cloudflare 認証状態は不明です', cloudflareAccounts: ({ accounts }: { accounts: string }) => `アカウント: ${accounts}`, tunnelCertificatePresent: 'トンネルログイン証明書あり', tunnelCertificateMissing: 'トンネルログイン証明書なし', actionPawsInstall: ({ version }: { version: string }) => `Paws CLI ${version} をインストール`, actionPawsUpgrade: ({ from, version }: { from: string; version: string }) => `Paws CLI を ${from} → ${version} に更新`, confirmPawsTitle: 'Paws CLI を同期しますか？', selectAlignmentTool: ({ component }: { component: string }) => `${component} の同期を選択`, previewComponentAlignment: ({ component }: { component: string }) => `${component} の同期をプレビュー`, confirmComponentAction: ({ component }: { component: string }) => `${component} の同期を適用`,
    },
    relationshipAdvisor: {
        title: '恋愛相談',
    ...
    relationshipAdvisorPlugin: {
        marketTitle: 'プラグイン',
        marketSubtitle: 'このバージョンのPawsに含まれるプラグインを見つけて有効化',
        searchPlaceholder: 'プラグインを検索',
        installedSection: 'インストール済み',
        featuredSection: 'おすすめ',
        emptySearch: 'プラグインが見つかりません',
        openPlugin: 'プラグインを開く',
        configure: '設定',
        configuration: 'プラグイン設定',
        permissions: '権限',
        permissionGrantNotice: 'インストールまたは更新すると、このアカウントにこれらの権限が付与されます。アンインストールすると取り消されます。',
        builtInCode: '組み込みの信頼済みコード',
        builtInCodeNotice: 'プラグインのコードは、このバージョンのPawsに同梱されています。インストールすると、このアカウントで有効になるだけです。',
        permissionAiProviderInvoke: 'AIプロバイダーを使用',
        permissionAiProviderInvokeDescription: 'このプラグインの会話リクエストを、設定済みのAIプロバイダーに送信します。',
        permissionSecretsUse: '暗号化されたシークレットを使用',
        permissionSecretsUseDescription: 'このプラグインの実行中に限り、暗号化された設定を読み取ります。',
        permissionConversationImagesRead: '会話内の画像を読み取る',
        permissionConversationImagesReadDescription: 'このプラグインへの送信を選択した画像添付ファイルを読み取ります。',
        permissionStorageImagesWrite: 'プラグインの画像を保存',
        permissionStorageImagesWriteDescription: 'このプラグインが作成または使用する画像をアップロードおよび削除します。',
        encryptionNotice: 'APIキーを含む設定は暗号化され、Pawsサーバーに保存されます。保存済みのキーがこのアプリに返されるのは、明示的に表示することを選択した場合のみです。',
        secretRevealFailed: '保存済みのキーを表示できません。もう一度お試しください。',
        apiKey: 'APIキー',
        apiKeyConfigured: ({ hint }: { hint: string }) => `設定済み · 末尾 ${hint}`,
        apiKeyPlaceholder: '例：sk-...',
        baseUrl: 'ベースURL',
        baseUrlPlaceholder: '例：https://api.deepseek.com',
        model: 'モデル',
        modelPlaceholder: '例：deepseek-v4-flash-vision-exp',
        unsavedChangesTitle: '未保存の変更',
        unsavedChangesSubtitle: '現在の設定はまだ保存されていません。移動する前に保存してください。',
        modelRecommendation: 'テキストと画像を理解できるよう、マルチモーダル対応モデルを優先してください。',
        testConnection: '接続をテスト',
        testConnectionSubtitle: 'テストに成功すると現在の設定が自動的に保存されます',
        connectionSuccess: '接続に成功しました',
        connectionSuccessSubtitle: '現在の設定でプロバイダーとモデルに接続できます',
        connectionAuthenticationFailed: '認証に失敗しました',
        connectionModelNotFound: 'モデルが見つかりません',
        connectionRateLimited: 'リクエスト上限に達しました',
        connectionTimedOut: '接続がタイムアウトしました',
        connectionUnreachable: 'プロバイダーに接続できません',
        connectionProviderError: 'プロバイダーからエラーが返されました',
        status: 'プラグインの状態',
        installed: 'インストール済み',
        installedSubtitle: 'このアカウントですぐに利用できます',
        reviewRequired: '再確認が必要',
        reviewRequiredSubtitle: 'プラグインのバージョンまたは権限が変更されています。確認して更新してから使用してください。',
        reviewAndUpdate: '確認して更新',
        notInstalled: '未インストール',
        notInstalledSubtitle: '設定を入力してプラグインをインストールしてください',
        update: '設定を更新',
        install: 'プラグインをインストール',
        installSubtitle: '設定を暗号化してPawsサーバーに保存します',
        uninstall: 'プラグインをアンインストール',
        uninstallSubtitle: 'プラグインと保存済みの認証情報を削除します',
    },
    healthPanel: {
    ...
    generatedImageBatchDownload: {
        ...
        savedBrowser: ({ count }: { count: number }) => `${count}件のダウンロードを開始しました · ブラウザに確認されたら複数ダウンロードを許可してください`,
        savedDirectory: ({ count }: { count: number }) => `${count}枚を選択したフォルダに保存しました`,
        savedPhotos: ({ count }: { count: number }) => `${count}枚を写真に保存しました`,
        cancelledBrowser: 'ダウンロードをキャンセルしました · 再試行して複数ダウンロードを許可してください',
        cancelledDirectory: 'フォルダが選択されていません · 保存先フォルダを選択してください',
        cancelledPhotos: '生成画像の保存には写真への追加権限が必要です',
        cancelledUnsupported: 'このプラットフォームでは一括ダウンロードを利用できません',
        partial: ({ succeeded, failed }: { succeeded: number; failed: number }) => `${succeeded}枚を保存 · ${failed}枚が失敗`,
        retryFailed: ({ count }: { count: number }) => `失敗分を再試行（${count}）`,
    },
    desktopWorkspace: {
        sessions: 'セッション',
    ...
    keyboardShortcuts: {
        ...
        openSettings: 'アプリの設定を開く',
        toggleLeftSidebar: 'セッションサイドバーの表示を切り替える',
        toggleRightPanel: '詳細パネルの表示を切り替える',
        closeOrGoBack: '最前面のレイヤーを閉じるか戻る',
        closeOrGoBackDetail: 'モーダル、Zenモード、オーバーレイを優先して閉じ、該当しない場合は画面履歴を戻ります',
        moveSelection: '選択項目を移動',
        runSelected: '選択したコマンドを実行',
        closeCommandPalette: 'コマンドパレットを閉じる',
        runNumberedResult: '表示中の1〜9番の結果を実行',
        sendMessage: 'メッセージを送信',
        insertNewLine: '改行を挿入',
        navigateSuggestions: '候補間を移動',
        acceptSuggestion: '候補を確定',
        closeSuggestions: '候補を閉じる',
        stopRunningAgent: '実行中のエージェントを停止',
        pressTwice: '2秒以内に2回押す',
        zoomIn: '拡大',
        zoomOut: '縮小',
        resetZoom: 'ズームをリセット',
    },
    ...
    settings: {
        ...
        about: 'このアプリについて',
        aboutFooter: 'PawsはCodexとClaude Codeのモバイルクライアントです。完全なエンドツーエンド暗号化を採用し、アカウントはデバイスにのみ保存されます。Anthropicとは提携していません。',
        whatsNew: '新機能',
    ...
    newSession: {
        ...
        submissionSending: "添付ファイルをアップロードしてメッセージをキューに追加中…",
        submissionProjecting: "メッセージを確認中…",
        submissionSaveFailed: "開始状況を保存できませんでした。ここでテキストを復元できます。",
        submissionFailed: "開始が一時停止しました。メッセージはここに保存されています。",
        submissionInterrupted: "開始が中断されました。再送信する前にセッションを確認してください。",
        submissionRestore: "テキストを復元",
        submissionCheckSession: "セッションを確認",
        submissionRestoreWarning: "セッションまたはメッセージがすでに存在する可能性があります。テキストの復元では再送信されません。再度送信する前にセッションを確認してください。",
        submissionAttachmentRecovery: "添付ファイルは受け付けられるまで選択状態が保持されます。ページ更新後は送信前に元のファイルを選択し直してください。",
        title: '新しいセッションを開始',
        askMode: 'Ask',
        agentMode: 'Agent',
    ...
    server: {
        ...
        notValidHappyServer: '有効なPaws Serverではありません',
    ...
    sessionInfo: {
        ...
        failedToRestoreSession: 'セッションを復元できませんでした',
        sessionRowRemoteLocation: ({ name }: { name: string }) => `リモート · ${name}`,
        sessionRowUnknownLocation: '場所不明/ローカル',
        sessionRowUnknownAgent: '不明なエージェント',
        sessionRowProjectPath: 'プロジェクト/パス',
        sessionRowMachineAgent: 'マシン/エージェント',
        sessionRowRelativeTime: '相対時刻',
        sessionRowRunningStatus: '実行状態',
        sessionRowMoreActions: 'その他のセッション操作',
    ...
    sessionShare: {
        ...
        coverAttribution: ({ photographer }: { photographer: string }) => `${photographer}による写真 · Pexels`,
    ...
    components: {
        ...
        messageComposer: {
            ...
            workingDirectoryBrowse: 'フォルダを参照',
            workingDirectoryRecent: '最近のディレクトリ',
            workingDirectoryNoRecent: 'このマシンに他の最近のディレクトリはありません',
            workingDirectorySelectCurrent: ({ name }: { name: string }) => `${name} を使用`,
            workingDirectoryError: ({ reason }: { reason: string }) => `このディレクトリは使用できません。パスとアクセス権を確認して再試行してください。${reason}`,
            workingDirectoryEmpty: '先にディレクトリを入力または選択してください。',
            workingDirectoryOffline: 'Agent のマシンがオフラインです。再接続してから変更してください。',
    ...
    agentInput: {
        ...
        taskPermission: {
            ...
            confirm: '確認が必要',
            confirmShort: '確認あり',
            confirmDescription: '追加の権限が必要な操作では、エージェントの確認フローを使用します。デバイスや外部サンドボックスの制限は引き続き適用されます。',
            fullAccess: 'フルアクセス',
            fullAccessShort: 'フルアクセス',
            fullAccessDescription: '対応している場合、エージェントの確認を省略します。デバイスや外部サンドボックスの制限は引き続き適用されます。',
            changesNextMessages: '次のメッセージ以降に適用されます。',
            unsupported: 'このエージェントでは、信頼できる2段階の権限を利用できません。',
            unavailable: '利用不可',
            riskTitle: 'フルアクセスを有効にしますか？',
            riskMessage: 'フルアクセスでは、対応している場合にエージェントの確認を省略し、潜在的に高リスクな操作も対象になります。デバイスや外部サンドボックスの制限を解除するものではありません。このタスクを信頼できる場合のみ続行してください。',
            riskConfirm: 'フルアクセスを有効化',
        },
        agent: {
            claude: 'Claude',
            codex: 'Codex',
    ...
    settingsVoice: {
        ...
        byoDescription: 'Paws のデフォルトの代わりに、独自の ElevenLabs エージェントを使用します。サブスクリプション不要 — 自分の ElevenLabs アカウントで直接接続できます。エージェントには2つのクライアントツールを定義する必要があります: messageClaudeCode（コーディングエージェントにテキストを送信）と processPermissionRequest（ツール使用を許可または拒否）。セッションコンテキストは {{initialConversationContext}} 動的変数を通じて受信されます。',
        customAgentId: 'ElevenLabs Agent ID',
        customAgentIdNotSet: '未設定',
        customAgentIdDescription: 'ElevenLabs Agent ID を入力してください。空のままにすると Paws のデフォルトが使用されます。',
        customAgentIdPlaceholder: 'e.g. abc123def456',
        bypassToken: '直接接続',
        bypassTokenSubtitle: 'Paws サーバーをスキップし、ElevenLabs に直接接続',
        ...
        developerTitle: '開発者',
        developerFooter: '現在の Voice 展開状況を確認する、開発者専用の診断情報とローカル制御です。直接接続とカスタム ElevenLabs エージェントの両方が有効な場合を除き、有料 Voice のゲート処理には Paws サーバーが使用されます。',
        experimentOverrideTitle: 'Voice 実験のオーバーライド',
        experimentOverrideSubtitle: 'このデバイスで Voice アップセル実験をオーバーライド',
        experimentOverrideDescription: 'Voice アップセル実験のローカルオーバーライドを選択してください。',
        experimentOverrideNone: 'オーバーライドなし',
        experimentOverrideControl: 'コントロールグループ',
        experimentOverrideSoftPaywall: 'ソフトペイウォール',
        experimentOverrideOnboardingUpsell: 'オンボーディング + アップセル',
        experimentStatusTitle: 'Voice 実験のステータス',
        experimentStatusVariant: ({ value }: { value: string }) => `Voice アップセル: ${value}`,
        experimentStatusSource: ({ value }: { value: string }) => `ソース: ${value}`,
        experimentStatusGate: ({ value }: { value: string }) => `ゲート: ${value}`,
        experimentStatusExperiments: ({ enabled }: { enabled: boolean }) => `実験機能設定: ${enabled ? 'オン' : 'オフ'}`,
        experimentStatusDirectByoAgent: '独自エージェントで直接接続',
        experimentStatusServerGate: 'Paws サーバーゲート',
        experimentSourceOverride: 'オーバーライド',
        experimentSourcePosthog: 'PostHog',
        experimentSourceDefault: 'デフォルト',
        resetCountersTitle: 'Voice カウンターをリセット',
        resetCountersMessage: 'このデバイスでオンボーディングとソフトペイウォールの動作に使用するローカル Voice カウンターを消去しますか？',
        resetCountersConfirm: 'リセット',
        counterSoftPaywallShown: ({ count }: { count: number }) => `ソフトペイウォール表示回数: ${count}`,
        counterOnboardingPromptLoads: ({ count }: { count: number }) => `オンボーディングプロンプト読込回数: ${count}`,
        counterVoiceMessages: ({ count }: { count: number }) => `Voice メッセージ数: ${count}`,
    },
    ...
    updateBanner: {
        otaOnlyTitle: "OTA 更新はありません",
        otaOnlyMessage: "このインストールで利用可能な OTA 更新はありません。このパッケージはネイティブアプリのバージョン確認に対応していません。",
        tapToDownloadGitHub: "GitHub Release からダウンロード",
        nativeUpdateMessage: "新しいアプリパッケージがあります。一部の更新には新しいパッケージが必要です。GitHub Release からダウンロードし、アンインストールせずに上書きインストールしてください。",
        downloadApp: "APK をダウンロード",
        nativeCheckFailedTitle: "アプリの更新を確認できません",
        nativeCheckFailedMessage: "新しいアプリがあるか確認できませんでした。後でもう一度お試しください。",
        ...
        forceCheckSubtitle: "アプリと OTA の更新を今すぐ確認",
        checking: 'アップデートを確認中…',
    ...
    terminal: {
        ...
        terminalRequestDescription: 'ターミナルがPawsアカウントへの接続を要求しています。これにより、ターミナルは安全にメッセージを送受信できるようになります。',
    ...
    machine: {
        ...
        codexUsageToday: '今日',
        codexUsageYesterday: '昨日',
        codexUsageInput: '入力',
        codexUsageCached: 'キャッシュ',
        codexUsageOutput: '出力',
        codexUsageReasoning: '推論',
    ...
    imageUpload: {
        ...
        mediaCollapse: ({ name }: { name: string }) => `「${name}」のプレーヤーを折りたたむ`,
        mediaFullscreen: ({ name }: { name: string }) => `「${name}」を全画面で再生`,
        mediaLoadFailed: 'このメディアファイルを読み込めませんでした。',
    ...
    agentSpace: {
        ...
        companion: {
            panelTitle: '毎日のサポート',
            panelSubtitle: '安定した生活リズムへ、小さな一歩を',
            tipBedtimeEyebrow: '就寝前 · 23:30',
            tipBedtimeTitle: '23:30までに布団へ',
            tipBedtimeBody: '就寝30分前には刺激の強いコンテンツをやめ、脳に眠りの合図を送りましょう。',
            tipMorningLightEyebrow: '朝 · 10分',
            tipMorningLightTitle: '10分間、自然光を浴びる',
            tipMorningLightBody: '起床後の早い時間に朝日を浴び、体内時計を整えましょう。',
            tipSleepWindowEyebrow: '今夜 · 7時間',
            tipSleepWindowTitle: '7時間の睡眠時間を確保',
            tipSleepWindowBody: 'まず睡眠時間を確保し、すべての習慣を一度に変えようとしないでください。',
            actionSleepTitle: '昨夜の睡眠を記録',
            actionSleepPrompt: '昨夜の睡眠を記録するのを手伝ってください。睡眠データのスクリーンショットか正確な時刻を追加します。',
            actionExerciseTitle: '1回分の運動を記録',
            actionExercisePrompt: '今日の運動を1回分記録するのを手伝ってください。運動の種類、時間、またはスクリーンショットを追加します。',
            actionDietTitle: '今日の食事を記録',
            actionDietPrompt: '今日の食事を記録するのを手伝ってください。食べ物、量、または写真を追加します。',
            actionWeeklyTitle: '今週の健康記録をまとめる',
            actionWeeklyPrompt: '今週の記録をもとに睡眠、運動、食事をまとめ、次に重視すべきことを提案してください。',
            paginationPrevious: '前のヒント',
            paginationNext: '次のヒント',
            actionAccessibility: ({ title }: { title: string }) => `クイックアクションを使用：${title}`,
            paginationAccessibility: ({ current, total }: { current: number; total: number }) => `${total}件中${current}件目のヒント`,
        },
    },
    agents: {
        cardTitle: 'マイエージェント',
        ...
        editTitle: 'エージェントを編集',
        name: '名前',
        namePlaceholder: 'このエージェントに名前を付けてください',
        machine: 'マシン',
        noMachines: '利用可能なマシンがありません',
        folder: 'フォルダ',
        folderPlaceholder: 'フォルダのパス（例: ~）',
        folderCustomPath: '上のカスタムパスを使用しています',
        folderRecent: '最近使用したフォルダ',
        folderNoRecent: '最近使用したフォルダはありません',
        presets: 'プリセット',
        addPreset: 'プリセットを追加',
        new: '新規',
        deleteConfirm: 'このエージェントを削除しますか？',
        entrySubtitle: '起動できるエージェントを管理',
        ...
        customImageStyleNoReadableImages: '読み取れる参照画像がありません。鮮明な JPG/PNG 画像を選び直してください。',
        ...
        imageStyleHealingScribbleSketchHint: '人物写真1枚を、密に探るような鉛筆線、穏やかで簡略化したアニメ顔、わずかな淡彩、広い温白色の紙面で構成する、生々しく癒やしのあるアニメ制作スケッチに変換します。\n顔型、髪、眼鏡、表情、ポーズ、特徴的な衣服または小物で本人らしさを保ち、文字は一切入れません。',
        ...
        imageStyleSceneDistillationZineHint: '写真を、意味と感情の核を保ちながら元写真の画素を使わない、3:5縦型または5:3横型のオリジナル紙 Zine イラストへ蒸留。\n表情豊かな破り縁、作家性のある文字組み、意図的な鮮やかな差し色1色。正確なトリガー 单色块模式 で連続した高彩度の単色面を有効化。',
        imageStyleGradeImages: '決定論的フォトグレーディング',
        imageStyleGradeImagesHint: '生成編集を使わず JPEG/PNG を補正、参照マッチ、または創造的にカラーグレーディング。\n構図、人物、顔、物体、文字、質感を厳密に保護し、控えめから変革的な強度、再現可能なレシピ、プレビュー、品質ゲートに対応します。',
        imageStyleScenesGatheredZine: '拾景ペーパー Zine',
        imageStyleScenesGatheredZineHint: '忠実な写真を広々とした3:5の紙ポスターに固定し、写真由来の形を高度に簡略化したイラスト領域へ展開。\n構造を担う鮮やかな1色、繊維が見える破り紙の境界、能動的な余白、控えめなマイクロテキスト。',
        imageStyleScenesGatheredZineSea: '拾景ペーパー Zine · 海',
        imageStyleScenesGatheredZineSeaHint: '同じ拾景コンパイラの海辺向け例。忠実な海と手すりの写真、広い静かな紙面、繊維質の破り境界、圧縮した印刷跡、構造色1色、控えめな文字。',
