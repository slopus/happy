...
export const pl: TranslationStructure = {
    ...
    deviceEnvironmentDashboard: {
        ...
        scan: 'Sprawdź ponownie',
        scanning: 'Sprawdzanie…',
        fleet: ({ total, online }: { total: number; online: number }) => `Urządzenia: ${total} · online: ${online}`,
        lastChecked: ({ time }: { time: string }) => `Sprawdzono: ${time}`,
        notChecked: 'Jeszcze nie sprawdzono',
        offline: 'Offline · poprzedni wynik',
        online: 'Online',
        latest: 'Najnowsza wersja',
        available: 'Dostępna aktualizacja',
        update: 'Aktualizuj',
        updateType: ({ count }: { count: number }) => `Aktualizuj narzędzie · ${count}`,
        updateAll: ({ count }: { count: number }) => `Aktualizuj dostępne · ${count}`,
        install: 'Zainstaluj',
        login: 'Zaloguj się',
        handle: 'Rozwiąż',
        retry: 'Ponów',
        queued: 'W kolejce',
        preparing: 'Sprawdzanie przed aktualizacją',
        updating: 'Aktualizowanie i weryfikacja',
        succeeded: 'Zaktualizowano · zweryfikowano',
        failed: 'Aktualizacja nie powiodła się',
        uncertain: 'Wynik niepotwierdzony',
        unresolvedHint: 'Zdalna aktualizacja może nadal trwać. Aktualizacje na tym urządzeniu są wstrzymane; sprawdzenie wersji nie potwierdza zakończenia procesu. Przed kontynuowaniem sprawdź na urządzeniu, czy aktualizacja została zakończona.',
        changed: 'Środowisko się zmieniło · sprawdź ponownie',
        pendingLogin: 'Zainstalowano · wymagane logowanie',
        pendingPair: 'Sprawdź zgodność aplikacji i CLI',
        usable: 'Zainstalowano',
        sourceUnknown: 'Najnowsza wersja niedostępna',
        ahead: 'Nowsza niż wersja z tego źródła',
        certificateOptional: 'Tunele tymczasowe dostępne · logowanie opcjonalne',
        updatesSummary: ({ count, machines }: { count: number; machines: number }) => `Dostępne aktualizacje: ${count} · urządzenia: ${machines}`,
        attentionSummary: ({ count }: { count: number }) => `Elementy wymagające osobnej konfiguracji: ${count}`,
        allCurrent: 'Brak dostępnych aktualizacji na urządzeniach online',
        batchProgress: ({ done, total }: { done: number; total: number }) => `Aktualizowane elementy: ${total} · ukończono: ${done}`,
        batchResult: ({ succeeded, failed, uncertain, skipped }: { succeeded: number; failed: number; uncertain: number; skipped: number }) => `Ukończono: ${succeeded} · błędy: ${failed} · niepotwierdzone: ${uncertain} · pominięto: ${skipped}`,
        background: 'Możesz zamknąć ten panel i sprawdzić postęp później.',
        verifyHint: 'Aktualizacje są weryfikowane automatycznie; urządzenia offline są pomijane.',
        horizontalHint: 'Przesuń w poziomie, aby porównać urządzenia',
        target: ({ version }: { version: string }) => `Dostępna wersja ${version}`,
        actionConfirmTitle: 'Kontynuować na tym urządzeniu?',
        actionConfirm: ({ machine, component, action, version }: { machine: string; component: string; action: string; version: string }) => `${machine}: ${action} ${component} (${version}). Logowanie i konfiguracja mogą otworzyć okno na tym urządzeniu.`,
        manualHint: 'Wykonaj poniższe polecenia na urządzeniu docelowym, a następnie sprawdź ponownie.',
        selectedAction: ({ machine, component, action }: { machine: string; component: string; action: string }) => `${machine} · ${component} · ${action}`,
        localSource: 'Wersja docelowa pochodzi ze źródła instalacji tego urządzenia.',
    },
    deviceEnvironment: {
        actionEgoUpgrade: ({ from, version }: { from: string; version: string }) => `Zaktualizuj Ego Lite i CLI ${from} → ${version}`,
        actionEgoOnboard: 'Dokończ konfigurację Ego CLI', actionWranglerInstall: ({ version }: { version: string }) => `Zainstaluj Wrangler ${version}`, actionWranglerUpgrade: ({ from, version }: { from: string; version: string }) => `Zaktualizuj Wrangler ${from} → ${version}`, actionWranglerAuthenticate: 'Zaloguj się do Cloudflare przez Wrangler', actionCloudflaredInstall: ({ version }: { version: string }) => `Zainstaluj cloudflared ${version}`, actionCloudflaredUpgrade: ({ from, version }: { from: string; version: string }) => `Zaktualizuj cloudflared ${from} → ${version}`, actionCloudflaredAuthenticate: 'Otwórz logowanie tunelu Cloudflare', confirmToolTitle: ({ component }: { component: string }) => `Wyrównać ${component}?`, confirmToolMessage: ({ actions }: { actions: string }) => `Sprawdź dokładną operację dla każdej maszyny:\n\n${actions}\n\nInstalacje i aktualizacje używają wyłącznie zweryfikowanego właściciela pakietu. Logowanie i konfiguracja otwierają oficjalny interaktywny proces na tej maszynie. Dane logowania nigdy nie są kopiowane między maszynami.`,
        egoPathReady: "Ego CLI jest dostępne w PATH",
        egoPathMissing: "Dodaj ~/.local/bin do PATH, aby uruchamiać ego-browser po nazwie.",
        egoCliMissing: "Brak Ego CLI. Dokończ wstępną konfigurację Ego Lite na tej maszynie.",
        egoAppMissing: "Brak aplikacji Ego Lite. Zainstaluj ją i dokończ wstępną konfigurację na tej maszynie.",
        egoUnsupported: "Sprawdzanie aplikacji Ego Lite wymaga macOS; na tej platformie wyświetlane są tylko wersje CLI.",
        pawsUnsupported: "Ujednolicanie Paws CLI wymaga Maca z Apple Silicon i zweryfikowanej globalnej instalacji npm.",
        componentUnsupported: "Sprawdzanie tego komponentu nie jest obsługiwane na tej platformie.",
        cloudflaredFormulaUnavailable: "Formuła cloudflared jest niedostępna. Sprawdź Homebrew na tej maszynie.",
        cloudflaredHomebrewMissing: "Brak Homebrew; nie można sprawdzić najnowszej wersji cloudflared.",
        componentVersionSourceMismatch: "Źródła wersji są niezgodne. Sprawdź instalację tego komponentu i skanuj ponownie.",
        title: "Środowisko urządzenia",
        subtitle: "Sprawdzaj i ujednolicaj narzędzia programistyczne na swoich maszynach",
        fleetReady: ({ ready, total }: { ready: number; total: number }) => `${ready}/${total} maszyn ma wersję docelową`,
        scanAll: "Skanuj wszystkie maszyny",
        scanning: "Skanowanie maszyn…",
        previewAlignment: "Podgląd ujednolicenia",
        previewing: "Przygotowywanie podglądu…",
        confirmTitle: "Ujednolicić GitHub CLI?",
        confirmMessage: ({ actions }: { actions: string }) => `Sprawdź dokładne działanie dla każdej maszyny:\n\n${actions}\n\nZostaną wykonane tylko wymienione instalacje i aktualizacje. Na każdej maszynie trzeba zalogować się osobno.`,
        confirmAction: "Zastosuj ujednolicenie",
        applying: "Ujednolicanie wersji…",
        completed: "Ukończono",
        githubCli: "GitHub CLI",
        daemonOnline: "Daemon połączony",
        daemonOffline: "Daemon rozłączony — pominięto",
        offlineRecovery: "Przywróć połączenie tej maszyny lub sprawdź ją przez SSH. Następnie skanuj ponownie.",
        versionInstalled: ({ version }: { version: string }) => `Zainstalowana: ${version}`,
        versionTarget: ({ version }: { version: string }) => `Docelowa: ${version}`,
        authReady: "Zalogowano do GitHub",
        authMissing: "Wymagane logowanie do GitHub",
        authUnknown: "Stan logowania do GitHub nieznany",
        actionNone: "Bez zmiany wersji",
        actionInstall: ({ version }: { version: string }) => `Zainstaluj gh ${version}`,
        actionUpgrade: ({ from, version }: { from: string; version: string }) => `Zaktualizuj gh ${from} → ${version}`,
        actionManualRepair: "Wymagana ręczna naprawa; bez automatycznych zmian",
        repairWithSsh: "Połącz się z tą maszyną przez SSH, aby dokończyć konfigurację lub naprawę.",
        repairLocally: "Otwórz terminal na tej maszynie, aby dokończyć konfigurację lub naprawę.",
        scanAgain: "Skanuj ponownie, aby odświeżyć stan maszyny",
        stateUnknown: "Stan nieznany; skanuj ponownie",
        timeoutRecovery: "Poczekaj na zakończenie Homebrew lub bieżącej operacji, a przed kolejną próbą skanuj ponownie.",
        partialFailure: "Niektóre maszyny nadal wymagają uwagi. Sprawdź wyniki poniżej.",
        versionSourceMismatch: "Źródła wersji są niezgodne. Odśwież informacje Homebrew na tych maszynach i skanuj ponownie.",
        homebrewMissing: "Brak Homebrew. Zainstaluj go na tej maszynie przed ujednoliceniem wersji.",
        unsupportedMachine: "Automatyczne ujednolicanie wymaga Maca z Apple Silicon i zainstalowanym Homebrew.",
        operationInProgress: "Na tej maszynie trwa inna operacja. Poczekaj i skanuj ponownie.",
        planExpired: "Podgląd wygasł lub stan maszyny się zmienił. Ponownie wykonaj skanowanie i podgląd.",
        formulaUnavailable: "Formuła gh jest niedostępna. Sprawdź Homebrew na tej maszynie.",
        versionAhead: "Zainstalowana wersja jest nowsza niż docelowa. Sprawdź ją ręcznie; wersja nie zostanie obniżona.",
        alignmentFailed: "Ujednolicenie nie zostało ukończone. Sprawdź tę maszynę i skanuj ponownie.",
        notInstalled: "Nie zainstalowano",
        previewRequired: "Otwórz podgląd, aby zobaczyć dokładne działanie",
        emptyFleet: "Nie zarejestrowano jeszcze maszyn. Połącz maszynę, aby rozpocząć.",
        authenticationNote: "Wersja i logowanie do GitHub są sprawdzane osobno. Zaloguj się na każdej maszynie poleceniem gh auth login.",
        developmentEnvironmentHealth: 'Stan środowiska programistycznego', componentReadyCount: ({ ready, total }: { ready: number; total: number }) => `${ready}/${total} kontroli gotowych`, componentReady: 'Gotowe', componentWarning: 'Wymaga uwagi', componentUnknown: 'Nieznane', pawsCli: 'Paws CLI', egoBrowser: 'Ego Lite i CLI', cloudflareWrangler: 'Cloudflare Wrangler', cloudflared: 'cloudflared', versionLatest: ({ version }: { version: string }) => `Najnowsza: ${version}`, pawsOwnershipVerified: 'globalny pakiet npm zweryfikowany', pawsOwnershipUnverified: 'Paws CLI nie należy do tej globalnej instalacji npm', egoAppVersion: ({ version }: { version: string }) => `Ego Lite: ${version}`, egoCliVersion: ({ version }: { version: string }) => `Ego CLI: ${version}`, egoChromiumVersion: ({ version }: { version: string }) => `Chromium: ${version}`, egoNodeVersion: ({ version }: { version: string }) => `Node: ${version}`, egoPaired: 'wersje Ego Lite i CLI są zgodne', egoMismatch: 'wersje Ego Lite i CLI są niezgodne', cloudflareAuthReady: 'Cloudflare uwierzytelniony', cloudflareAuthMissing: 'Wymagane logowanie do Cloudflare', cloudflareAuthUnknown: 'Uwierzytelnianie Cloudflare nieznane', cloudflareAccounts: ({ accounts }: { accounts: string }) => `Konta: ${accounts}`, tunnelCertificatePresent: 'Certyfikat logowania tunelu jest obecny', tunnelCertificateMissing: 'Brak certyfikatu logowania tunelu', actionPawsInstall: ({ version }: { version: string }) => `Zainstaluj Paws CLI ${version}`, actionPawsUpgrade: ({ from, version }: { from: string; version: string }) => `Zaktualizuj Paws CLI ${from} → ${version}`, confirmPawsTitle: 'Wyrównać Paws CLI?', selectAlignmentTool: ({ component }: { component: string }) => `Wybierz wyrównanie ${component}`, previewComponentAlignment: ({ component }: { component: string }) => `Podgląd wyrównania ${component}`, confirmComponentAction: ({ component }: { component: string }) => `Zastosuj wyrównanie ${component}`,
        confirmPawsMessage: ({ actions }: { actions: string }) => `Sprawdź działanie Paws CLI na każdej maszynie:\n\n${actions}\n\nZostaną uruchomione tylko wymienione globalne instalacje i aktualizacje npm. Wymagana jest zweryfikowana własność npm.`, pawsTimeoutRecovery: 'Poczekaj na zakończenie npm lub bieżącej operacji, a następnie skanuj ponownie.', componentTimeoutRecovery: 'Poczekaj na zakończenie bieżącej operacji, a następnie skanuj ponownie.', pawsOwnershipMismatch: 'Nie można było zweryfikować własności Paws CLI. Przed wyrównaniem sprawdź globalny prefiks npm.', inspectOnly: 'Tylko kontrola',
    },
    relationshipAdvisor: {
        title: 'Doradca relacji',
        cloudSubtitle: 'Szybki czat w chmurze',
        ...
        clear: 'Wyczyść',
        clearAccessibility: 'Wyczyść rozmowę',
        clearConfirmTitle: 'Wyczyścić tę rozmowę?',
        clearConfirmMessage: 'Zapisane wiadomości doradcy relacji zostaną usunięte z tego urządzenia.',
        emptyPrompt: 'Opisz sytuację, a od razu zastanowimy się, co odpisać.',
        imageCount: ({ count }: { count: number }) => `${count} ${plural({ count, one: 'obraz', few: 'obrazy', many: 'obrazów' })}`,
        unavailable: 'Nie można teraz nawiązać połączenia. Spróbuj ponownie później.',
        emptyResponse: 'Model nie zwrócił żadnego tekstu. Spróbuj ponownie.',
        stopAccessibility: 'Zatrzymaj generowanie',
        installSubtitle: 'Zainstaluj wtyczkę, aby korzystać z doradcy relacji',
    },
    relationshipAdvisorPlugin: {
        marketTitle: 'Wtyczki',
        marketSubtitle: 'Odkrywaj i włączaj wtyczki dołączone do tej wersji Paws',
        searchPlaceholder: 'Szukaj wtyczek',
        installedSection: 'Zainstalowane',
        featuredSection: 'Polecane',
        emptySearch: 'Nie znaleziono wtyczek',
        openPlugin: 'Otwórz wtyczkę',
        configure: 'Skonfiguruj',
        configuration: 'Konfiguracja wtyczki',
        permissions: 'Uprawnienia',
        permissionGrantNotice: 'Instalacja lub aktualizacja przyznaje wtyczce te uprawnienia na tym koncie. Odinstalowanie je odbiera.',
        builtInCode: 'Wbudowany zaufany kod',
        builtInCodeNotice: 'Kod wtyczki jest dostarczany z tą wersją Paws. Instalacja jedynie włącza go na Twoim koncie.',
        permissionAiProviderInvoke: 'Korzystanie z dostawcy AI',
        permissionAiProviderInvokeDescription: 'Wysyłanie żądań konwersacji tej wtyczki do skonfigurowanego dostawcy AI.',
        permissionSecretsUse: 'Korzystanie z zaszyfrowanych sekretów',
        permissionSecretsUseDescription: 'Odczytywanie zaszyfrowanej konfiguracji tej wtyczki tylko podczas jej działania.',
        permissionConversationImagesRead: 'Odczytywanie obrazów z rozmowy',
        permissionConversationImagesReadDescription: 'Odczytywanie załączników graficznych, które wybierzesz do wysłania tej wtyczce.',
        permissionStorageImagesWrite: 'Zapisywanie obrazów wtyczki',
        permissionStorageImagesWriteDescription: 'Przesyłanie i usuwanie obrazów utworzonych lub używanych przez tę wtyczkę.',
        encryptionNotice: 'Konfiguracja, w tym klucz API, jest szyfrowana i przechowywana na serwerze Paws. Zapisany klucz jest zwracany do tej aplikacji tylko po wyraźnym wybraniu opcji jego wyświetlenia.',
        secretRevealFailed: 'Nie można wyświetlić zapisanego klucza. Spróbuj ponownie.',
        apiKey: 'Klucz API',
        apiKeyConfigured: ({ hint }: { hint: string }) => `Skonfigurowano · końcówka ${hint}`,
        apiKeyPlaceholder: 'Na przykład sk-...',
        baseUrl: 'Bazowy adres URL',
        baseUrlPlaceholder: 'Na przykład https://api.deepseek.com',
        model: 'Model',
        modelPlaceholder: 'Na przykład deepseek-v4-flash-vision-exp',
        unsavedChangesTitle: 'Niezapisane zmiany',
        unsavedChangesSubtitle: 'Bieżąca konfiguracja nie została jeszcze zapisana. Zapisz ją przed wyjściem.',
        modelRecommendation: 'W pierwszej kolejności wybieraj modele obsługujące multimodalność, aby rozumiały tekst i obrazy.',
        testConnection: 'Testuj połączenie',
        testConnectionSubtitle: 'Po pomyślnym teście bieżąca konfiguracja zostanie automatycznie zapisana',
        connectionSuccess: 'Połączenie udane',
        connectionSuccessSubtitle: 'Bieżące ustawienia umożliwiają połączenie z dostawcą i modelem',
        connectionAuthenticationFailed: 'Uwierzytelnianie nie powiodło się',
        connectionModelNotFound: 'Nie znaleziono modelu',
        connectionRateLimited: 'Osiągnięto limit żądań',
        connectionTimedOut: 'Upłynął limit czasu połączenia',
        connectionUnreachable: 'Nie można połączyć się z dostawcą',
        connectionProviderError: 'Dostawca zwrócił błąd',
        status: 'Stan wtyczki',
        installed: 'Zainstalowana',
        installedSubtitle: 'Gotowa do użycia na tym koncie',
        reviewRequired: 'Wymagane ponowne sprawdzenie',
        reviewRequiredSubtitle: 'Wersja lub uprawnienia wtyczki uległy zmianie. Sprawdź i zaktualizuj przed użyciem.',
        reviewAndUpdate: 'Sprawdź i zaktualizuj',
        notInstalled: 'Niezainstalowana',
        notInstalledSubtitle: 'Uzupełnij konfigurację i zainstaluj wtyczkę',
        update: 'Aktualizuj konfigurację',
        install: 'Zainstaluj wtyczkę',
        installSubtitle: 'Zaszyfruj konfigurację i zapisz ją na serwerze Paws',
        uninstall: 'Odinstaluj wtyczkę',
        uninstallSubtitle: 'Usuń wtyczkę i zapisane dane uwierzytelniające',
    },
    healthPanel: {
    ...
    generatedImageBatchDownload: {
        preparing: ({ ready, total }: { ready: number; total: number }) => `Przygotowywanie ${ready}/${total}`,
        generating: ({ completed, total }: { completed: number; total: number }) => `Generowanie ${completed}/${total}`,
        downloadAll: ({ count }: { count: number }) => `Pobierz wszystkie (${count})`,
        downloading: ({ completed, total }: { completed: number; total: number }) => `Pobieranie ${completed}/${total}`,
        saved: ({ count }: { count: number }) => `Zapisano obrazów: ${count}`,
        savedBrowser: ({ count }: { count: number }) => `Rozpoczęto pobieranie: ${count} · Zezwól na wiele plików, jeśli przeglądarka zapyta`,
        savedDirectory: ({ count }: { count: number }) => `Zapisano ${count} obrazów w wybranym folderze`,
        savedPhotos: ({ count }: { count: number }) => `Zapisano ${count} obrazów w Zdjęciach`,
        cancelledBrowser: 'Pobieranie anulowane · Spróbuj ponownie i zezwól na wiele plików',
        cancelledDirectory: 'Nie wybrano folderu · Wybierz folder zapisu obrazów',
        cancelledPhotos: 'Zapisywanie wygenerowanych obrazów wymaga dostępu do Zdjęć',
        cancelledUnsupported: 'Pobieranie wsadowe nie jest dostępne na tej platformie',
        partial: ({ succeeded, failed }: { succeeded: number; failed: number }) => `Zapisano: ${succeeded} · Błędy: ${failed}`,
        retryFailed: ({ count }: { count: number }) => `Ponów nieudane (${count})`,
    },
    desktopWorkspace: {
        sessions: 'Sesje',
        showSessions: 'Pokaż panel boczny sesji',
        hideSessions: 'Ukryj panel boczny sesji',
    ...
    keyboardShortcuts: {
        ...
        close: 'Zamknij okno skrótów klawiaturowych',
        open: 'Otwórz skróty klawiaturowe',
        common: 'Ogólne',
        navigation: 'Nawigacja',
        commandPalette: 'Paleta poleceń',
        conversation: 'Rozmowa i wprowadzanie tekstu',
        desktopApp: 'Aplikacja komputerowa',
        openCommandPalette: 'Otwórz paletę poleceń',
        openSettings: 'Otwórz ustawienia aplikacji',
        toggleLeftSidebar: 'Pokaż lub ukryj panel boczny sesji',
        toggleRightPanel: 'Pokaż lub ukryj panel szczegółów',
        closeOrGoBack: 'Zamknij najwyższą warstwę lub wróć',
        closeOrGoBackDetail: 'Zamyka okno modalne lub nakładkę albo wyłącza tryb zen; jeśli nie ma czego zamknąć, wraca w historii nawigacji',
        moveSelection: 'Przesuwaj zaznaczenie',
        runSelected: 'Wykonaj wybrane polecenie',
        closeCommandPalette: 'Zamknij paletę poleceń',
        runNumberedResult: 'Uruchom widoczny wynik o numerze 1–9',
        sendMessage: 'Wyślij wiadomość',
        insertNewLine: 'Wstaw nowy wiersz',
        navigateSuggestions: 'Przechodź między sugestiami',
        acceptSuggestion: 'Zaakceptuj sugestię',
        closeSuggestions: 'Zamknij sugestie',
        stopRunningAgent: 'Zatrzymaj działającego agenta',
        pressTwice: 'Naciśnij dwa razy w ciągu dwóch sekund',
        zoomIn: 'Powiększ',
        zoomOut: 'Pomniejsz',
    ...
    sessionInfo: {
        ...
        agentPanelRuntimeLocation: 'Lokalizacja środowiska wykonawczego',
        agentPanelCurrentExecution: 'To wykonanie',
        agentPanelSessionManagement: 'Zarządzanie sesją',
        agentPanelMachineStatus: 'Stan maszyny',
        agentPanelAddress: 'Adres',
        agentPanelWorkingDirectory: 'Katalog roboczy',
        agentPanelAgent: 'Agent',
        agentPanelModel: 'Model',
        agentPanelEffort: 'Poziom rozumowania',
        agentPanelPermissions: 'Uprawnienia',
        agentPanelOfflineNotice: 'Ta maszyna jest offline. Ustawienia zostają zachowane, ale wykonywanie jest niedostępne.',
        agentPanelReadOnly: 'Tylko do odczytu',
        agentPanelEditable: 'Edytowalne',
        ...
        sessionRowRemoteLocation: ({ name }: { name: string }) => `Zdalnie · ${name}`,
        sessionRowUnknownLocation: 'Lokalizacja nieznana/lokalna',
        sessionRowUnknownAgent: 'Nieznany agent',
        sessionRowProjectPath: 'Projekt/ścieżka',
        sessionRowMachineAgent: 'Urządzenie/agent',
        sessionRowRelativeTime: 'Czas względny',
        sessionRowRunningStatus: 'Stan działania',
        sessionRowMoreActions: 'Więcej działań sesji',
    ...
    sessionShare: {
        ...
        confirmTitle: 'Udostępnić tę sesję?',
        confirmMessage: 'Zostanie utworzona publiczna migawka bieżącej rozmowy wraz ze wszystkimi załącznikami. Każda osoba mająca link będzie mogła ją wyświetlić.',
        confirmAction: 'Utwórz publiczny link',
        preparing: 'Przygotowywanie rozmowy…',
        uploading: ({ completed, total }: { completed: number; total: number }) => `Przesyłanie załączników (${completed}/${total})…`,
        publishing: 'Publikowanie migawki…',
        copyLink: 'Kopiuj link',
        openSharedPage: 'Otwórz udostępnioną stronę',
        updateSnapshot: 'Aktualizuj migawkę',
        revokeSharing: 'Cofnij udostępnianie',
        revokeTitle: 'Cofnąć publiczny link?',
        revokeMessage: 'Udostępniona strona i wszystkie jej załączniki natychmiast przestaną działać.',
        revokeAction: 'Cofnij link',
        shared: 'Sesja udostępniona',
        linkCopied: 'Link skopiowany',
        sharedOn: ({ date }: { date: string }) => `Udostępniono ${date}`,
        sharedViaPaws: 'Udostępniono przez Paws',
        thinking: 'Myślenie',
        toolRunning: 'W toku',
        toolCompleted: 'Ukończono',
        toolFailed: 'Niepowodzenie',
        downloadAttachment: 'Pobierz załącznik',
        attachmentUnavailable: 'Załącznik niedostępny',
        notFoundTitle: 'Ta udostępniona sesja jest niedostępna',
        notFoundMessage: 'Ten link może być nieprawidłowy lub właściciel mógł go cofnąć.',
        authenticationUnavailable: 'Uwierzytelnianie jest niedostępne',
        ...
        appearance: 'Wygląd udostępnienia',
        cover: 'Okładka',
        coverPreview: 'Podgląd okładki',
        noCover: 'Bez okładki',
        randomCover: 'Losowy obraz',
        ...
        coverAttribution: ({ photographer }: { photographer: string }) => `Zdjęcie: ${photographer} · Pexels`,
    ...
    components: {
        ...
        messageComposer: {
            ...
            workingDirectoryBrowse: 'Przeglądaj foldery',
            workingDirectoryRecent: 'Ostatnie katalogi',
            workingDirectoryNoRecent: 'Brak innych ostatnich katalogów na tym komputerze',
            workingDirectorySelectCurrent: ({ name }: { name: string }) => `Użyj ${name}`,
            workingDirectoryError: ({ reason }: { reason: string }) => `Nie można użyć tego katalogu. Sprawdź ścieżkę i dostęp, a następnie spróbuj ponownie. ${reason}`,
            workingDirectoryEmpty: 'Najpierw wpisz lub wybierz katalog.',
            workingDirectoryOffline: 'Komputer Agenta jest offline. Połącz go ponownie przed zmianą katalogu.',
    ...
    agentInput: {
        ...
        taskPermission: {
            title: 'Uprawnienia',
            confirm: 'Wymaga potwierdzenia',
            confirmShort: 'Pytaj',
            confirmDescription: 'Używa mechanizmu potwierdzania agenta przy działaniach wymagających dodatkowych uprawnień. Ograniczenia urządzenia i zewnętrznej piaskownicy nadal obowiązują.',
            fullAccess: 'Pełny dostęp',
            fullAccessShort: 'Pełny',
            fullAccessDescription: 'Pomija potwierdzenia agenta tam, gdzie jest to obsługiwane. Ograniczenia urządzenia i zewnętrznej piaskownicy nadal obowiązują.',
            changesNextMessages: 'Dotyczy następnej i kolejnych wiadomości.',
            unsupported: 'Ten agent nie oferuje dwóch wiarygodnych poziomów uprawnień.',
            unavailable: 'Niedostępne',
            riskTitle: 'Włączyć pełny dostęp?',
            riskMessage: 'Pełny dostęp pomija potwierdzenia agenta tam, gdzie jest to obsługiwane, również przy potencjalnie ryzykownych działaniach. Nie znosi ograniczeń urządzenia ani zewnętrznej piaskownicy. Kontynuuj tylko, jeśli ufasz temu zadaniu.',
            riskConfirm: 'Włącz pełny dostęp',
        },
        agent: {
            claude: 'Claude',
            codex: 'Codex',
    ...
    sidebarLists: {
        lists: 'Lists', tags: 'Tags', newList: 'New list', newTag: 'New tag', tagNamePlaceholder: 'Tag name', noTaggedSessions: 'No sessions use this tag yet.', noTags: 'No tags yet', agentList: 'Agent list', workspaceList: 'Workspace list', newSessionInList: 'New session in this list', emptyList: 'No sessions in this list yet.', unassigned: 'Unassigned', unassignedDescription: 'Sessions not in a list', organizeSession: 'Organize session', listName: 'List name', listNamePlaceholder: 'For example: Happy', duplicateListName: 'A list with this name already exists.', listType: 'List type', color: 'Color', defaultMachine: 'Default machine', defaultDirectory: 'Default directory', directoryPlaceholder: '~/projects/happy', defaultAgent: 'Default Agent', noPreset: 'No preset', agentPrompt: 'Agent prompt', agentPromptPlaceholder: 'Prompt applied when starting from this Agent list', belongsToList: 'List', tagsMultiSelect: 'Tags (select multiple)', selectedTags: 'Wybrane tagi', tagInputPlaceholder: 'Wpisz #, aby wyszukać lub utworzyć tag', createTagNamed: ({ name }: { name: string }) => `Utwórz ${name}`, tagLimitReached: 'Osiągnięto limit tagów', close: 'Close', editList: 'Edytuj listę', deleteList: 'Usuń listę', deleteListConfirm: ({ name }: { name: string }) => `Po usunięciu listy „${name}” jej sesje wrócą do Nieprzypisanych. Tagi pozostaną bez zmian.`, tagActions: 'Tag actions', groupedByList: 'Grouped by list', tagDetailsMeta: ({ sessionCount, listCount }: { sessionCount: number; listCount: number }) => `${sessionCount} sessions · ${listCount} lists`, archived: 'Zarchiwizowane', showArchived: 'Pokaż zarchiwizowane', hideArchived: 'Ukryj zarchiwizowane', archivedSessionsHidden: 'Zarchiwizowane sesje są ukryte.', tagSessionsNotLoaded: 'Powiązane sesje historyczne nie zostały jeszcze wczytane.', tagSessionsPartiallyLoaded: ({ loadedCount, sessionCount }: { loadedCount: number; sessionCount: number }) => `Wyświetlono ${loadedCount} z ${sessionCount} powiązanych sesji.`, deleteTag: 'Delete tag', deleteTagConfirm: ({ name }: { name: string }) => `Deleting the tag "${name}" removes it from every session, but does not delete any sessions.`, colors: { blue: 'Blue', green: 'Green', purple: 'Purple', orange: 'Orange', pink: 'Pink' },
    },
    sessionSearch: {
    ...
    mcpApps: {
        loading: 'Ładowanie App…',
        offline: 'Ta App jest niedostępna, gdy sesja jest offline.',
        retry: 'Spróbuj ponownie',
        unsupported: 'To urządzenie nie obsługuje MCP Apps.',
        unavailable: 'Ta App jest niedostępna.',
        openLinkTitle: 'Otworzyć link zewnętrzny?',
        openLinkMessage: 'Ta App chce otworzyć link poza Paws.',
        openLinkConfirm: 'Otwórz link',
        openLinkCancel: 'Anuluj',
    },
    ...
    toolGroup: {
        ...
        openSkillDetails: ({ title }: { title: string }) => `Pokaż szczegóły błędu umiejętności: ${title}`,
        closeSkillDetails: ({ title }: { title: string }) => `Ukryj szczegóły błędu umiejętności: ${title}`,
        skillFailureNoDetails: 'Nie zarejestrowano szczegółów diagnostycznych.',
        subagentStatus: {
            running: 'W toku',
            completed: 'Ukończony',
            failed: 'Nieudany',
    ...
    settingsVoice: {
        ...
        developerFooter: 'Diagnostyka i lokalne opcje sterowania dla deweloperów, dotyczące bieżącego wdrożenia Voice. Płatna bramka Voice korzysta z serwera Paws, chyba że włączone są zarówno Połączenie bezpośrednie, jak i niestandardowy agent ElevenLabs.',
        experimentOverrideTitle: 'Nadpisanie eksperymentu Voice',
        experimentOverrideSubtitle: 'Nadpisz eksperyment dosprzedaży Voice na tym urządzeniu',
        experimentOverrideDescription: 'Wybierz lokalne nadpisanie eksperymentu dosprzedaży Voice.',
        experimentOverrideNone: 'Bez nadpisania',
        experimentOverrideControl: 'Grupa kontrolna',
        experimentOverrideSoftPaywall: 'Miękki paywall',
        experimentOverrideOnboardingUpsell: 'Wprowadzenie + dosprzedaż',
        experimentStatusTitle: 'Stan eksperymentu Voice',
        experimentStatusVariant: ({ value }: { value: string }) => `dosprzedaż Voice: ${value}`,
        experimentStatusSource: ({ value }: { value: string }) => `źródło: ${value}`,
        experimentStatusGate: ({ value }: { value: string }) => `bramka: ${value}`,
        experimentStatusExperiments: ({ enabled }: { enabled: boolean }) => `ustawienie eksperymentów: ${enabled ? 'włączone' : 'wyłączone'}`,
        experimentStatusDirectByoAgent: 'bezpośrednie połączenie z własnym agentem',
        experimentStatusServerGate: 'bramka serwera Paws',
        experimentSourceOverride: 'nadpisanie',
        experimentSourcePosthog: 'PostHog',
        experimentSourceDefault: 'domyślne',
        resetCountersTitle: 'Resetuj liczniki Voice',
        resetCountersMessage: 'Wyczyścić lokalne liczniki Voice używane do wprowadzenia i działania miękkiego paywalla na tym urządzeniu?',
        resetCountersConfirm: 'Resetuj',
        counterSoftPaywallShown: ({ count }: { count: number }) => `wyświetlenia miękkiego paywalla: ${count}`,
        counterOnboardingPromptLoads: ({ count }: { count: number }) => `wczytania monitu wprowadzającego: ${count}`,
        counterVoiceMessages: ({ count }: { count: number }) => `wiadomości Voice: ${count}`,
    },
    ...
    settingsLanguage: {
        ...
        needsRestart: 'Wymagane ponowne uruchomienie',
    ...
    updateBanner: {
        otaOnlyTitle: "Brak aktualizacji OTA",
        otaOnlyMessage: "Dla tej instalacji nie ma aktualizacji OTA. Ten pakiet nie obsługuje sprawdzania natywnej wersji aplikacji.",
        tapToDownloadGitHub: "Pobierz z GitHub Release",
        nativeUpdateMessage: "Dostępny jest nowy pakiet aplikacji. Niektóre aktualizacje wymagają nowego pakietu. Pobierz go z GitHub Release i zainstaluj bez odinstalowywania obecnej aplikacji.",
        downloadApp: "Pobierz APK",
        nativeCheckFailedTitle: "Nie można sprawdzić aktualizacji aplikacji",
        nativeCheckFailedMessage: "Nie udało się potwierdzić dostępności nowej aplikacji. Spróbuj ponownie później.",
        ...
        forceCheckSubtitle: "Sprawdź teraz aktualizacje aplikacji i OTA",
    ...
    machine: {
        ...
        codexUsageHeatmap: 'Aktywność w ciągu ostatniego roku',
        codexUsageHeatmapLegend: 'Mniej → Więcej',
        codexUsageHeatmapDay: ({ date, tokens, sessions }: { date: string; tokens: string; sessions: number }) => `${date}: ${tokens} tokenów · ${sessions} ses.`,
    ...
    markdown: {
        ...
        mermaidOpenFullscreen: 'Otwórz diagram na pełnym ekranie',
        mermaidCloseFullscreen: 'Zamknij diagram pełnoekranowy',
    },
    ...
    imageUpload: {
        ...
        mediaCollapse: ({ name }: { name: string }) => `Zwiń odtwarzacz „${name}”`,
        mediaFullscreen: ({ name }: { name: string }) => `Odtwórz „${name}” na pełnym ekranie`,
        mediaLoadFailed: 'Nie udało się wczytać pliku multimedialnego.',
    ...
    agentSpace: {
        ...
        entering: 'Wchodzenie do przestrzeni…',
        enterFailed: 'Nie udało się wejść do przestrzeni tego agenta.',
        companion: {
            panelTitle: 'Codzienny pomocnik',
            panelSubtitle: 'Małe kroki do bardziej stabilnego rytmu',
            tipBedtimeEyebrow: 'Przed snem · 23:30',
            tipBedtimeTitle: 'Połóż się do 23:30',
            tipBedtimeBody: 'Wyłącz pobudzające treści 30 minut wcześniej, aby dać mózgowi wyraźny sygnał do snu.',
            tipMorningLightEyebrow: 'Rano · 10 min',
            tipMorningLightTitle: '10 minut naturalnego światła',
            tipMorningLightBody: 'Wyjdź na poranne światło krótko po przebudzeniu, aby ustabilizować rytm dobowy.',
            tipSleepWindowEyebrow: 'Dziś w nocy · 7 godzin',
            tipSleepWindowTitle: 'Zarezerwuj 7 godzin na sen',
            tipSleepWindowBody: 'Najpierw zadbaj o czas przeznaczony na sen; nie próbuj zmieniać wszystkich nawyków naraz.',
            actionSleepTitle: 'Zapisz sen z ostatniej nocy',
            actionSleepPrompt: 'Pomóż mi zapisać sen z ostatniej nocy. Dodam zrzut ekranu lub dokładne godziny.',
            actionExerciseTitle: 'Zapisz trening',
            actionExercisePrompt: 'Pomóż mi zapisać dzisiejszy trening. Dodam rodzaj ćwiczenia, czas trwania lub zrzut ekranu.',
            actionDietTitle: 'Zapisz dzisiejszą dietę',
            actionDietPrompt: 'Pomóż mi zapisać dzisiejszą dietę. Dodam produkty, porcje lub zdjęcia.',
            actionWeeklyTitle: 'Podsumuj dane zdrowotne z tego tygodnia',
            actionWeeklyPrompt: 'Na podstawie zapisów z tego tygodnia podsumuj mój sen, ćwiczenia i dietę, a następnie doradź, na czym skupić się dalej.',
            paginationPrevious: 'Poprzednia wskazówka',
            paginationNext: 'Następna wskazówka',
            actionAccessibility: ({ title }: { title: string }) => `Użyj szybkiej akcji: ${title}`,
            paginationAccessibility: ({ current, total }: { current: number; total: number }) => `Wskazówka ${current} z ${total}`,
        },
    },
    agents: {
        ...
        duplicatePath: 'Agent dla tej maszyny i folderu już istnieje.',
        ...
        folderCustomPath: 'Używany jest niestandardowy folder powyżej',
        folderRecent: 'Ostatnie foldery',
        folderNoRecent: 'Brak ostatnio używanych folderów',
        presets: 'Presety',
        ...
        customImageStyleNoReadableImages: 'Brak czytelnych obrazów referencyjnych. Wybierz wyraźne pliki JPG/PNG i spróbuj ponownie.',
        ...
        imageStyleSceneDistillationZineHint: 'Przekształć zdjęcie w oryginalną papierową ilustrację Zine 3:5 lub 5:3, zachowując znaczeniowe i emocjonalne punkty odniesienia, ale bez pikseli oryginalnej fotografii.\nWyraziste poszarpane krawędzie, autorska typografia i jeden celowy, żywy akcent; dokładny wyzwalacz 单色块模式 włącza jednolite nasycone pole koloru.',
        imageStyleGradeImages: 'Deterministyczny grading zdjęć',
        imageStyleGradeImagesHint: 'Koryguje, dopasowuje lub kreatywnie zmienia kolor zdjęć JPEG/PNG bez edycji generatywnej.\nŚciśle chroni geometrię, tożsamość, twarze, obiekty, tekst i fakturę; obsługuje intensywność od zachowawczej do transformacyjnej, powtarzalne receptury, podglądy i bramki jakości.',
        imageStyleScenesGatheredZine: 'Zine zebranych scen',
        imageStyleScenesGatheredZineHint: 'Kotwiczy wierną fotografię w przestronnym, dotykowym plakacie 3:5 i rozwija kształty źródłowe w mocno uproszczone pole ilustracji.\nJeden strukturalny żywy kolor, włóknista granica rozdartego papieru, aktywna pustka i dyskretny mikrotekst.',
        imageStyleScenesGatheredZineSea: 'Zine zebranych scen · Morze',
        imageStyleScenesGatheredZineSeaHint: 'Nadmorski przykład tego samego kompilatora: wierne zdjęcie morza/barierki, szerokie spokojne pole papieru, włókniste rozdarcia, skondensowane ślady druku, jeden kolor strukturalny i dyskretny mikrotekst.',
