...
export const zhHant: TranslationStructure = {
    ...
    sessionInfo: {
        ...
        worktree: {
            ...
            createPRConfirm: ({ branch }: { branch: string }) => `為分支 "${branch}" 建立 Pull Request？`,
            createPRSuccess: ({ url }: { url: string }) => `Pull Request 已建立：${url}`,
            createPRFailed: '建立 Pull Request 失敗',
            ghNotInstalled: '此機器未安裝 GitHub CLI (gh)',
            mergeBranch: '合併分支',
            mergeBranchSubtitle: '將 worktree 分支合併回基礎分支',
            mergeConfirm: ({ branch }: { branch: string }) => `將 "${branch}" 合併到基礎分支？`,
            mergeSuccess: '分支合併成功',
            mergeFailed: '合併分支失敗',
            mergeConflicts: '合併存在衝突，請在機器上手動解決。',
            cleanup: '移除 Worktree',
            cleanupSubtitle: '刪除 worktree 目錄（分支將保留）',
            cleanupConfirm: '確定移除 worktree？分支將保留以確保安全。',
