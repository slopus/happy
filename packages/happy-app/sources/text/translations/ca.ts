...
export const ca: TranslationStructure = {
    ...
    deviceEnvironmentDashboard: {
        ...
        scan: 'Torna a comprovar',
        scanning: 'S’està comprovant…',
        fleet: ({ total, online }: { total: number; online: number }) => `${total} dispositius · ${online} en línia`,
        lastChecked: ({ time }: { time: string }) => `Comprovat: ${time}`,
        notChecked: 'Encara no s’ha comprovat',
        offline: 'Fora de línia · resultat anterior',
        online: 'En línia',
        latest: 'Actualitzat',
        available: 'Actualització disponible',
        update: 'Actualitza',
        updateType: ({ count }: { count: number }) => `Actualitza l’eina · ${count}`,
        updateAll: ({ count }: { count: number }) => `Actualitza els disponibles · ${count}`,
        install: 'Instal·la',
        login: 'Inicia sessió',
        handle: 'Resol',
        retry: 'Torna-ho a provar',
        queued: 'A la cua',
        preparing: 'S’està comprovant l’actualització',
        updating: 'S’està actualitzant i verificant',
        succeeded: 'Actualitzat · verificat',
        failed: 'L’actualització ha fallat',
        uncertain: 'Resultat sense confirmar',
        unresolvedHint: 'És possible que encara s’estigui executant una actualització remota. Les actualitzacions d’aquest dispositiu estan en pausa; comprovar les versions no confirma que el procés hagi acabat. Verifica que hagi finalitzat al dispositiu abans de continuar.',
        changed: 'L’entorn ha canviat · torna a revisar-lo',
        pendingLogin: 'Instal·lat · cal iniciar sessió',
        pendingPair: 'Hi ha un problema de compatibilitat entre l’app i la CLI',
        usable: 'Instal·lat',
        sourceUnknown: 'No es pot obtenir l’última versió',
        ahead: 'Més recent que aquesta font',
        certificateOptional: 'Túnels temporals disponibles · inici de sessió opcional',
        updatesSummary: ({ count, machines }: { count: number; machines: number }) => `${count} actualitzacions en ${machines} dispositius`,
        attentionSummary: ({ count }: { count: number }) => `${count} elements requereixen configuració individual`,
        allCurrent: 'No hi ha actualitzacions disponibles als dispositius en línia',
        batchProgress: ({ done, total }: { done: number; total: number }) => `S’estan actualitzant ${total} elements · ${done} completats`,
        batchResult: ({ succeeded, failed, uncertain, skipped }: { succeeded: number; failed: number; uncertain: number; skipped: number }) => `Completats: ${succeeded} · fallits: ${failed} · sense confirmar: ${uncertain} · omesos: ${skipped}`,
        background: 'Pots tancar aquest tauler i consultar el progrés més tard.',
        verifyHint: 'Les actualitzacions es verifiquen automàticament; s’ometen els dispositius fora de línia.',
        horizontalHint: 'Llisca horitzontalment per comparar dispositius',
        target: ({ version }: { version: string }) => `Disponible: ${version}`,
        actionConfirmTitle: 'Vols continuar en aquest dispositiu?',
        actionConfirm: ({ machine, component, action, version }: { machine: string; component: string; action: string; version: string }) => `${machine}: ${action} ${component} (${version}). L’inici de sessió i la configuració poden obrir una finestra en aquest dispositiu.`,
        manualHint: 'Executa les ordres següents al dispositiu de destinació i torna a comprovar.',
        selectedAction: ({ machine, component, action }: { machine: string; component: string; action: string }) => `${machine} · ${component} · ${action}`,
        localSource: 'La versió de destinació prové de la font d’instal·lació d’aquest dispositiu.',
    },
    deviceEnvironment: {
        actionEgoUpgrade: ({ from, version }: { from: string; version: string }) => `Actualitza Ego Lite i CLI ${from} → ${version}`,
        actionEgoOnboard: 'Completa la configuració inicial d’Ego CLI', actionWranglerInstall: ({ version }: { version: string }) => `Instal·la Wrangler ${version}`, actionWranglerUpgrade: ({ from, version }: { from: string; version: string }) => `Actualitza Wrangler ${from} → ${version}`, actionWranglerAuthenticate: 'Inicia sessió a Cloudflare amb Wrangler', actionCloudflaredInstall: ({ version }: { version: string }) => `Instal·la cloudflared ${version}`, actionCloudflaredUpgrade: ({ from, version }: { from: string; version: string }) => `Actualitza cloudflared ${from} → ${version}`, actionCloudflaredAuthenticate: 'Obre l’inici de sessió del túnel de Cloudflare', confirmToolTitle: ({ component }: { component: string }) => `Alinear ${component}?`, confirmToolMessage: ({ actions }: { actions: string }) => `Revisa l’acció exacta per a cada màquina:\n\n${actions}\n\nLes instal·lacions i actualitzacions només utilitzen el propietari verificat del paquet. L’inici de sessió i la configuració inicial obren el flux interactiu oficial a la màquina. Les credencials no es copien mai entre màquines.`,
        egoPathReady: "Ego CLI està disponible al PATH",
        egoPathMissing: "Afegeix ~/.local/bin al PATH per executar ego-browser pel nom.",
        egoCliMissing: "Falta Ego CLI. Completa la configuració inicial d’Ego Lite en aquesta màquina.",
        egoAppMissing: "Falta l’aplicació Ego Lite. Instal·la-la i completa’n la configuració inicial en aquesta màquina.",
        egoUnsupported: "Les comprovacions de l’aplicació Ego Lite requereixen macOS; en aquesta plataforma només es mostren les versions de la CLI.",
        pawsUnsupported: "L’alineació de Paws CLI requereix un Mac amb Apple Silicon i una instal·lació global d’npm verificada.",
        componentUnsupported: "La comprovació d’aquest component no és compatible amb aquesta plataforma.",
        cloudflaredFormulaUnavailable: "La fórmula cloudflared no està disponible. Revisa Homebrew en aquesta màquina.",
        cloudflaredHomebrewMissing: "Falta Homebrew; no es pot comprovar l’última versió de cloudflared.",
        componentVersionSourceMismatch: "Les fonts de versió no coincideixen. Revisa la instal·lació d’aquest component i torna a escanejar.",
        title: "Entorn del dispositiu",
        subtitle: "Inspecciona i alinea les eines de desenvolupament de les teves màquines",
        fleetReady: ({ ready, total }: { ready: number; total: number }) => `${ready}/${total} màquines a la versió objectiu`,
        scanAll: "Escaneja totes les màquines",
        scanning: "S’estan escanejant les màquines…",
        previewAlignment: "Previsualitza l’alineació",
        previewing: "S’està preparant la previsualització…",
        confirmTitle: "Vols alinear GitHub CLI?",
        confirmMessage: ({ actions }: { actions: string }) => `Revisa l’acció exacta per a cada màquina:\n\n${actions}\n\nNomés s’executaran les instal·lacions i actualitzacions indicades. Cal iniciar sessió a cada màquina.`,
        confirmAction: "Aplica l’alineació",
        applying: "S’està aplicant l’alineació…",
        completed: "Completat",
        githubCli: "GitHub CLI",
        daemonOnline: "Daemon connectat",
        daemonOffline: "Daemon desconnectat: s’ha omès",
        offlineRecovery: "Restableix la connexió d’aquesta màquina o revisa-la per SSH. Després, torna a escanejar.",
        versionInstalled: ({ version }: { version: string }) => `Instal·lada: ${version}`,
        versionTarget: ({ version }: { version: string }) => `Objectiu: ${version}`,
        authReady: "GitHub autenticat",
        authMissing: "Cal iniciar sessió a GitHub",
        authUnknown: "Autenticació de GitHub desconeguda",
        actionNone: "Cap canvi de versió",
        actionInstall: ({ version }: { version: string }) => `Instal·la gh ${version}`,
        actionUpgrade: ({ from, version }: { from: string; version: string }) => `Actualitza gh ${from} → ${version}`,
        actionManualRepair: "Cal una reparació manual; no es faran canvis automàtics",
        repairWithSsh: "Connecta’t a aquesta màquina per SSH per completar la configuració o reparació.",
        repairLocally: "Obre un terminal en aquesta màquina per completar la configuració o reparació.",
        scanAgain: "Torna a escanejar per actualitzar l’estat de la màquina",
        stateUnknown: "Estat desconegut; torna a escanejar",
        timeoutRecovery: "Espera que Homebrew o l’operació actual acabi i torna a escanejar abans de reintentar-ho.",
        partialFailure: "Algunes màquines encara necessiten atenció. Revisa’n els resultats a continuació.",
        versionSourceMismatch: "Les fonts de versió no coincideixen. Actualitza la informació de Homebrew a les màquines afectades i torna a escanejar.",
        homebrewMissing: "Falta Homebrew. Instal·la’l en aquesta màquina abans d’alinear.",
        unsupportedMachine: "L’alineació automàtica requereix un Mac amb Apple Silicon i Homebrew instal·lat.",
        operationInProgress: "Hi ha una altra operació en curs en aquesta màquina. Espera i torna a escanejar.",
        planExpired: "La previsualització ha caducat o la màquina ha canviat. Torna a escanejar i previsualitzar.",
        formulaUnavailable: "La fórmula gh no està disponible. Revisa Homebrew en aquesta màquina.",
        versionAhead: "La versió instal·lada és posterior a l’objectiu. Revisa-la manualment; no es baixarà de versió.",
        alignmentFailed: "L’alineació no s’ha completat. Revisa aquesta màquina i torna a escanejar.",
        notInstalled: "No instal·lat",
        previewRequired: "Previsualitza per veure l’acció exacta",
        emptyFleet: "Encara no hi ha màquines registrades. Connecta’n una per començar.",
        authenticationNote: "La versió i l’autenticació de GitHub es comproven per separat. Inicia sessió a cada màquina amb gh auth login.",
        confirmPawsMessage: ({ actions }: { actions: string }) => `Revisa l’acció de Paws CLI a cada màquina:\n\n${actions}\n\nNomés s’executaran les instal·lacions i actualitzacions globals d’npm indicades. Cal una propietat d’npm verificada.`, pawsTimeoutRecovery: 'Espera que npm o l’operació actual acabi i torna a escanejar.', componentTimeoutRecovery: 'Espera que l’operació actual acabi i torna a escanejar.', pawsOwnershipMismatch: 'No s’ha pogut verificar la propietat de Paws CLI. Comprova el prefix global d’npm abans d’alinear.', inspectOnly: 'Només inspecció',
        developmentEnvironmentHealth: 'Estat de l’entorn de desenvolupament', componentReadyCount: ({ ready, total }: { ready: number; total: number }) => `${ready}/${total} comprovacions a punt`, componentReady: 'A punt', componentWarning: 'Cal atenció', componentUnknown: 'Desconegut', pawsCli: 'Paws CLI', egoBrowser: 'Ego Lite i CLI', cloudflareWrangler: 'Cloudflare Wrangler', cloudflared: 'cloudflared', versionLatest: ({ version }: { version: string }) => `Última: ${version}`, pawsOwnershipVerified: 'paquet global npm verificat', pawsOwnershipUnverified: 'Paws CLI no pertany a aquesta instal·lació global d’npm', egoAppVersion: ({ version }: { version: string }) => `Ego Lite: ${version}`, egoCliVersion: ({ version }: { version: string }) => `Ego CLI: ${version}`, egoChromiumVersion: ({ version }: { version: string }) => `Chromium: ${version}`, egoNodeVersion: ({ version }: { version: string }) => `Node: ${version}`, egoPaired: 'les versions d’Ego Lite i CLI coincideixen', egoMismatch: 'les versions d’Ego Lite i CLI no coincideixen', cloudflareAuthReady: 'Cloudflare autenticat', cloudflareAuthMissing: 'Cal iniciar sessió a Cloudflare', cloudflareAuthUnknown: 'Autenticació de Cloudflare desconeguda', cloudflareAccounts: ({ accounts }: { accounts: string }) => `Comptes: ${accounts}`, tunnelCertificatePresent: 'Certificat d’inici de sessió del túnel present', tunnelCertificateMissing: 'Falta el certificat d’inici de sessió del túnel', actionPawsInstall: ({ version }: { version: string }) => `Instal·la Paws CLI ${version}`, actionPawsUpgrade: ({ from, version }: { from: string; version: string }) => `Actualitza Paws CLI ${from} → ${version}`, confirmPawsTitle: 'Alinear Paws CLI?', selectAlignmentTool: ({ component }: { component: string }) => `Selecciona l’alineació de ${component}`, previewComponentAlignment: ({ component }: { component: string }) => `Previsualitza l’alineació de ${component}`, confirmComponentAction: ({ component }: { component: string }) => `Aplica l’alineació de ${component}`,
    },
    relationshipAdvisor: {
    ...
    relationshipAdvisorPlugin: {
        marketTitle: 'Complements',
        marketSubtitle: 'Descobreix i activa els complements inclosos amb aquesta versió de Paws.',
        searchPlaceholder: 'Cerca complements',
        installedSection: 'Instal·lats',
        featuredSection: 'Destacats',
        emptySearch: 'No s’ha trobat cap complement',
        openPlugin: 'Obre el complement',
        configure: 'Configura',
        configuration: 'Configuració del complement',
        permissions: 'Permisos',
        permissionGrantNotice: 'En instal·lar o actualitzar, es concedeixen aquests permisos a aquest compte. En desinstal·lar, es revoquen.',
        builtInCode: 'Codi integrat de confiança',
        builtInCodeNotice: 'El codi del complement s’inclou amb aquesta versió de Paws. La instal·lació només l’activa per al teu compte.',
        permissionAiProviderInvoke: 'Utilitza un proveïdor d’IA',
        permissionAiProviderInvokeDescription: 'Envia les sol·licituds de conversa d’aquest complement al proveïdor d’IA configurat.',
        permissionSecretsUse: 'Utilitza secrets xifrats',
        permissionSecretsUseDescription: 'Llegeix la configuració xifrada d’aquest complement només mentre s’executa.',
        permissionConversationImagesRead: 'Llegeix imatges de la conversa',
        permissionConversationImagesReadDescription: 'Llegeix els fitxers d’imatge adjunts que decideixis enviar a aquest complement.',
        permissionStorageImagesWrite: 'Desa imatges del complement',
        permissionStorageImagesWriteDescription: 'Puja i suprimeix imatges creades o utilitzades per aquest complement.',
        encryptionNotice: 'La configuració, inclosa la clau de l’API, es xifra i es desa al servidor de Paws. Una clau desada només es retorna a aquesta aplicació quan tries explícitament mostrar-la.',
        secretRevealFailed: 'No s’ha pogut mostrar la clau desada. Torna-ho a provar.',
        apiKey: 'Clau de l’API',
        apiKeyConfigured: ({ hint }: { hint: string }) => `Configurada · acaba en ${hint}`,
        apiKeyPlaceholder: 'Per exemple: sk-...',
        baseUrl: 'URL base',
        baseUrlPlaceholder: 'Per exemple: https://api.deepseek.com',
        model: 'Model',
        modelPlaceholder: 'Per exemple: deepseek-v4-flash-vision-exp',
        unsavedChangesTitle: 'Canvis sense desar',
        unsavedChangesSubtitle: 'La configuració actual encara no s’ha desat. Desa-la abans de sortir',
        modelRecommendation: 'Tria preferentment un model compatible amb multimodalitat per entendre text i imatges',
        testConnection: 'Prova la connexió',
        testConnectionSubtitle: 'Després d’una prova correcta, la configuració actual es desa automàticament',
        connectionSuccess: 'Connexió correcta',
        connectionSuccessSubtitle: 'La configuració actual permet connectar amb el proveïdor i el model',
        connectionAuthenticationFailed: 'Ha fallat l’autenticació',
        connectionModelNotFound: 'No s’ha trobat el model',
        connectionRateLimited: 'S’ha assolit el límit de sol·licituds',
        connectionTimedOut: 'S’ha esgotat el temps de connexió',
        connectionUnreachable: 'No s’ha pogut contactar amb el proveïdor',
        connectionProviderError: 'El proveïdor ha retornat un error',
        status: 'Estat del complement',
        installed: 'Instal·lat',
        installedSubtitle: 'A punt per utilitzar en aquest compte',
        reviewRequired: 'Cal revisar',
        reviewRequiredSubtitle: 'La versió o els permisos han canviat. Revisa i actualitza abans de fer-lo servir.',
        reviewAndUpdate: 'Revisa i actualitza',
        notInstalled: 'No instal·lat',
        notInstalledSubtitle: 'Completa la configuració i instal·la el complement',
        update: 'Actualitza la configuració',
        install: 'Instal·la el complement',
        installSubtitle: 'Xifra la configuració i desa-la al servidor de Paws',
        uninstall: 'Desinstal·la el complement',
        uninstallSubtitle: 'Suprimeix el complement i les credencials desades',
    },
    healthPanel: {
        ...
        scoreLabel: ' pts',
        tonightSleep: 'Aquesta nit',
        deep: 'Profund',
        light: 'Superficial',
        rem: 'REM',
        bedtime: 'Adormit',
        wakeTime: 'Despert',
        structureTitle: 'Fases del Somni',
        viewBar: 'Barres',
        viewDonut: 'Anell',
    ...
    generatedImageBatchDownload: {
        preparing: ({ ready, total }: { ready: number; total: number }) => `Preparant ${ready}/${total}`,
        generating: ({ completed, total }: { completed: number; total: number }) => `Generant ${completed}/${total}`,
        downloadAll: ({ count }: { count: number }) => `Baixa-les totes (${count})`,
        downloading: ({ completed, total }: { completed: number; total: number }) => `Baixant ${completed}/${total}`,
        saved: ({ count }: { count: number }) => `${count} imatges desades`,
        savedBrowser: ({ count }: { count: number }) => `${count} baixades iniciades · Permet diverses baixades si el navegador ho demana`,
        savedDirectory: ({ count }: { count: number }) => `${count} imatges desades a la carpeta seleccionada`,
        savedPhotos: ({ count }: { count: number }) => `${count} imatges desades a Fotos`,
        cancelledBrowser: 'Baixades cancel·lades · Torna-ho a provar i permet diverses baixades',
        cancelledDirectory: 'No s’ha seleccionat cap carpeta · Tria on desar les imatges',
        cancelledPhotos: 'Cal accés a Fotos per desar les imatges generades',
        cancelledUnsupported: 'La baixada per lots no està disponible en aquesta plataforma',
        partial: ({ succeeded, failed }: { succeeded: number; failed: number }) => `${succeeded} desades · ${failed} amb error`,
        retryFailed: ({ count }: { count: number }) => `Torna a provar les fallides (${count})`,
    },
    desktopWorkspace: {
        sessions: 'Sessions',
        showSessions: 'Mostra la barra lateral de sessions',
    ...
    keyboardShortcuts: {
        ...
        close: 'Tanca les dreceres de teclat',
        open: 'Obre les dreceres de teclat',
        common: 'Comunes',
        navigation: 'Navegació',
        commandPalette: 'Paleta de comandes',
        conversation: 'Conversa i entrada',
        desktopApp: 'Aplicació d\'escriptori',
        openCommandPalette: 'Obre la paleta de comandes',
        openSettings: 'Obre la configuració de l\'aplicació',
        toggleLeftSidebar: 'Mostra o amaga la barra lateral de sessions',
        toggleRightPanel: 'Mostra o amaga el tauler de detalls',
        closeOrGoBack: 'Tanca la capa superior o torna enrere',
        closeOrGoBackDetail: 'Tanca el diàleg modal o la superposició, o surt del mode zen; si no hi ha res per tancar, torna enrere a l\'historial de navegació',
        moveSelection: 'Mou la selecció',
        runSelected: 'Executa la comanda seleccionada',
        closeCommandPalette: 'Tanca la paleta de comandes',
        runNumberedResult: 'Executa el resultat visible numerat de l\'1 al 9',
        sendMessage: 'Envia un missatge',
        insertNewLine: 'Insereix una línia nova',
        navigateSuggestions: 'Navega pels suggeriments',
        acceptSuggestion: 'Accepta el suggeriment',
        closeSuggestions: 'Tanca els suggeriments',
        stopRunningAgent: 'Atura l\'agent en execució',
        pressTwice: 'Prem dues vegades en dos segons',
    ...
    inbox: {
        ...
        searchCodexCandidatesPlaceholder: 'Cerca per títol, projecte o ordinador',
        noCodexCandidatesFound: 'No hi ha sessions Codex coincidents.',
    },
    ...
    rightPanelCapabilityHub: {
        ...
        empty: {
            ...
            folderBrowser: 'Aquesta carpeta és buida',
        },
        folderBrowser: {
            upOneLevel: 'Pujar un nivell',
    ...
    sessionInfo: {
        ...
        restoreSession: 'Restaura la sessió',
        failedToRestoreSession: 'No s\'ha pogut restaurar la sessió',
        sessionRowRemoteLocation: ({ name }: { name: string }) => `Remot · ${name}`,
        sessionRowUnknownLocation: 'Ubicació desconeguda/local',
        sessionRowUnknownAgent: 'Agent desconegut',
        sessionRowProjectPath: 'Projecte/ruta',
        sessionRowMachineAgent: 'Equip/agent',
        sessionRowRelativeTime: 'Temps relatiu',
        sessionRowRunningStatus: 'Estat d\'execució',
        sessionRowMoreActions: 'Més accions de sessió',
    ...
    sessionShare: {
        ...
        appearance: 'Aparença de la compartició',
        cover: 'Portada',
        coverPreview: 'Previsualització de la portada',
        noCover: 'Sense portada',
        randomCover: 'Imatge aleatòria',
        uploadCover: 'Puja una imatge',
        removeCover: 'Elimina la portada',
        coverProviderUnavailable: 'Pexels no està disponible. Encara podeu pujar una imatge o compartir sense portada.',
        coverUploadFailed: 'No s\'ha pogut seleccionar la imatge. Torneu-ho a provar.',
        themeColor: 'Color del tema',
        themeColorOption: ({ theme }: { theme: string }) => `Color del tema: ${theme}`,
        appearanceLight: 'Clar',
        appearanceDark: 'Fosc',
        appearanceSystem: 'Sistema',
        coverAttribution: ({ photographer }: { photographer: string }) => `Foto de ${photographer} a Pexels`,
    ...
    components: {
        ...
        messageComposer: {
            ...
            workingDirectoryBrowse: 'Explora carpetes',
            workingDirectoryRecent: 'Directoris recents',
            workingDirectoryNoRecent: 'No hi ha altres directoris recents en aquesta màquina',
            workingDirectorySelectCurrent: ({ name }: { name: string }) => `Utilitza ${name}`,
            workingDirectoryError: ({ reason }: { reason: string }) => `No es pot utilitzar aquest directori. Comprova la ruta i l\'accés i torna-ho a provar. ${reason}`,
            workingDirectoryEmpty: 'Introdueix o selecciona primer un directori.',
            workingDirectoryOffline: 'La màquina de l\'Agent està fora de línia. Torna-la a connectar abans de canviar el directori.',
    ...
    agentInput: {
        ...
        taskPermission: {
            title: 'Permisos',
            confirm: 'Requereix confirmació',
            confirmShort: 'Pregunta',
            confirmDescription: "Utilitza el flux de confirmació de l'agent per a les accions que necessiten permisos addicionals. Els límits del dispositiu i de l'entorn aïllat extern continuen vigents.",
            fullAccess: 'Accés complet',
            fullAccessShort: 'Complet',
            fullAccessDescription: "Omet les confirmacions de l'agent quan és compatible. Els límits del dispositiu i de l'entorn aïllat extern continuen vigents.",
            changesNextMessages: 'S’aplica al missatge següent i als posteriors.',
            unsupported: 'Aquest agent no ofereix dos nivells de permisos fiables.',
            unavailable: 'No disponible',
            riskTitle: 'Vols activar l’accés complet?',
            riskMessage: "L'accés complet omet les confirmacions de l'agent quan és compatible, fins i tot per a accions potencialment perilloses. No anul·la els límits del dispositiu ni de l'entorn aïllat extern. Continua només si confies en aquesta tasca.",
            riskConfirm: 'Activa l’accés complet',
        },
        agent: {
            claude: 'Claude',
            codex: 'Codex',
    ...
    mcpApps: {
        loading: 'S’està carregant l’App…',
        offline: 'Aquesta App no està disponible mentre la sessió està fora de línia.',
        retry: 'Torna-ho a provar',
        unsupported: 'Aquest dispositiu no admet MCP Apps.',
        unavailable: 'Aquesta App no està disponible.',
        openLinkTitle: 'Vols obrir l’enllaç extern?',
        openLinkMessage: 'Aquesta App vol obrir un enllaç fora de Paws.',
        openLinkConfirm: 'Obre l’enllaç',
        openLinkCancel: 'Cancel·la',
    },
    ...
    settingsVoice: {
        ...
        developerFooter: 'Diagnòstic i controls locals exclusius per a desenvolupadors per al desplegament actual de Voice. La barrera de pagament de Voice passa pel servidor de Paws, tret que estiguin activades tant la connexió directa com un agent d’ElevenLabs personalitzat.',
        experimentOverrideTitle: 'Substitució de l’experiment de Voice',
        experimentOverrideSubtitle: 'Substitueix l’experiment de venda addicional de Voice en aquest dispositiu',
        experimentOverrideDescription: 'Tria una substitució local per a l’experiment de venda addicional de Voice.',
        experimentOverrideNone: 'Sense substitució',
        experimentOverrideControl: 'Grup de control',
        experimentOverrideSoftPaywall: 'Mur de pagament flexible',
        experimentOverrideOnboardingUpsell: 'Incorporació + venda addicional',
        experimentStatusTitle: 'Estat de l’experiment de Voice',
        experimentStatusVariant: ({ value }: { value: string }) => `venda addicional de Voice: ${value}`,
        experimentStatusSource: ({ value }: { value: string }) => `origen: ${value}`,
        experimentStatusGate: ({ value }: { value: string }) => `barrera: ${value}`,
        experimentStatusExperiments: ({ enabled }: { enabled: boolean }) => `configuració d’experiments: ${enabled ? 'activada' : 'desactivada'}`,
        experimentStatusDirectByoAgent: 'connexió directa amb agent propi',
        experimentStatusServerGate: 'barrera del servidor de Paws',
        experimentSourceOverride: 'substitució',
        experimentSourcePosthog: 'PostHog',
        experimentSourceDefault: 'valor predeterminat',
        resetCountersTitle: 'Restableix els comptadors de Voice',
        resetCountersMessage: 'Voleu esborrar els comptadors locals de Voice utilitzats per a la incorporació i el comportament del mur de pagament flexible en aquest dispositiu?',
        resetCountersConfirm: 'Restableix',
        counterSoftPaywallShown: ({ count }: { count: number }) => `vegades que s’ha mostrat el mur de pagament flexible: ${count}`,
        counterOnboardingPromptLoads: ({ count }: { count: number }) => `càrregues del missatge d’incorporació: ${count}`,
        counterVoiceMessages: ({ count }: { count: number }) => `missatges de Voice: ${count}`,
    },
    ...
    updateBanner: {
        otaOnlyTitle: "No hi ha actualitzacions OTA",
        otaOnlyMessage: "No hi ha actualitzacions OTA per a aquesta instal·lació. Aquest paquet no admet la comprovació de versions natives.",
        tapToDownloadGitHub: "Descarrega des de GitHub Release",
        nativeUpdateMessage: "Hi ha un paquet nou de l’aplicació. Algunes actualitzacions requereixen el paquet nou. Descarrega’l des de GitHub Release i instal·la’l sobre l’aplicació existent, sense desinstal·lar-la.",
        downloadApp: "Descarrega APK",
        nativeCheckFailedTitle: "No s’ha pogut comprovar l’aplicació",
        nativeCheckFailedMessage: "No s’ha pogut confirmar si hi ha una nova aplicació. Torna-ho a provar més tard.",
        ...
        forceCheckSubtitle: "Comprova ara les actualitzacions de l’aplicació i OTA",
    ...
    markdown: {
        ...
        mermaidOpenFullscreen: 'Obre el diagrama a pantalla completa',
        mermaidCloseFullscreen: 'Tanca el diagrama a pantalla completa',
    },
    ...
    imageUpload: {
        ...
        chooseSourcePdf: 'Document PDF',
        documentPdf: 'Document PDF',
        documentOpen: ({ name }: { name: string }) => `Obre «${name}»`,
        documentOpenFailed: "No s'ha pogut obrir aquest PDF.",
        mediaVideo: 'Vídeo',
        mediaAudio: 'Àudio',
        mediaPlay: ({ name }: { name: string }) => `Reprodueix «${name}»`,
        mediaCollapse: ({ name }: { name: string }) => `Replega el reproductor de «${name}»`,
        mediaFullscreen: ({ name }: { name: string }) => `Reprodueix «${name}» a pantalla completa`,
        mediaLoadFailed: "No s'ha pogut carregar el fitxer multimèdia.",
    ...
    imageViewer: {
        ...
        stopMotionPhoto: 'Atura la foto en moviment',
        ...
        motionPhotoLoadFailedMessage: "No s'ha pogut reproduir aquesta foto en moviment.",
    },
    ...
    agentSpace: {
        ...
        companion: {
            panelTitle: 'Acompanyament diari',
            panelSubtitle: 'Petits passos per mantenir un ritme més estable',
            tipBedtimeEyebrow: 'Abans d’anar a dormir · 23:30',
            tipBedtimeTitle: 'Ves al llit abans de les 23:30',
            tipBedtimeBody: 'Deixa de consumir contingut estimulant 30 minuts abans per donar al cervell un senyal clar que és hora de dormir.',
            tipMorningLightEyebrow: 'Al matí · 10 min',
            tipMorningLightTitle: '10 minuts de llum natural',
            tipMorningLightBody: 'Pren llum del matí poc després de despertar-te per ajudar a estabilitzar el ritme circadiari.',
            tipSleepWindowEyebrow: 'Aquesta nit · 7 hores',
            tipSleepWindowTitle: 'Reserva 7 hores per dormir',
            tipSleepWindowBody: 'Protegeix primer la franja de son; no intentis canviar tots els hàbits alhora.',
            actionSleepTitle: 'Registra el son d’ahir a la nit',
            actionSleepPrompt: 'Ajuda’m a registrar el son d’ahir a la nit. Afegiré una captura de pantalla o les hores exactes.',
            actionExerciseTitle: 'Registra una sessió d’exercici',
            actionExercisePrompt: 'Ajuda’m a registrar una sessió d’exercici d’avui. Afegiré el tipus d’exercici, la durada o una captura de pantalla.',
            actionDietTitle: 'Registra l’alimentació d’avui',
            actionDietPrompt: 'Ajuda’m a registrar l’alimentació d’avui. Afegiré els aliments, les racions o fotos.',
            actionWeeklyTitle: 'Resumeix les dades de salut d’aquesta setmana',
            actionWeeklyPrompt: 'A partir dels registres d’aquesta setmana, resumeix el son, l’exercici i l’alimentació, i suggereix en què m’hauria de centrar després.',
            paginationPrevious: 'Consell anterior',
            paginationNext: 'Consell següent',
            actionAccessibility: ({ title }: { title: string }) => `Fes servir l’acció ràpida: ${title}`,
            paginationAccessibility: ({ current, total }: { current: number; total: number }) => `Consell ${current} de ${total}`,
        },
    },
    agents: {
        ...
        folderCustomPath: 'S’està utilitzant la carpeta personalitzada de dalt',
        folderRecent: 'Carpetes recents',
        folderNoRecent: 'Encara no hi ha carpetes recents',
        presets: 'Predefinits',
        ...
        customImageStyleNoReadableImages: "No hi ha imatges de referència llegibles. Tria imatges JPG/PNG nítides i torna-ho a provar.",
        ...
        imageStyleSceneDistillationZineHint: 'Destil·la una foto en una il·lustració Zine original sobre paper, vertical 3:5 o horitzontal 5:3; conserva els ancoratges semàntics i emocionals, sense píxels de la foto original.\nVores esquinçades expressives, tipografia d’autor i un accent viu intencional; l’activador exacte 单色块模式 crea un únic camp continu de color saturat.',
        imageStyleGradeImages: 'Etalonatge fotogràfic determinista',
        imageStyleGradeImagesHint: 'Corregeix, iguala o aplica color creatiu a fotos JPEG/PNG sense edició generativa.\nConserva estrictament geometria, identitat, cares, objectes, text i textura; admet intensitat conservadora a transformadora, receptes reproduïbles, previsualitzacions i controls de qualitat.',
        imageStyleScenesGatheredZine: 'Zine d’escenes recollides',
        imageStyleScenesGatheredZineHint: 'Ancora una fotografia fidel en un pòster tàctil 3:5 espaiós i amplia formes de la font en un camp il·lustrat molt simplificat.\nUn color viu estructural, unió de paper estripat amb fibres visibles, espai negatiu actiu i microtext discret.',
        imageStyleScenesGatheredZineSea: 'Zine d’escenes recollides · Mar',
        imageStyleScenesGatheredZineSeaHint: 'Referència costanera del mateix compilador: fotografia fidel de mar/barana, paper ampli i tranquil, vores esquinçades fibroses, marques impreses comprimides, un color estructural i microtext discret.',
