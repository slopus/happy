...
/**
 * Russian translations for the Paws app
...
export const ru: TranslationStructure = {
    ...
    deviceEnvironmentDashboard: {
        ...
        scan: 'Проверить снова',
        scanning: 'Проверка…',
        fleet: ({ total, online }: { total: number; online: number }) => `Всего устройств: ${total} · в сети: ${online}`,
        lastChecked: ({ time }: { time: string }) => `Проверено: ${time}`,
        notChecked: 'Ещё не проверено',
        offline: 'Не в сети · предыдущий результат',
        online: 'В сети',
        latest: 'Последняя версия',
        available: 'Доступно обновление',
        update: 'Обновить',
        updateType: ({ count }: { count: number }) => `Обновить инструмент · ${count}`,
        updateAll: ({ count }: { count: number }) => `Обновить доступное · ${count}`,
        install: 'Установить',
        login: 'Войти',
        handle: 'Исправить',
        retry: 'Повторить',
        queued: 'В очереди',
        preparing: 'Проверка перед обновлением',
        updating: 'Обновление и проверка',
        succeeded: 'Обновлено · проверено',
        failed: 'Ошибка обновления',
        uncertain: 'Результат не подтверждён',
        unresolvedHint: 'Удалённое обновление может всё ещё выполняться. Обновления на этом устройстве приостановлены; проверка версий не подтверждает завершение процесса. Перед продолжением убедитесь на самом устройстве, что обновление завершено.',
        changed: 'Среда изменилась · проверьте ещё раз',
        pendingLogin: 'Установлено · требуется вход',
        pendingPair: 'Требуется проверить совместимость приложения и CLI',
        usable: 'Установлено',
        sourceUnknown: 'Последняя версия недоступна',
        ahead: 'Новее версии из этого источника',
        certificateOptional: 'Временные туннели доступны · вход в аккаунт необязателен',
        updatesSummary: ({ count, machines }: { count: number; machines: number }) => `Доступно обновлений: ${count} · устройств: ${machines}`,
        attentionSummary: ({ count }: { count: number }) => `Требуют отдельной настройки: ${count}`,
        allCurrent: 'Для устройств в сети нет доступных обновлений',
        batchProgress: ({ done, total }: { done: number; total: number }) => `Обновление элементов: ${total} · завершено: ${done}`,
        batchResult: ({ succeeded, failed, uncertain, skipped }: { succeeded: number; failed: number; uncertain: number; skipped: number }) => `Завершено: ${succeeded} · ошибок: ${failed} · не подтверждено: ${uncertain} · пропущено: ${skipped}`,
        background: 'Можно закрыть эту панель и проверить ход выполнения позже.',
        verifyHint: 'Обновления проверяются автоматически; устройства не в сети пропускаются.',
        horizontalHint: 'Проведите по горизонтали для сравнения устройств',
        target: ({ version }: { version: string }) => `Доступна версия ${version}`,
        actionConfirmTitle: 'Продолжить на этом устройстве?',
        actionConfirm: ({ machine, component, action, version }: { machine: string; component: string; action: string; version: string }) => `${machine}: ${action} ${component} (${version}). Для входа и настройки на этом устройстве может открыться окно.`,
        manualHint: 'Выполните команды ниже на целевом устройстве, затем проверьте снова.',
        selectedAction: ({ machine, component, action }: { machine: string; component: string; action: string }) => `${machine} · ${component} · ${action}`,
        localSource: 'Целевая версия определяется источником установки на этом устройстве.',
    },
    deviceEnvironment: {
        ...
        egoAppMissing: "Приложение Ego Lite не найдено. Установите его и завершите первоначальную настройку на этой машине.",
        egoUnsupported: "Проверка приложения Ego Lite требует macOS; на этой платформе показаны только версии CLI.",
        pawsUnsupported: "Выравнивание Paws CLI требует Mac с Apple Silicon и подтверждённой глобальной установки npm.",
        componentUnsupported: "Проверка этого компонента не поддерживается на этой платформе.",
        cloudflaredFormulaUnavailable: "Формула cloudflared недоступна. Проверьте Homebrew на этой машине.",
        cloudflaredHomebrewMissing: "Homebrew не найден; проверить последнюю версию cloudflared невозможно.",
        componentVersionSourceMismatch: "Источники версий не совпадают. Проверьте установку этого компонента и повторите сканирование.",
        title: "Среда устройства",
        subtitle: "Проверяйте инструменты разработки и выравнивайте их версии на ваших машинах",
        fleetReady: ({ ready, total }: { ready: number; total: number }) => `${ready}/${total} машин с целевой версией`,
        scanAll: "Сканировать все машины",
        scanning: "Сканирование машин…",
        previewAlignment: "Предпросмотр выравнивания",
        previewing: "Подготовка предпросмотра…",
        confirmTitle: "Выровнять GitHub CLI?",
        confirmMessage: ({ actions }: { actions: string }) => `Проверьте точное действие для каждой машины:\n\n${actions}\n\nБудут выполнены только перечисленные установки и обновления. На каждой машине необходимо войти в учётную запись отдельно.`,
        confirmAction: "Применить выравнивание",
        applying: "Выравнивание версий…",
        completed: "Завершено",
        githubCli: "GitHub CLI",
        daemonOnline: "Daemon подключён",
        daemonOffline: "Daemon отключён — пропущено",
        offlineRecovery: "Восстановите подключение этой машины или проверьте её через SSH. Затем повторите сканирование.",
        versionInstalled: ({ version }: { version: string }) => `Установлена: ${version}`,
        versionTarget: ({ version }: { version: string }) => `Целевая: ${version}`,
        authReady: "Вход в GitHub выполнен",
        authMissing: "Требуется вход в GitHub",
        authUnknown: "Состояние входа в GitHub неизвестно",
        actionNone: "Без изменения версии",
        actionInstall: ({ version }: { version: string }) => `Установить gh ${version}`,
        actionUpgrade: ({ from, version }: { from: string; version: string }) => `Обновить gh ${from} → ${version}`,
        actionManualRepair: "Требуется ручное исправление; автоматических изменений не будет",
        repairWithSsh: "Подключитесь к этой машине через SSH для завершения настройки или исправления.",
        repairLocally: "Откройте терминал на этой машине для завершения настройки или исправления.",
        scanAgain: "Повторите сканирование, чтобы обновить состояние машины",
        stateUnknown: "Состояние неизвестно; повторите сканирование",
        timeoutRecovery: "Дождитесь завершения Homebrew или текущей операции, затем выполните сканирование перед повторной попыткой.",
        partialFailure: "Некоторым машинам всё ещё требуется внимание. Проверьте результаты ниже.",
        versionSourceMismatch: "Источники версий не совпадают. Обновите сведения Homebrew на затронутых машинах и повторите сканирование.",
        homebrewMissing: "Homebrew не установлен. Установите его на этой машине перед выравниванием.",
        unsupportedMachine: "Автоматическое выравнивание требует Mac с Apple Silicon и установленным Homebrew.",
        operationInProgress: "На этой машине выполняется другая операция. Подождите и повторите сканирование.",
        planExpired: "Предпросмотр устарел или состояние машины изменилось. Повторите сканирование и предпросмотр.",
        formulaUnavailable: "Формула gh недоступна. Проверьте Homebrew на этой машине.",
        versionAhead: "Установленная версия новее целевой. Проверьте её вручную; понижение версии не выполняется.",
        alignmentFailed: "Выравнивание не завершено. Проверьте эту машину и повторите сканирование.",
        notInstalled: "Не установлено",
        previewRequired: "Откройте предпросмотр, чтобы увидеть точное действие",
        emptyFleet: "Машины ещё не зарегистрированы. Подключите машину, чтобы начать.",
        authenticationNote: "Версия и вход в GitHub проверяются отдельно. Выполните вход на каждой машине с помощью gh auth login.",
        developmentEnvironmentHealth: 'Здоровье среды разработки', componentReadyCount: ({ ready, total }: { ready: number; total: number }) => `${ready}/${total} проверок готовы`, componentReady: 'Готово', componentWarning: 'Требуется внимание', componentUnknown: 'Неизвестно', pawsCli: 'Paws CLI', egoBrowser: 'Ego Lite и CLI', cloudflareWrangler: 'Cloudflare Wrangler', cloudflared: 'cloudflared', versionLatest: ({ version }: { version: string }) => `Последняя: ${version}`, pawsOwnershipVerified: 'глобальный пакет npm подтверждён', pawsOwnershipUnverified: 'Paws CLI не принадлежит этой глобальной установке npm', egoAppVersion: ({ version }: { version: string }) => `Ego Lite: ${version}`, egoCliVersion: ({ version }: { version: string }) => `Ego CLI: ${version}`, egoChromiumVersion: ({ version }: { version: string }) => `Chromium: ${version}`, egoNodeVersion: ({ version }: { version: string }) => `Node: ${version}`, egoPaired: 'версии Ego Lite и CLI совпадают', egoMismatch: 'версии Ego Lite и CLI не совпадают', cloudflareAuthReady: 'Cloudflare аутентифицирован', cloudflareAuthMissing: 'Требуется вход в Cloudflare', cloudflareAuthUnknown: 'Аутентификация Cloudflare неизвестна', cloudflareAccounts: ({ accounts }: { accounts: string }) => `Учётные записи: ${accounts}`, tunnelCertificatePresent: 'Сертификат входа в туннель присутствует', tunnelCertificateMissing: 'Сертификат входа в туннель отсутствует', actionPawsInstall: ({ version }: { version: string }) => `Установить Paws CLI ${version}`, actionPawsUpgrade: ({ from, version }: { from: string; version: string }) => `Обновить Paws CLI ${from} → ${version}`, confirmPawsTitle: 'Выровнять Paws CLI?', selectAlignmentTool: ({ component }: { component: string }) => `Выбрать выравнивание ${component}`, previewComponentAlignment: ({ component }: { component: string }) => `Предпросмотр выравнивания ${component}`, confirmComponentAction: ({ component }: { component: string }) => `Применить выравнивание ${component}`,
        confirmPawsMessage: ({ actions }: { actions: string }) => `Проверьте действие Paws CLI для каждой машины:\n\n${actions}\n\nБудут выполнены только перечисленные глобальные установки и обновления npm. Требуется подтверждённая принадлежность npm.`, pawsTimeoutRecovery: 'Дождитесь завершения npm или текущей операции, затем выполните повторное сканирование.', componentTimeoutRecovery: 'Дождитесь завершения текущей операции, затем выполните повторное сканирование.', pawsOwnershipMismatch: 'Не удалось подтвердить принадлежность Paws CLI. Проверьте глобальный префикс npm перед выравниванием.', inspectOnly: 'Только проверка',
    },
    relationshipAdvisor: {
        ...
        emptyResponse: 'Модель не вернула текст. Повторите попытку.',
        stopAccessibility: 'Остановить генерацию',
        installSubtitle: 'Установите плагин, чтобы пользоваться советником по отношениям',
    },
    relationshipAdvisorPlugin: {
        marketTitle: 'Плагины',
        marketSubtitle: 'Находите и включайте плагины, входящие в эту версию Paws',
        searchPlaceholder: 'Поиск плагинов',
        installedSection: 'Установленные',
        featuredSection: 'Рекомендуемые',
        emptySearch: 'Плагины не найдены',
        openPlugin: 'Открыть плагин',
        configure: 'Настроить',
        configuration: 'Настройка плагина',
        permissions: 'Разрешения',
        permissionGrantNotice: 'Установка или обновление предоставляет эти разрешения для этой учётной записи. Удаление плагина отзывает их.',
        builtInCode: 'Встроенный доверенный код',
        builtInCodeNotice: 'Код плагина поставляется вместе с этой версией Paws. Установка только включает его для вашей учётной записи.',
        permissionAiProviderInvoke: 'Использование поставщика ИИ',
        permissionAiProviderInvokeDescription: 'Отправка запросов из диалогов этого плагина настроенному поставщику ИИ.',
        permissionSecretsUse: 'Использование зашифрованных секретов',
        permissionSecretsUseDescription: 'Чтение зашифрованной конфигурации этого плагина только во время его работы.',
        permissionConversationImagesRead: 'Чтение изображений из диалогов',
        permissionConversationImagesReadDescription: 'Чтение изображений, которые вы выбрали для отправки этому плагину.',
        permissionStorageImagesWrite: 'Запись изображений плагина',
        permissionStorageImagesWriteDescription: 'Загрузка и удаление изображений, созданных или используемых этим плагином.',
        encryptionNotice: 'Настройки, включая ключ API, шифруются и хранятся на сервере Paws. Сохранённый ключ возвращается в это приложение, только если вы явно выбрали его отображение.',
        secretRevealFailed: 'Не удалось отобразить сохранённый ключ. Повторите попытку.',
        apiKey: 'Ключ API',
        apiKeyConfigured: ({ hint }: { hint: string }) => `Настроен · оканчивается на ${hint}`,
        apiKeyPlaceholder: 'Например, sk-...',
        baseUrl: 'Базовый URL',
        baseUrlPlaceholder: 'Например, https://api.deepseek.com',
        model: 'Модель',
        modelPlaceholder: 'Например, deepseek-v4-flash-vision-exp',
        unsavedChangesTitle: 'Несохранённые изменения',
        unsavedChangesSubtitle: 'Текущая конфигурация ещё не сохранена. Сохраните её перед выходом.',
        modelRecommendation: 'В первую очередь выбирайте модели с поддержкой мультимодальности, чтобы понимать текст и изображения.',
        testConnection: 'Проверить подключение',
        testConnectionSubtitle: 'После успешной проверки текущая конфигурация будет сохранена автоматически',
        connectionSuccess: 'Подключение выполнено',
        connectionSuccessSubtitle: 'Текущие настройки позволяют подключиться к провайдеру и модели',
        connectionAuthenticationFailed: 'Ошибка аутентификации',
        connectionModelNotFound: 'Модель не найдена',
        connectionRateLimited: 'Достигнут лимит запросов',
        connectionTimedOut: 'Время ожидания подключения истекло',
        connectionUnreachable: 'Не удалось подключиться к провайдеру',
        connectionProviderError: 'Провайдер вернул ошибку',
        status: 'Статус плагина',
        installed: 'Установлен',
        installedSubtitle: 'Готов к использованию в этой учётной записи',
        reviewRequired: 'Требуется повторное подтверждение',
        reviewRequiredSubtitle: 'Версия плагина или разрешения изменились. Проверьте и обновите перед использованием.',
        reviewAndUpdate: 'Проверить и обновить',
        notInstalled: 'Не установлен',
        notInstalledSubtitle: 'Заполните настройки и установите плагин',
        update: 'Обновить настройки',
        install: 'Установить плагин',
        installSubtitle: 'Зашифровать настройки и сохранить их на сервере Paws',
        uninstall: 'Удалить плагин',
        uninstallSubtitle: 'Удалить плагин и сохранённые учётные данные',
    },
    healthPanel: {
        ...
        scoreLabel: ' pts',
        tonightSleep: 'Сегодня',
        deep: 'Глубокий',
        light: 'Лёгкий',
        rem: 'REM',
        bedtime: 'Засыпание',
        wakeTime: 'Пробуждение',
        structureTitle: 'Фазы сна',
        viewBar: 'Столбцы',
        viewDonut: 'Кольцо',
    ...
    generatedImageBatchDownload: {
        ...
        savedBrowser: ({ count }: { count: number }) => `Начато загрузок: ${count} · Разрешите несколько загрузок, если браузер спросит`,
        savedDirectory: ({ count }: { count: number }) => `Изображений сохранено в выбранную папку: ${count}`,
        savedPhotos: ({ count }: { count: number }) => `Изображений сохранено в Фото: ${count}`,
        cancelledBrowser: 'Загрузки отменены · Повторите и разрешите несколько загрузок',
        cancelledDirectory: 'Папка не выбрана · Выберите папку для сохранения изображений',
        cancelledPhotos: 'Для сохранения созданных изображений нужен доступ к Фото',
        cancelledUnsupported: 'Пакетная загрузка недоступна на этой платформе',
        partial: ({ succeeded, failed }: { succeeded: number; failed: number }) => `Сохранено: ${succeeded} · Ошибок: ${failed}`,
        retryFailed: ({ count }: { count: number }) => `Повторить ошибки (${count})`,
    },
    desktopWorkspace: {
        sessions: 'Сессии',
    ...
    keyboardShortcuts: {
        ...
        close: 'Закрыть список сочетаний клавиш',
        open: 'Открыть список сочетаний клавиш',
        common: 'Общие',
        navigation: 'Навигация',
        commandPalette: 'Палитра команд',
        conversation: 'Чат и ввод',
        desktopApp: 'Настольное приложение',
        openCommandPalette: 'Открыть палитру команд',
        openSettings: 'Открыть настройки приложения',
        toggleLeftSidebar: 'Показать или скрыть боковую панель сессий',
        toggleRightPanel: 'Показать или скрыть панель деталей',
        closeOrGoBack: 'Закрыть верхний слой или вернуться назад',
        closeOrGoBackDetail: 'Закрывает модальное окно или оверлей либо выходит из Дзен-режима; если закрывать нечего, возвращается по истории навигации',
        moveSelection: 'Перемещать выделение',
        runSelected: 'Выполнить выбранную команду',
        closeCommandPalette: 'Закрыть палитру команд',
        runNumberedResult: 'Выполнить видимый результат с номером 1–9',
        sendMessage: 'Отправить сообщение',
        insertNewLine: 'Вставить новую строку',
        navigateSuggestions: 'Перемещаться по подсказкам',
        acceptSuggestion: 'Принять подсказку',
        closeSuggestions: 'Закрыть подсказки',
        stopRunningAgent: 'Остановить работающего агента',
        pressTwice: 'Нажмите дважды в течение двух секунд',
    ...
    settings: {
        ...
        about: 'О программе',
        aboutFooter: 'Paws — мобильное приложение для работы с Codex и Claude Code. Использует сквозное шифрование, все данные аккаунта хранятся только на вашем устройстве. Не связано с Anthropic.',
        whatsNew: 'Что нового',
    ...
    generatedImages: {
        ...
        pluginInstalledSubtitle: 'Галерея созданных изображений доступна в этой учётной записи.',
        pluginNotInstalledSubtitle: 'Установите плагин, чтобы просматривать созданные изображения.',
        pluginInstallSubtitle: 'Включить галерею созданных изображений для этой учётной записи.',
        pluginUninstallSubtitle: 'Отключить доступ к галерее, не удаляя созданные изображения.',
        pluginPrivacyNotice: 'Ключ API не требуется. На сервере Paws хранится только зашифрованная отметка об установке.',
    ...
    server: {
        ...
        notValidHappyServer: 'Это не валидный сервер Paws',
    ...
    sessionInfo: {
        ...
        sessionRowRemoteLocation: ({ name }: { name: string }) => `Удалённо · ${name}`,
        sessionRowUnknownLocation: 'Расположение неизвестно/локально',
        sessionRowUnknownAgent: 'Неизвестный агент',
        sessionRowProjectPath: 'Проект/путь',
        sessionRowMachineAgent: 'Устройство/агент',
        sessionRowRelativeTime: 'Относительное время',
        sessionRowRunningStatus: 'Статус выполнения',
        sessionRowMoreActions: 'Другие действия с сессией',
        ...
        happyHome: 'Домашний каталог Paws',
    ...
    sessionShare: {
        ...
        confirmTitle: 'Опубликовать эту сессию?',
        confirmMessage: 'Будет создан публичный снимок текущего диалога со всеми вложениями. Любой, у кого есть ссылка, сможет его просмотреть.',
        confirmAction: 'Создать публичную ссылку',
        preparing: 'Подготовка диалога…',
        uploading: ({ completed, total }: { completed: number; total: number }) => `Загрузка вложений (${completed}/${total})…`,
        publishing: 'Публикация снимка…',
        copyLink: 'Копировать ссылку',
        openSharedPage: 'Открыть общедоступную страницу',
        updateSnapshot: 'Обновить снимок',
        revokeSharing: 'Отозвать доступ',
        revokeTitle: 'Отозвать публичную ссылку?',
        revokeMessage: 'Общедоступная страница и все её вложения сразу перестанут работать.',
        revokeAction: 'Отозвать ссылку',
        shared: 'Сессия опубликована',
        linkCopied: 'Ссылка скопирована',
        sharedOn: ({ date }: { date: string }) => `Опубликовано ${date}`,
        sharedViaPaws: 'Опубликовано через Paws',
        thinking: 'Размышляет',
        toolRunning: 'Выполняется',
        toolCompleted: 'Выполнено',
        toolFailed: 'Ошибка',
        downloadAttachment: 'Скачать вложение',
        attachmentUnavailable: 'Вложение недоступно',
        notFoundTitle: 'Эта опубликованная сессия недоступна',
        notFoundMessage: 'Ссылка может быть недействительна или владелец мог её отозвать.',
        authenticationUnavailable: 'Аутентификация недоступна',
        ...
        appearance: 'Оформление публикации',
        cover: 'Обложка',
        coverPreview: 'Предпросмотр обложки',
        noCover: 'Без обложки',
        randomCover: 'Случайное изображение',
        ...
        coverAttribution: ({ photographer }: { photographer: string }) => `Фото: ${photographer} · Pexels`,
    ...
    components: {
        emptyMainScreen: {
            ...
            installCli: 'Установите Paws CLI',
            runIt: 'Запустите его',
        ...
        messageComposer: {
            ...
            workingDirectoryBrowse: 'Обзор папок',
            workingDirectoryRecent: 'Недавние каталоги',
            workingDirectoryNoRecent: 'На этом компьютере нет других недавних каталогов',
            workingDirectorySelectCurrent: ({ name }: { name: string }) => `Использовать ${name}`,
            workingDirectoryError: ({ reason }: { reason: string }) => `Этот каталог нельзя использовать. Проверьте путь и доступ, затем повторите попытку. ${reason}`,
            workingDirectoryEmpty: 'Сначала введите или выберите каталог.',
            workingDirectoryOffline: 'Компьютер Agent не в сети. Подключите его перед сменой каталога.',
    ...
    agentInput: {
        ...
        taskPermission: {
            ...
            confirmShort: 'Спросить',
            confirmDescription: 'Для действий, требующих дополнительных разрешений, используется подтверждение агента. Ограничения устройства и внешней песочницы продолжают действовать.',
            fullAccess: 'Полный доступ',
            fullAccessShort: 'Полный',
            fullAccessDescription: 'Обходит подтверждения агента там, где это поддерживается. Ограничения устройства и внешней песочницы продолжают действовать.',
            changesNextMessages: 'Применяется к следующему и последующим сообщениям.',
            unsupported: 'Этот агент не предоставляет два надёжных уровня разрешений.',
            unavailable: 'Недоступно',
            riskTitle: 'Включить полный доступ?',
            riskMessage: 'Полный доступ обходит подтверждения агента там, где это поддерживается, в том числе для потенциально опасных действий. Он не отменяет ограничения устройства или внешней песочницы. Продолжайте, только если доверяете этой задаче.',
            riskConfirm: 'Включить полный доступ',
        },
        agent: {
            claude: 'Claude',
            codex: 'Codex',
    ...
    settingsVoice: {
        ...
        byoDescription: 'Используйте собственного агента ElevenLabs вместо стандартного Paws. Подписка не требуется — подключайтесь напрямую через свой аккаунт ElevenLabs. Ваш агент должен определить два клиентских инструмента: messageClaudeCode (отправляет текст агенту кодирования) и processPermissionRequest (разрешает или запрещает использование инструментов). Контекст сессии передаётся через динамическую переменную {{initialConversationContext}}.',
        customAgentId: 'ElevenLabs Agent ID',
        customAgentIdNotSet: 'Не настроено',
        customAgentIdDescription: 'Введите ваш ElevenLabs Agent ID. Оставьте пустым, чтобы использовать стандартный Paws.',
        customAgentIdPlaceholder: 'e.g. abc123def456',
        bypassToken: 'Прямое подключение',
        bypassTokenSubtitle: 'Пропустить сервер Paws, подключиться напрямую к ElevenLabs',
        ...
        developerFooter: 'Диагностика и локальные элементы управления для разработчиков, относящиеся к текущему внедрению Voice. Платный доступ к Voice предоставляется через сервер Paws, если одновременно не включены прямое подключение и пользовательский агент ElevenLabs.',
        experimentOverrideTitle: 'Переопределение эксперимента Voice',
        experimentOverrideSubtitle: 'Переопределить эксперимент по продвижению Voice на этом устройстве',
        experimentOverrideDescription: 'Выберите локальное переопределение эксперимента по продвижению Voice.',
        experimentOverrideNone: 'Без переопределения',
        experimentOverrideControl: 'Контрольная группа',
        experimentOverrideSoftPaywall: 'Мягкий пейвол',
        experimentOverrideOnboardingUpsell: 'Онбординг + продвижение',
        experimentStatusTitle: 'Состояние эксперимента Voice',
        experimentStatusVariant: ({ value }: { value: string }) => `продвижение Voice: ${value}`,
        experimentStatusSource: ({ value }: { value: string }) => `источник: ${value}`,
        experimentStatusGate: ({ value }: { value: string }) => `схема доступа: ${value}`,
        experimentStatusExperiments: ({ enabled }: { enabled: boolean }) => `настройка экспериментов: ${enabled ? 'включена' : 'выключена'}`,
        experimentStatusDirectByoAgent: 'прямое подключение со своим агентом',
        experimentStatusServerGate: 'доступ через сервер Paws',
        experimentSourceOverride: 'переопределение',
        experimentSourcePosthog: 'PostHog',
        experimentSourceDefault: 'по умолчанию',
        resetCountersTitle: 'Сбросить счётчики Voice',
        resetCountersMessage: 'Очистить локальные счётчики Voice, используемые для онбординга и логики мягкого пейвола на этом устройстве?',
        resetCountersConfirm: 'Сбросить',
        counterSoftPaywallShown: ({ count }: { count: number }) => `показов мягкого пейвола: ${count}`,
        counterOnboardingPromptLoads: ({ count }: { count: number }) => `загрузок онбординг-промпта: ${count}`,
        counterVoiceMessages: ({ count }: { count: number }) => `сообщений Voice: ${count}`,
    },
    ...
    updateBanner: {
        otaOnlyTitle: "Нет обновлений OTA",
        otaOnlyMessage: "Для этой установки нет обновлений OTA. Этот пакет не поддерживает проверку нативной версии приложения.",
        tapToDownloadGitHub: "Скачать из GitHub Release",
        nativeUpdateMessage: "Доступен новый пакет приложения. Для некоторых обновлений требуется новый пакет. Скачайте его из GitHub Release и установите поверх текущего приложения, не удаляя его.",
        downloadApp: "Скачать APK",
        nativeCheckFailedTitle: "Не удалось проверить обновление приложения",
        nativeCheckFailedMessage: "Не удалось подтвердить наличие новой версии приложения. Повторите попытку позже.",
        ...
        forceCheckSubtitle: "Проверить обновления приложения и OTA",
    ...
    terminal: {
        ...
        terminalRequestDescription: 'Терминал запрашивает подключение к вашему аккаунту Paws. Это позволит терминалу безопасно отправлять и получать сообщения.',
    ...
    machine: {
        ...
        codexUsage: 'Использование Codex',
        codexUsageToday: 'Сегодня',
        codexUsageYesterday: 'Вчера',
        codexUsageInput: 'Ввод',
        codexUsageCached: 'кэшировано',
        codexUsageOutput: 'Вывод',
        codexUsageReasoning: 'рассуждение',
    ...
    imageUpload: {
        ...
        chooseSourcePdf: 'Документ PDF',
        documentPdf: 'Документ PDF',
        documentOpen: ({ name }: { name: string }) => `Открыть «${name}»`,
        documentOpenFailed: 'Не удалось открыть этот PDF-документ.',
        mediaVideo: 'Видео',
        mediaAudio: 'Аудио',
        mediaPlay: ({ name }: { name: string }) => `Воспроизвести «${name}»`,
        mediaCollapse: ({ name }: { name: string }) => `Свернуть проигрыватель «${name}»`,
        mediaFullscreen: ({ name }: { name: string }) => `Воспроизвести «${name}» на весь экран`,
    ...
    agentSpace: {
        ...
        companion: {
            panelTitle: 'Ежедневный помощник',
            panelSubtitle: 'Маленькие шаги к более стабильному ритму',
            tipBedtimeEyebrow: 'Перед сном · 23:30',
            tipBedtimeTitle: 'Ложитесь до 23:30',
            tipBedtimeBody: 'За 30 минут до сна отключите возбуждающий контент, чтобы дать мозгу чёткий сигнал ко сну.',
            tipMorningLightEyebrow: 'Утро · 10 мин',
            tipMorningLightTitle: '10 минут естественного света',
            tipMorningLightBody: 'Постарайтесь побыть на утреннем свету вскоре после пробуждения, чтобы стабилизировать циркадный ритм.',
            tipSleepWindowEyebrow: 'Сегодня ночью · 7 часов',
            tipSleepWindowTitle: 'Выделите 7 часов на сон',
            tipSleepWindowBody: 'Сначала защитите время для сна; не пытайтесь изменить все привычки сразу.',
            actionSleepTitle: 'Записать сон за прошлую ночь',
            actionSleepPrompt: 'Помоги мне записать сон за прошлую ночь. Я добавлю снимок экрана или точное время.',
            actionExerciseTitle: 'Записать тренировку',
            actionExercisePrompt: 'Помоги мне записать сегодняшнюю тренировку. Я добавлю вид упражнения, продолжительность или снимок экрана.',
            actionDietTitle: 'Записать сегодняшнее питание',
            actionDietPrompt: 'Помоги мне записать сегодняшнее питание. Я добавлю продукты, порции или фотографии.',
            actionWeeklyTitle: 'Подвести итоги здоровья за эту неделю',
            actionWeeklyPrompt: 'На основе записей за эту неделю подведи итоги по сну, тренировкам и питанию, а затем посоветуй, на чём сосредоточиться дальше.',
            paginationPrevious: 'Предыдущий совет',
            paginationNext: 'Следующий совет',
            actionAccessibility: ({ title }: { title: string }) => `Быстрое действие: ${title}`,
            paginationAccessibility: ({ current, total }: { current: number; total: number }) => `Совет ${current} из ${total}`,
        },
    },
    agents: {
        ...
        name: 'Имя',
        namePlaceholder: 'Дайте агенту имя',
        machine: 'Машина',
        noMachines: 'Нет доступных машин',
        folder: 'Папка',
        folderPlaceholder: 'Путь к папке, напр. ~',
        folderCustomPath: 'Используется пользовательская папка выше',
        folderRecent: 'Недавние папки',
        folderNoRecent: 'Недавних папок пока нет',
        presets: 'Пресеты',
        addPreset: 'Добавить пресет',
        new: 'Новый',
        deleteConfirm: 'Удалить этого агента?',
        entrySubtitle: 'Управляйте запускаемыми агентами',
        ...
        customImageStyleNoReadableImages: 'Нет пригодных референс-изображений. Выберите чёткие JPG/PNG и повторите.',
        ...
        imageStyleHealingScribbleSketchHint: 'Превратите портрет в сырой, умиротворяющий конструктивный аниме-набросок с плотными поисковыми линиями графита, спокойным упрощённым лицом, редкими бледными цветами и большим пространством тёпло-белой бумаги.\nСохраняет узнаваемость через форму лица, волосы, очки, выражение, позу и один ключевой предмет одежды или реквизит; всегда без текста.',
        ...
        imageStylePhotoIllustrationEditorialEchoHint: 'Превратите одну фотографию в вертикальный или горизонтальный редакционный постер, адаптированный к сцене: точный снимок станет основой, а отдельный акварельно-тушевый отголосок, заголовок, метка этюда и короткая подпись будут созданы специально для сюжета и набраны чётким настоящим текстом.',
        imageStylePhotoIllustrationDiptychLakeside: 'Минималистичный диптих у озера',
        imageStylePhotoIllustrationDiptychLakesideHint: 'Объедините одну фотографию у озера с радикально упрощённым геометрическим изображением той же сцены. Сохраните изгиб дощатой дорожки, ритм причала, положение лодки, линию берега, горизонт и профиль гор с помощью взятых из исходного снимка цветовых полей синего, зелёного, кораллового и тёплого оттенка слоновой кости.',
        ...
        imageStyleSceneDistillationZineHint: 'Превратите фото в оригинальную бумажную Zine-иллюстрацию 3:5 или 5:3, сохранив смысловые и эмоциональные опоры, но не пиксели исходного снимка.\nВыразительные рваные края, авторская типографика и один яркий акцент; точный триггер 单色块模式 включает единое насыщенное цветовое поле.',
        imageStyleGradeImages: 'Детерминированный цветокор фото',
        imageStyleGradeImagesHint: 'Исправляет, сопоставляет или творчески тонирует JPEG/PNG без генеративного редактирования.\nСтрого сохраняет геометрию, личность, лица, объекты, текст и текстуру; поддерживает интенсивность от консервативной до преобразующей, воспроизводимые рецепты, превью и контроль качества.',
        imageStyleScenesGatheredZine: 'Zine собранных сцен',
        imageStyleScenesGatheredZineHint: 'Закрепляет правдивую фотографию в просторном тактильном плакате 3:5 и развивает исходные формы в сильно упрощённое поле иллюстрации.\nОдин структурный яркий цвет, волокнистая граница рваной бумаги, активное пустое пространство и сдержанный микротекст.',
        imageStyleScenesGatheredZineSea: 'Zine собранных сцен · Море',
        imageStyleScenesGatheredZineSeaHint: 'Морской пример того же компилятора: правдивое фото моря/перил, широкое тихое поле бумаги, волокнистые рваные края, сжатые печатные следы, один структурный цвет и сдержанный микротекст.',
