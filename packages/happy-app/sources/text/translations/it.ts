...
export const it: TranslationStructure = {
    ...
    newSession: {
        ...
        useCustomPath: ({ path }: { path: string }) => `Usa ${path}`,
        noResults: 'Nessun risultato',
        searchMachines: 'Cerca macchine...',
        searchOrEnterPath: 'Cerca o inserisci percorso...',
    ...
    session: {
        ...
        forkingSession: 'Biforcazione sessione...',
        resumedFrom: ({ name }: { name: string }) => `Ripresa da ${name}`,
        forkedFrom: ({ name }: { name: string }) => `Biforcata da ${name}`,
        resumedSession: 'Sessione ripresa',
        forkedSession: 'Sessione biforcata',
        archive: 'Archivia',
        archiveConfirmTitle: 'Archivia sessione',
        archiveConfirmMessage: 'Sei sicuro di voler archiviare questa sessione? Potrai trovarla nelle sessioni archiviate in seguito.',
    ...
    agentInput: {
        ...
        filesAttached: ({ count }: { count: number }) => count === 1 ? '1 file allegato' : `${count} file allegati`,
        fileTooLargeTitle: 'File troppo grande',
        fileTooLarge: ({ names, limit }: { names: string; limit: string }) => `I seguenti file superano il limite di ${limit} e non sono stati allegati: ${names}`,
    },
    ...
    message: {
        ...
        compactionMarker: 'Conversazione compattata',
        subtask: 'Sottoattività',
    },

    permissions: {
        sessionStopped: 'Sessione interrotta',
