...
export const zhHans: TranslationStructure = {
    ...
    sessionInfo: {
        ...
        worktree: {
            ...
            createPRConfirm: ({ branch }: { branch: string }) => `为分支 "${branch}" 创建 Pull Request？`,
            createPRSuccess: ({ url }: { url: string }) => `Pull Request 已创建：${url}`,
            createPRFailed: '创建 Pull Request 失败',
            ghNotInstalled: '此机器未安装 GitHub CLI (gh)',
            mergeBranch: '合并分支',
            mergeBranchSubtitle: '将 worktree 分支合并回基础分支',
            mergeConfirm: ({ branch }: { branch: string }) => `将 "${branch}" 合并到基础分支？`,
            mergeSuccess: '分支合并成功',
            mergeFailed: '合并分支失败',
            mergeConflicts: '合并存在冲突，请在机器上手动解决。',
            cleanup: '移除 Worktree',
            cleanupSubtitle: '删除 worktree 目录（分支将保留）',
            cleanupConfirm: '确定移除 worktree？分支将保留以确保安全。',
