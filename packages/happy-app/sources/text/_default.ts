...
export const en = {
    codexAccounts: {
        ...
        refreshQuota: 'Refresh quota', refreshingQuota: 'Refreshing…', refreshQuotaConfirmTitle: 'Refresh Codex quota?',
        refreshQuotaConfirm: 'This starts one short hidden Codex probe on the bound device. It uses a small amount of the selected account’s quota and does not create a Paws chat session.',
        refreshQuotaUnavailable: 'Bind this account to an online device to refresh its quota.',
        passiveHint: 'Quota is the last observation, not a live balance. Refresh runs one short hidden Codex probe.',
    ...
    deviceEnvironmentDashboard: {
        ...
        waitingForConnection: 'Waiting for connection to inspect automatically',
        incompleteSummary: ({ count }: { count: number }) => `${count} items are unverified. Check again.`,
        noOnline: 'No devices are online',
        confirmStopped: 'I verified it has stopped',
        confirmStoppedMessage: 'Only continue after checking on the device that the previous update process has ended. This does not stop a remote process and will only recheck the device; it will not retry the update.',
        tools: 'Development tools',
        scan: 'Check again',
        scanning: 'Checking…',
        fleet: ({ total, online }: { total: number; online: number }) => `${total} devices · ${online} online`,
        lastChecked: ({ time }: { time: string }) => `Checked ${time}`,
        notChecked: 'Not checked yet',
        offline: 'Offline · previous result',
        online: 'Online',
        latest: 'Up to date',
        available: 'Update available',
        update: 'Update',
        updateType: ({ count }: { count: number }) => `Update tool · ${count}`,
        updateAll: ({ count }: { count: number }) => `Update available · ${count}`,
        install: 'Install',
        login: 'Sign in',
        handle: 'Resolve',
        retry: 'Retry',
        queued: 'Queued',
        preparing: 'Checking update',
        updating: 'Updating and verifying',
        succeeded: 'Updated · verified',
        failed: 'Update failed',
        uncertain: 'Result unconfirmed',
        unresolvedHint: 'A remote update may still be running. Updates on this device are paused; checking versions cannot confirm that it has stopped. Verify completion on the device before continuing.',
        changed: 'Environment changed · review again',
        pendingLogin: 'Installed · sign-in required',
        pendingPair: 'App / CLI pairing needs attention',
        usable: 'Installed',
        sourceUnknown: 'Latest version unavailable',
        ahead: 'Newer than this source',
        certificateOptional: 'Quick tunnels available · account login optional',
        updatesSummary: ({ count, machines }: { count: number; machines: number }) => `${count} updates across ${machines} devices`,
        attentionSummary: ({ count }: { count: number }) => `${count} items need separate setup`,
        allCurrent: 'No available updates on online devices',
        batchProgress: ({ done, total }: { done: number; total: number }) => `Updating ${total} items · ${done} finished`,
        batchResult: ({ succeeded, failed, uncertain, skipped }: { succeeded: number; failed: number; uncertain: number; skipped: number }) => `Completed ${succeeded} · failed ${failed} · unconfirmed ${uncertain} · skipped ${skipped}`,
        background: 'You can close this panel and check progress later.',
        verifyHint: 'Updates are verified automatically; offline devices are skipped.',
        horizontalHint: 'Swipe horizontally to compare devices',
        target: ({ version }: { version: string }) => `Available ${version}`,
        actionConfirmTitle: 'Continue on this device?',
        actionConfirm: ({ machine, component, action, version }: { machine: string; component: string; action: string; version: string }) => `${machine}: ${action} ${component} (${version}). Sign-in and setup may open a window on that device.`,
        manualHint: 'Complete the commands on this device, then check again.',
        selectedAction: ({ machine, component, action }: { machine: string; component: string; action: string }) => `${machine} · ${component} · ${action}`,
        localSource: 'Target comes from this device’s installation source.',
    },
    deviceEnvironment: {
        egoPathReady: "Ego CLI is available on PATH",
        egoPathMissing: "Add ~/.local/bin to PATH to run ego-browser by name.",
        egoCliMissing: "Ego CLI is missing. Complete Ego Lite onboarding on this machine.",
        egoAppMissing: "Ego Lite app is missing. Install it and complete onboarding on this machine.",
        egoUnsupported: "Ego Lite app checks require macOS; only CLI versions are shown on this platform.",
        pawsUnsupported: "Paws CLI alignment requires an Apple Silicon Mac and a verified npm global installation.",
        componentUnsupported: "This component check is not supported on this platform.",
        cloudflaredFormulaUnavailable: "The cloudflared formula is unavailable. Check Homebrew on this machine.",
        cloudflaredHomebrewMissing: "Homebrew is missing; the latest cloudflared version cannot be checked.",
        componentVersionSourceMismatch: "Version sources disagree. Check this component’s installation and scan again.",
        title: 'Device Environment',
        subtitle: 'Inspect and align development tools across your machines',
        fleetReady: ({ ready, total }: { ready: number; total: number }) => `${ready}/${total} machines at target version`,
        scanAll: 'Scan all machines',
        scanning: 'Scanning machines…',
        previewAlignment: 'Preview alignment',
        previewing: 'Preparing preview…',
        confirmTitle: 'Align GitHub CLI?',
        confirmMessage: ({ actions }: { actions: string }) => `Review the exact action for each machine:\n\n${actions}\n\nOnly the listed installs and upgrades will run. Authentication requires sign-in on each machine.`,
        confirmPawsMessage: ({ actions }: { actions: string }) => `Review the exact Paws CLI action for each machine:\n\n${actions}\n\nOnly the listed global npm installs and upgrades will run. Alignment requires verified npm ownership on each machine.`,
        confirmAction: 'Apply alignment',
        applying: 'Applying alignment…',
        completed: 'Completed',
        ...
        authMissing: 'GitHub sign-in required',
        authUnknown: 'GitHub authentication unknown',
        actionNone: 'No version change',
        actionInstall: ({ version }: { version: string }) => `Install gh ${version}`,
        actionUpgrade: ({ from, version }: { from: string; version: string }) => `Upgrade gh ${from} → ${version}`,
        actionManualRepair: 'Manual repair required — no automatic change',
        repairWithSsh: 'Connect to this machine over SSH to complete setup or repair.',
        repairLocally: 'Open a terminal on this machine to complete setup or repair.',
        scanAgain: 'Scan again to refresh the machine state',
        stateUnknown: 'State unknown; scan again',
        timeoutRecovery: 'Wait for Homebrew or the current operation to finish, then scan again before retrying.',
        pawsTimeoutRecovery: 'Wait for npm or the current operation to finish, then scan again before retrying.',
        componentTimeoutRecovery: 'Wait for the current operation to finish, then scan again before retrying.',
        partialFailure: 'Some machines still need attention. Review their results below.',
        versionSourceMismatch: 'Version sources disagree. Refresh Homebrew on the affected machines, then scan again.',
        homebrewMissing: 'Homebrew is missing. Install it on this machine before alignment.',
        unsupportedMachine: 'Automatic alignment in this PoC requires an Apple Silicon Mac with Homebrew installed.',
        operationInProgress: 'Another operation is running on this machine. Wait, then scan again.',
        planExpired: 'Preview expired or the machine changed. Scan and preview again.',
        formulaUnavailable: 'The gh formula is unavailable. Check Homebrew on this machine.',
        versionAhead: 'The installed version is newer than the target. Review it manually; no downgrade will run.',
        alignmentFailed: 'Alignment did not complete. Review this machine and scan again.',
        notInstalled: 'Not installed',
        previewRequired: 'Preview to see the exact action',
        emptyFleet: 'No machines registered yet. Connect a machine to get started.',
        authenticationNote: 'Version readiness and GitHub authentication are separate. Sign in on each machine with gh auth login.',
        developmentEnvironmentHealth: 'Development environment health',
        componentReadyCount: ({ ready, total }: { ready: number; total: number }) => `${ready}/${total} component checks ready`,
        componentReady: 'Ready',
        componentWarning: 'Needs attention',
        componentUnknown: 'Unknown',
        pawsCli: 'Paws CLI',
        egoBrowser: 'Ego Lite and CLI',
        cloudflareWrangler: 'Cloudflare Wrangler',
        cloudflared: 'cloudflared',
        versionLatest: ({ version }: { version: string }) => `Latest: ${version}`,
        pawsOwnershipVerified: 'npm global package verified',
        pawsOwnershipUnverified: 'Paws CLI is not owned by this npm global installation',
        pawsOwnershipMismatch: 'Paws CLI ownership could not be verified. Check the npm global prefix before aligning.',
        inspectOnly: 'Inspection only',
        egoAppVersion: ({ version }: { version: string }) => `Ego Lite: ${version}`,
        egoCliVersion: ({ version }: { version: string }) => `Ego CLI: ${version}`,
        egoChromiumVersion: ({ version }: { version: string }) => `Chromium: ${version}`,
        egoNodeVersion: ({ version }: { version: string }) => `Node: ${version}`,
        egoPaired: 'Ego Lite and CLI versions match',
        egoMismatch: 'Ego Lite and CLI versions do not match',
        cloudflareAuthReady: 'Cloudflare authenticated',
        cloudflareAuthMissing: 'Cloudflare sign-in required',
        cloudflareAuthUnknown: 'Cloudflare authentication unknown',
        cloudflareAccounts: ({ accounts }: { accounts: string }) => `Accounts: ${accounts}`,
        tunnelCertificatePresent: 'Tunnel login certificate present',
        tunnelCertificateMissing: 'Tunnel login certificate missing',
        actionPawsInstall: ({ version }: { version: string }) => `Install Paws CLI ${version}`,
        actionPawsUpgrade: ({ from, version }: { from: string; version: string }) => `Upgrade Paws CLI ${from} → ${version}`,
        actionEgoUpgrade: ({ from, version }: { from: string; version: string }) => `Update Ego Lite and CLI ${from} → ${version}`,
        actionEgoOnboard: 'Complete Ego CLI onboarding',
        actionWranglerInstall: ({ version }: { version: string }) => `Install Wrangler ${version}`,
        actionWranglerUpgrade: ({ from, version }: { from: string; version: string }) => `Upgrade Wrangler ${from} → ${version}`,
        actionWranglerAuthenticate: 'Sign in to Cloudflare with Wrangler',
        actionCloudflaredInstall: ({ version }: { version: string }) => `Install cloudflared ${version}`,
        actionCloudflaredUpgrade: ({ from, version }: { from: string; version: string }) => `Upgrade cloudflared ${from} → ${version}`,
        actionCloudflaredAuthenticate: 'Open Cloudflare tunnel login',
        confirmPawsTitle: 'Align Paws CLI?',
        confirmToolTitle: ({ component }: { component: string }) => `Align ${component}?`,
        confirmToolMessage: ({ actions }: { actions: string }) => `Review the exact action for each machine:\n\n${actions}\n\nInstalls and upgrades only use the verified package owner. Sign-in and onboarding open the official interactive flow on that machine. Credentials are never copied between machines.`,
        selectAlignmentTool: ({ component }: { component: string }) => `Select ${component} alignment`,
        previewComponentAlignment: ({ component }: { component: string }) => `Preview ${component} alignment`,
        confirmComponentAction: ({ component }: { component: string }) => `Apply ${component} alignment`,
    },
    relationshipAdvisor: {
    ...
    relationshipAdvisorPlugin: {
        marketTitle: 'Plugins',
        marketSubtitle: 'Discover and enable plugins included with this version of Paws.',
        ...
        permissionGrantNotice: 'Installing or updating grants these permissions to this account. Uninstalling revokes them.',
        builtInCode: 'Built-in trusted code',
        builtInCodeNotice: 'The plugin code ships with this version of Paws. Installing only enables it for your account.',
        permissionAiProviderInvoke: 'Use an AI provider',
        permissionAiProviderInvokeDescription: "Send this plugin's conversation requests to the configured AI provider.",
        permissionSecretsUse: 'Use encrypted secrets',
        permissionSecretsUseDescription: "Read this plugin's encrypted configuration only while it runs.",
        permissionConversationImagesRead: 'Read conversation images',
        permissionConversationImagesReadDescription: 'Read image attachments you choose to send to this plugin.',
        permissionStorageImagesWrite: 'Write plugin images',
        permissionStorageImagesWriteDescription: 'Upload and delete images created or used by this plugin.',
        encryptionNotice: 'Your configuration, including the API key, is encrypted and stored on the Paws server. A saved key is returned to this app only when you explicitly choose to reveal it.',
        secretRevealFailed: 'Unable to reveal the saved key. Please try again.',
        apiKey: 'API Key',
        apiKeyConfigured: ({ hint }: { hint: string }) => `Configured · ends in ${hint}`,
        apiKeyPlaceholder: 'e.g. sk-...',
        baseUrl: 'Base URL',
        baseUrlPlaceholder: 'e.g. https://api.deepseek.com',
        model: 'Model',
        modelPlaceholder: 'e.g. deepseek-v4-flash-vision-exp',
        unsavedChangesTitle: 'Unsaved changes',
        unsavedChangesSubtitle: 'Your current configuration has not been saved. Save it before leaving.',
        modelRecommendation: 'Prefer a multimodal model so it can understand text and images.',
        testConnection: 'Test connection',
        testConnectionSubtitle: 'After a successful test, the current configuration will be saved automatically',
        connectionSuccess: 'Connection successful',
        connectionSuccessSubtitle: 'The current settings can reach the provider and model',
        connectionAuthenticationFailed: 'Authentication failed',
        connectionModelNotFound: 'Model not found',
        connectionRateLimited: 'Rate limit reached',
        connectionTimedOut: 'Connection timed out',
        connectionUnreachable: 'Unable to reach the provider',
        connectionProviderError: 'The provider returned an error',
        status: 'Plugin status',
        installed: 'Installed',
        installedSubtitle: 'Ready to use on this account',
        reviewRequired: 'Review required',
        reviewRequiredSubtitle: 'The plugin version or permissions changed. Review and update before use.',
        reviewAndUpdate: 'Review and update',
        notInstalled: 'Not installed',
        notInstalledSubtitle: 'Complete the configuration and install the plugin to use it',
        update: 'Update configuration',
        install: 'Install plugin',
        installSubtitle: 'Encrypt and save this configuration on the Paws server',
        uninstall: 'Uninstall plugin',
        uninstallSubtitle: 'Remove the plugin and its saved credentials',
    },
    healthPanel: {
    ...
    generatedImageBatchDownload: {
        preparing: ({ ready, total }: { ready: number; total: number }) => `Preparing ${ready}/${total}`,
        generating: ({ completed, total }: { completed: number; total: number }) => `Generating ${completed}/${total}`,
        downloadAll: ({ count }: { count: number }) => `Download all (${count})`,
        downloading: ({ completed, total }: { completed: number; total: number }) => `Downloading ${completed}/${total}`,
        saved: ({ count }: { count: number }) => `Saved ${count} image${count === 1 ? '' : 's'}`,
        savedBrowser: ({ count }: { count: number }) => `${count} download${count === 1 ? '' : 's'} started · Allow multiple downloads if your browser asks`,
        savedDirectory: ({ count }: { count: number }) => `Saved ${count} image${count === 1 ? '' : 's'} to the selected folder`,
        savedPhotos: ({ count }: { count: number }) => `Saved ${count} image${count === 1 ? '' : 's'} to Photos`,
        cancelledBrowser: 'Downloads cancelled · Try again and allow multiple downloads',
        cancelledDirectory: 'No folder selected · Choose a folder to save the images',
        cancelledPhotos: 'Photos access is required to save generated images',
        cancelledUnsupported: 'Batch download is not available on this platform',
        partial: ({ succeeded, failed }: { succeeded: number; failed: number }) => `${succeeded} saved · ${failed} failed`,
        retryFailed: ({ count }: { count: number }) => `Retry failed (${count})`,
    },
    desktopWorkspace: {
        sessions: 'Sessions',
    ...
    interactivePreviews: {
        ...
        provider: 'Cloudflare',
        hostedProvider: 'Cloudflare cloud hosting',
        hostedDescription: 'Hosted for 24 hours, even after the session machine shuts down. The server retries expired preview deletion if Cloudflare is unavailable.',
        tunnelProvider: 'Anonymous temporary tunnel',
        accountId: 'Cloudflare Account ID',
        apiToken: 'API Token',
        tokenHelp: 'Use an API Token with Account / Cloudflare Pages / Edit permission scoped to the selected account. It is stored encrypted on the server and is never included in previews.',
        configure: 'Configure cloud hosting',
        invalidCredentials: 'Check your Cloudflare Account ID and scoped API Token.',
        secureConnectionRequired: 'Cloud hosting credentials require an HTTPS Happy Server connection. Switch to an HTTPS server before entering your Cloudflare API Token.',
        saveConnection: 'Save connection',
        cloudflareProvider: 'Cloudflare',
        cloudflareDescription: 'No login required for anonymous tunnels. Ask the assistant to publish via a Cloudflare tunnel. The session machine needs cloudflared installed.',
        sessionLifetime: 'Available while the session and its machine are running, for up to 24 hours.',
        connection: 'Cloudflare hosting account',
        loading: 'Loading connection…',
        unavailable: 'Cloud hosting is unavailable on this Happy Server. Anonymous temporary tunnels are still available.',
        disconnected: 'Not connected',
        connected: ({ name }: { name: string }) => `Connected to ${name}`,
        project: 'Preview project',
        connect: 'Connect with API Token',
        reconnect: 'Update API Token',
        disconnect: 'Disconnect cloud hosting',
        disconnectTitle: 'Disconnect Cloudflare hosting?',
        disconnectBody: 'New hosted previews will stop immediately. Existing hosted previews are removed when possible. Anonymous temporary tunnels remain available.',
        disconnectWarning: 'Some hosted previews could not be removed. Delete them manually in Cloudflare and revoke the API Token.',
        popupBlocked: 'Configure cloud hosting with your Cloudflare Account ID and scoped API Token.',
        safeError: 'Unable to update temporary previews. Please retry.',
        retry: 'Retry',
        disclosure: 'Preview links are public. Publish only non-sensitive content. Hosted previews are scheduled for deletion after 24 hours; anonymous tunnels may close earlier when the session or its machine stops.',
    ...
    imageUpload: {
        ...
        mediaCollapse: ({ name }: { name: string }) => `Collapse ${name} player`,
        mediaFullscreen: ({ name }: { name: string }) => `View ${name} in full screen`,
        mediaLoadFailed: 'Could not load this media file.',
    ...
    sessionInfo: {
        ...
        agentPanelRuntimeLocation: 'Runtime location',
        agentPanelCurrentExecution: 'This execution',
        agentPanelSessionManagement: 'Session management',
        agentPanelMachineStatus: 'Machine status',
        agentPanelAddress: 'Address',
        agentPanelWorkingDirectory: 'Working directory',
        agentPanelAgent: 'Agent',
        agentPanelModel: 'Model',
        agentPanelEffort: 'Reasoning effort',
        agentPanelPermissions: 'Permissions',
        agentPanelOfflineNotice: 'This machine is offline. Settings are kept, but execution is unavailable.',
        agentPanelReadOnly: 'Read-only',
        ...
        sessionRowRemoteLocation: ({ name }: { name: string }) => `Remote · ${name}`,
        sessionRowUnknownLocation: 'Location unknown/local',
        sessionRowUnknownAgent: 'Unknown agent',
        sessionRowProjectPath: 'Project/path',
        sessionRowMachineAgent: 'Machine/Agent',
        sessionRowRelativeTime: 'Relative time',
        sessionRowRunningStatus: 'Running status',
        sessionRowMoreActions: 'More session actions',
    ...
    sessionShare: {
        ...
        coverAttribution: ({ photographer }: { photographer: string }) => `Photo by ${photographer} on Pexels`,
    ...
    components: {
        ...
        messageComposer: {
            ...
            workingDirectoryBrowse: 'Browse folders',
            workingDirectoryRecent: 'Recent directories',
            workingDirectoryNoRecent: 'No other recent directories on this machine',
            workingDirectorySelectCurrent: ({ name }: { name: string }) => `Use ${name}`,
            workingDirectoryError: ({ reason }: { reason: string }) => `This directory cannot be used. Check the path and access, then try again. ${reason}`,
            workingDirectoryEmpty: 'Enter or browse to a directory first.',
            workingDirectoryOffline: 'The Agent machine is offline. Reconnect it before changing directories.',
    ...
    agentInput: {
        ...
        taskPermission: {
            ...
            confirmShort: 'Ask first',
            confirmDescription: 'Uses the agent confirmation flow for actions that need extra permission. Device and outer sandbox limits still apply.',
            fullAccess: 'Full access',
            fullAccessShort: 'Full',
            fullAccessDescription: 'Bypasses agent confirmations where supported. Device and outer sandbox limits still apply.',
            changesNextMessages: 'Applies to the next and later messages.',
            unsupported: 'This agent does not offer two reliable permission levels.',
            unavailable: 'Unavailable',
            riskTitle: 'Enable full access?',
            riskMessage: 'Full access bypasses agent confirmations where supported, including for potentially high-risk actions. It does not override device or outer sandbox limits. Only continue if you trust this task.',
            riskConfirm: 'Enable full access',
        },
        agent: {
            claude: 'Claude',
            codex: 'Codex',
    ...
    sidebarLists: {
        ...
        noTaggedSessions: 'No sessions use this tag yet.',
        noTags: 'No tags yet',
        agentList: 'Agent list',
        workspaceList: 'Workspace list',
        newSessionInList: 'New session in this list',
        emptyList: 'No sessions in this list yet.',
        unassigned: 'Unassigned',
        unassignedDescription: 'Sessions not in a list',
        organizeSession: 'Organize session',
        listName: 'List name',
        listNamePlaceholder: 'For example: Happy',
        duplicateListName: 'A list with this name already exists.',
        listType: 'List type',
        color: 'Color',
        defaultMachine: 'Default machine',
        defaultDirectory: 'Default directory',
        directoryPlaceholder: '~/projects/happy',
        defaultAgent: 'Default Agent',
        noPreset: 'No preset',
        agentPrompt: 'Agent prompt',
        agentPromptPlaceholder: 'Prompt applied when starting from this Agent list',
        belongsToList: 'List',
        tagsMultiSelect: 'Tags (select multiple)',
        selectedTags: 'Selected Tags',
        tagInputPlaceholder: 'Type # to search or create a Tag',
        createTagNamed: ({ name }: { name: string }) => `Create ${name}`,
        ...
        showArchived: 'Show archived',
        hideArchived: 'Hide archived',
        archivedSessionsHidden: 'Archived sessions are hidden.',
        tagSessionsNotLoaded: 'Linked historical sessions have not loaded yet.',
        tagSessionsPartiallyLoaded: ({ loadedCount, sessionCount }: { loadedCount: number; sessionCount: number }) => `Showing ${loadedCount} loaded sessions out of ${sessionCount} linked sessions.`,
    ...
    updateBanner: {
        otaOnlyTitle: "No OTA update available",
        otaOnlyMessage: "No OTA update is available for this installation. Native App update checking is not supported for this package.",
        tapToDownloadGitHub: 'Tap to download from GitHub Release',
        nativeUpdateMessage: "A newer App package is available. Some updates require the newer package. Download it from GitHub Release and install it over the existing App; no uninstall is needed.",
        downloadApp: 'Download APK',
        nativeCheckFailedTitle: 'App update check unavailable',
        nativeCheckFailedMessage: 'The native App version could not be confirmed. Please retry later.',
        ...
        forceCheckSubtitle: "Check for App and OTA updates now",
    ...
    machine: {
        ...
        codexUsageHeatmap: 'Activity Over the Past Year',
    ...
    agents: {
        ...
        imageStyleSceneDistillationZineHint: 'Distill a photo into an original 3:5 portrait or 5:3 landscape paper-zine illustration—keep semantic and emotional anchors, never photographic pixels.\nExpressive torn edges, authorial type, one purposeful vivid accent; exact trigger 单色块模式 enables one contiguous saturated color field.',
        imageStyleGradeImages: 'Deterministic Photo Grade',
        imageStyleGradeImagesHint: 'Correct, match, or creatively color-grade JPEG/PNG photos without generative editing.\nStrictly preserves geometry, identity, faces, objects, text, and texture; supports conservative to transformative intensity, reproducible recipes, previews, and quality gates.',
        imageStyleScenesGatheredZine: 'Gathered Scenes Zine',
        imageStyleScenesGatheredZineHint: 'Anchor truthful photography in a spacious 3:5 tactile paper poster and expand selected source shapes into a highly simplified illustration field.\nOne structural vivid hue, a clearly fibrous torn-paper handoff, active negative space, and restrained micro-text.',
        imageStyleScenesGatheredZineSea: 'Gathered Scenes Zine · Sea',
        imageStyleScenesGatheredZineSeaHint: 'A coastal reference for the same Gathered Scenes compiler: truthful sea/railing photography, broad quiet paper, fibrous torn boundaries, compressed print marks, one structural hue, and restrained micro-text.',
