...
export async function linkTaskToSession(
    ...
    await updateTodoLinkedSessions(credentials, taskId, linkedSessions);
}
 ...
 */
export async function removeSessionLink(credentials: AuthCredentials, sessionId: string): Promise<void> {
    const todos = storage.getState().todoState?.todos || {};
    ...
    for (const [taskId, todo] of Object.entries(todos)) {
        if (todo.linkedSessions?.[sessionId]) {
            ...
            await updateTodoLinkedSessions(credentials, taskId, remaining);
            break;
        }
    }
}
