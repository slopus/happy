...
import { removeTaskLinks } from '@/-zen/model/taskSessionLink';
...
export const ZenView = React.memo(() => {
    ...
    // Get linked sessions for this task (reactive to storage changes)
    const linkedSessions = storage(useShallow(state => {
        const todo = state.todoState?.todos[todoId];
    ...
    React.useEffect(() => {
        ...
        const handleKeyDown = (event: KeyboardEvent) => {
            // Navigate to new todo when any letter key is pressed (except when editing)
                    ...
                    {linkedSessions.length > 0 && (
                        <View style={styles.linkedSessionsSection}>
                            ...
                            {linkedSessions.map((link, index) => (
                                <Pressable
                                    key={link.sessionId}
                                    onPress={() => { router.back(); router.push(`/session/${link.sessionId}`); }}
