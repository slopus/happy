...
// Global client reference accessible from the VoiceSession implementation
...
class OpenAIVoiceSessionImpl implements VoiceSession {
    ...
    async startSession(sessionConfig: VoiceSessionConfig): Promise<void> {
        if (!clientInstance) {
            console.warn('[OpenAIVoiceSession] Client not initialized');
            return;
        }
        ...
        try {
            ...
            console.error('[OpenAIVoiceSession] Failed to start:', error);
...
export const RealtimeVoiceSession: React.FC = () => {
    ...
    useEffect(() => {
        // Create OpenAI Realtime client with RN WebRTC
        clientInstance = new OpenAIRealtimeClient(
            {
                onConnect: () => {
                    console.log('[OpenAIVoiceSession] Connected');
                    ...
                    try {
                        ...
                        console.warn('[OpenAIVoiceSession] Failed to send roster:', error);
                    }
                },
                onDisconnect: (reason) => {
                    console.log('[OpenAIVoiceSession] Disconnected:', reason);
                    storage.getState().setRealtimeStatus('disconnected');
                ...
                onError: (error) => {
                    console.warn('[OpenAIVoiceSession] Error:', error);
                    storage.getState().setRealtimeStatus('disconnected');
                    storage.getState().setRealtimeMode('idle', true);
                },
            },
