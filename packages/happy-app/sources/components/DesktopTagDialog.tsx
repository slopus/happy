...
export function DesktopTagDetailDialog({ groups, listColors, onClose, onDelete, selectedSessionId, tag }: {
            ...
            <View style={styles.detailToolbar}>
                <View style={styles.detailMetaGroup}>
                    ...
                    <Text style={styles.detailMeta} testID={`tag-detail-meta-${tag.id}`}>
                        {t('sidebarLists.tagDetailsMeta', { sessionCount, listCount: groups.length })}
                    </Text>
                </View>
                <Pressable
                    accessibilityLabel={`${t('sidebarLists.deleteTag')} ${tag.name}`}
                    accessibilityRole="button"
                    onPress={() => onDelete(tag)}
                    style={({ pressed }) => [styles.detailDelete, pressed && styles.detailDeletePressed]}
                    testID={`tag-detail-delete-${tag.id}`}
                >
                    <Feather color={theme.colors.deleteAction} name="trash-2" size={14} />
                    <Text style={styles.detailDeleteText}>{t('sidebarLists.deleteTag')}</Text>
                </Pressable>
            </View>
            <ScrollView contentContainerStyle={styles.detailContent} style={styles.detailScroll}>
                {groups.length === 0 ? <Text style={styles.empty}>{t('sidebarLists.noTaggedSessions')}</Text> : null}
                {groups.map((group) => {
                    const expanded = !collapsedGroupIds.has(group.id);
                    const label = group.list?.name ?? t('sidebarLists.unassigned');
                    ...
                    return (
                        <View key={group.id} style={styles.group} testID={`tag-detail-group-${group.id}`}>
                            ...
                            >
                                ...
                                <Feather color={color} name={group.list?.kind === 'agent' ? 'cpu' : group.list ? 'folder' : 'inbox'} size={15} />
...
const styles = StyleSheet.create((theme) => ({
    ...
    detailDelete: { alignItems: 'center', borderRadius: 7, flexDirection: 'row', gap: 6, minHeight: 32, paddingHorizontal: 9 },
    detailDeletePressed: { backgroundColor: theme.colors.surfacePressed },
    detailDeleteText: { color: theme.colors.deleteAction, fontSize: 12, ...Typography.default('semiBold') },
