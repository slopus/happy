...
The `/group-chat` screen opens the same workspace in a Web iframe or native WebView. A single-use 30-second ticket is exchanged for an in-memory scoped session; the Paws bearer and recovery secret never appear in a URL or browser storage. The server validates the bearer and independently restores and verifies credential ownership against the fixed trusted relay before attaching the SDK. SDK credentials remain in memory. This is a trusted hosted Agent executor, with the same account access needed to run Agents; it is not a browser-only execution service.
