...
describe('SessionClient', () => {
    ...
    describe('sendPermissionDecision', () => {
        it('encrypts and sends a permission-response control message', () => {
            const opts = makeOptions();
            const client = new SessionClient(opts);
            ...
            expect(data.sid).toBe(opts.sessionId);
            expect(decrypted.type).toBe('permission-response');
            expect(decrypted.sessionID).toBe(opts.sessionId);
            expect(decrypted.requestID).toBe('perm-123');
            expect(decrypted.callID).toBe('call-456');
            expect(decrypted.decision).toBe('always');
