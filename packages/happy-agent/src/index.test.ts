...
describe('happy-agent CLI', () => {
    ...
    describe('approve command', () => {
        it('should show approve help with session-id, request-id, and --always options', () => {
            const { stdout } = runCli('approve', '--help');
            expect(stdout).toContain('Approve a pending permission request');
            expect(stdout).toContain('session-id');
        ...
        it('should fail with auth error when not authenticated', () => {
            const { stderr, exitCode } = runCli('approve', 'fake-id');
            expect(exitCode).not.toBe(0);
            expect(stderr).toContain('happy-agent auth login');
        });
    });
