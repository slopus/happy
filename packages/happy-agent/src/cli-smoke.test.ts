...
describe('Smoke: CLI command surface', () => {
    ...
    describe('7. approve command', () => {
        it('shows help with session-id, request-id, and --always', () => {
        ...
        it('fails with auth error when not authenticated', () => {
            const { stderr, exitCode } = runCli('approve', 'abc');
            expect(exitCode).not.toBe(0);
            expect(stderr).toContain('happy-agent auth login');
        });
    });

    describe('8. stop command', () => {
