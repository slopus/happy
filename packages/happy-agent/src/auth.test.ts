...
import { authLogin, authLogout, authStatus, inspectLocalCredentialSeedSource, trySeedCredentialsFromLocalAccessKey } from './auth';
...
describe('auth', () => {
    ...
    describe('authLogin', () => {
        it('seeds credentials from a local legacy access.key before QR auth', async () => {
            const seededSecret = getRandomBytes(32);
            writeFileSync(
                join(config.homeDir, 'access.key'),
                JSON.stringify({
                    token: 'seeded-token',
                    secret: encodeBase64(seededSecret),
                }),
            );
            ...
            expect(creds!.token).toBe('seeded-token');
            expect(creds!.secret).toEqual(seededSecret);
    ...
    describe('authStatus', () => {
        ...
        it('reports that local data-key auth cannot seed happy-agent', async () => {
            writeFileSync(
                join(config.homeDir, 'access.key'),
                JSON.stringify({
                    token: 'data-key-token',
                    encryption: {
                        publicKey: encodeBase64(getRandomBytes(32)),
                        machineKey: encodeBase64(getRandomBytes(32)),
                    },
                }),
            );
            ...
            const calls = consoleSpy.mock.calls.map(c => String(c[0]));
            expect(calls).toContain('- Local Seed: Unavailable');
            expect(calls.some(call => call.includes('data-key'))).toBe(true);
        });
    ...
    describe('local credential seed helpers', () => {
        it('inspects legacy and data-key access.key formats correctly', () => {
            expect(inspectLocalCredentialSeedSource(config)).toBe('missing');
            ...
            expect(inspectLocalCredentialSeedSource(config)).toBe('legacy');
            ...
            expect(inspectLocalCredentialSeedSource(config)).toBe('data-key');
        });

        it('seeds agent credentials only from legacy access.key', () => {
            expect(trySeedCredentialsFromLocalAccessKey(config)).toBe(false);
            ...
            expect(trySeedCredentialsFromLocalAccessKey(config)).toBe(true);
            expect(readCredentials(config)?.secret).toEqual(seededSecret);
        });
    });
});
