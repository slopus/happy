    ...
    const spawnSession = async (options: SpawnSessionOptions): Promise<SpawnSessionResult> => {
      ...
      try {
        ...
        if (useTmux && tmuxSessionName !== undefined) {
          ...
          // Determine agent command - support claude, codex, gemini, and openclaw
          const agent = options.agent === 'openclaw' ? 'openclaw' : options.agent === 'gemini' ? 'gemini' : (options.agent === 'codex' ? 'codex' : 'claude');
        ...
        if (!useTmux) {
          ...
          // Construct arguments for the CLI - support claude, codex, gemini, and openclaw
