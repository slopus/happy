...
describe('orchestrator prompt helpers', () => {
  ...
  it('enables orchestrator tools only when feature is on and not worker session', () => {
    expect(shouldEnableOrchestratorTools({ HAPPY_ORCHESTRATOR_V1: '1' } as NodeJS.ProcessEnv)).toBe(true);
    expect(shouldEnableOrchestratorTools({
      HAPPY_ORCHESTRATOR_V1: '1',
      HAPPY_ORCH_ONESHOT: '1',
    } as NodeJS.ProcessEnv)).toBe(false);
    expect(shouldEnableOrchestratorTools({} as NodeJS.ProcessEnv)).toBe(false);
  });

  it('only returns orchestrator instructions for controller sessions', () => {
    const controller = getOrchestratorToolsInstruction({ HAPPY_ORCHESTRATOR_V1: '1' } as NodeJS.ProcessEnv);
    ...
    expect(getOrchestratorToolsInstruction({} as NodeJS.ProcessEnv)).toBeNull();
  });

  it('appends orchestrator instructions onto first-turn base instruction', () => {
    const combined = buildFirstTurnToolingInstruction('change_title', { HAPPY_ORCHESTRATOR_V1: '1' } as NodeJS.ProcessEnv);
