...
export class BackgroundSyncManager {
	...
	private connectionHealthInterval: ReturnType<typeof setInterval> | null =
		null;
	...
	constructor(
		private config: BackgroundSyncConfig = DEFAULT_BACKGROUND_CONFIG,
	) {
	...
	private async registerBackgroundTasks() {
		try {
			...
			if (!EXPO_MODULES_AVAILABILITY.taskManager) {
				log.log(
					"⚠️ TaskManager not available, background tasks will use fallback methods",
				);
				return;
			}
			...
			TaskManager.defineTask(
				...
				this.backgroundSyncTask.bind(this),
			);

			// Register connection maintenance task
			TaskManager.defineTask(
				CONNECTION_MAINTENANCE_TASK,
				this.connectionMaintenanceTask.bind(this),
			);
			...
			if (Platform.OS !== "web" && EXPO_MODULES_AVAILABILITY.backgroundFetch) {
				...
				log.log(
					"⚠️ BackgroundFetch not available, background sync will use alternative methods",
				);
			}
		} catch (error) {
	...
	private async startWebBackgroundSync() {
		if (typeof document !== "undefined") {
			...
			document.addEventListener(
				"visibilitychange",
				this.handleWebVisibilityChange.bind(this),
			);
		}
	...
	private async maintainCriticalConnections() {
		try {
			...
			if (!apiSocket.isConnected()) {
				log.log(
					"🔌 Socket disconnected in background, attempting reconnection",
				);
				await this.attemptReconnection();
			} else {
	 ...
	 */
	private async processCriticalOperation(operation: {
		operation: string;
		data: any;
		timestamp: number;
	}) {
	...
	private async checkConnectionHealth() {
		...
		log.log(
			`🔍 Connection health check: connected=${isConnected}, quality=${connectionQuality}`,
		);
	...
	private async preserveSessionState() {
		try {
			...
			log.log(
				`💾 Preserving state for ${activeSessions.length} active sessions`,
			);
		} catch (error) {
	...
	private async cleanupStaleConnections() {
		try {
			...
			log.log(
				`🧹 Cleaned up stale operations, ${this.criticalSyncQueue.length} remaining`,
			);
		} catch (error) {
	...
	private performMinimalConnectionCheck() {
		if (apiSocket.isConnected()) {
			...
			if (Date.now() - lastActivity > 60000) {
				...
				log.log(
					"🔌 Connection appears stale, will reconnect on app activation",
				);
			}
		}
	}
