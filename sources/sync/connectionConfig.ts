...
export function getConnectionConfig(): ConnectionConfig {
	...
	const forceDisableEnhanced =
		process.env.EXPO_PUBLIC_DISABLE_ENHANCED_CONNECTION === "true";

	if (forceDisableEnhanced) {
		console.warn(
			"🔧 Enhanced connection management disabled via environment variable",
		);
...
export function setConnectionConfigOverride(
...
export function getEnhancedFeaturesStatus(): EnhancedFeatures {
	...
	return {
		connectionHealthMonitoring:
			config.enableEnhancedConnectionManagement &&
			config.enableConnectionHealthMonitoring,
		staleConnectionCleanup:
			config.enableEnhancedConnectionManagement &&
			config.enableStaleConnectionCleanup,
		sessionStatePersistence:
			config.enableEnhancedConnectionManagement &&
			config.enableSessionStatePersistence,
		adaptiveRetryLogic:
			config.enableEnhancedConnectionManagement &&
			config.enableExponentialBackoff,
