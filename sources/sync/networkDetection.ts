...
export class NetworkDetection {
	...
	start(): void {
		...
		try {
			...
			console.warn(
				"🌐 NetworkDetection: Failed to add NetInfo listener:",
				error,
			);
		}
	 ...
	 */
	private async createNetworkProfile(
	...
	private calculateStability(): number {
		...
		const recentResults = this.latencyHistory.slice(
	...
	public getOptimalStrategy(profile: NetworkProfile): ConnectionStrategy {
		...
		if (!strategy) {
			if (profile.type === "unknown") {
				...
				strategy =
					NETWORK_STRATEGIES[fallbackKey] ||
					NETWORK_STRATEGIES["unknown-default"];
			}
		}
		...
		if (profile.type === "cellular" && profile.generation) {
			strategy = this.adjustStrategyForCellularGeneration(
				strategy,
				profile.generation,
			);
		}
	...
	private adjustStrategyForStability(
		...
		if (profile.stability < 0.5) {
			...
			adjusted.timeouts.connection = Math.max(
				adjusted.timeouts.connection * 1.3,
				15000,
			);
			adjusted.retryPolicy.maxAttempts = Math.min(
				adjusted.retryPolicy.maxAttempts + 2,
				8,
			);
			adjusted.retryPolicy.backoffMultiplier = Math.min(
				adjusted.retryPolicy.backoffMultiplier * 1.2,
				3.0,
			);
			...
			adjusted.timeouts.connection = Math.max(
				adjusted.timeouts.connection * 0.8,
				5000,
			);
			adjusted.timeouts.heartbeat = Math.min(
				adjusted.timeouts.heartbeat * 1.2,
				40000,
			);
		}
	...
	private adjustStrategyForCellularGeneration(
		...
		switch (generation) {
			case "3g":
				...
				adjusted.timeouts.connection = Math.max(
					adjusted.timeouts.connection * 1.5,
					20000,
				);
				adjusted.timeouts.heartbeat = Math.max(
					adjusted.timeouts.heartbeat * 0.8,
					15000,
				);
				adjusted.retryPolicy.maxAttempts = Math.min(
					adjusted.retryPolicy.maxAttempts + 1,
					7,
				);
				adjusted.heartbeatProfile = "aggressive";
				break;
			...
			case "5g":
				...
				adjusted.timeouts.heartbeat = Math.min(
					adjusted.timeouts.heartbeat * 1.1,
					35000,
				);
				break;
		}
		...
		return {
			...
			successRate:
				this.latencyHistory.length > 0
					? this.latencyHistory.filter((r) => r.success).length /
			...
			averageLatency:
				this.latencyHistory.length > 0
					? this.latencyHistory
						...
						this.latencyHistory.filter((r) => r.success && r.latency !== null)
							.length
					: null,
