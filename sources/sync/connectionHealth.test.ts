...
describe("ConnectionHealthMonitor - Heartbeat Profiles", () => {
	...
	describe("Heartbeat Profiles Configuration", () => {
		...
		it("should validate aggressive profile detects failures faster than standard", () => {
			...
			expect(aggressive.maxConsecutiveFailures).toBeLessThanOrEqual(
				standard.maxConsecutiveFailures,
			);
		});
		...
		it("should validate battery saver reduces ping frequency", () => {
			...
			expect(batterySaver.interval).toBeGreaterThanOrEqual(
				standard.interval * 2,
	...
	describe("Ping Behavior with Profiles", () => {
		...
		it("should track failure history for auto-detection", async () => {
			...
			expect(
				monitor_any.failureHistory[monitor_any.failureHistory.length - 1],
			).toMatchObject({
		...
		it("should track latency history for auto-detection", async () => {
			...
			expect(
				monitor_any.latencyHistory[monitor_any.latencyHistory.length - 1],
			).toBeGreaterThan(100);
		});
	...
	describe("Performance Requirements", () => {
		...
		it("should clean up old failure history", () => {
			...
			expect(
				monitor_any.failureHistory.every((f: any) => f.type !== "old"),
			).toBe(true);
		});
	});
	...
	describe("Integration with Existing System", () => {
		...
		it("should track network changes for auto-detection", () => {
			...
			const onStatusChangeCallback = (apiSocket.onStatusChange as Mock).mock
				.calls[0][0];
	...
	describe("Error Handling and Edge Cases", () => {
		...
		it("should handle periodic auto-detection triggers", () => {
			const applyAutoDetectedProfileSpy = vi.spyOn(
				monitor,
				"applyAutoDetectedProfile",
			);
			const monitor_any = monitor as any;
			...
			if (
				monitor_any.latencyHistory.length % 10 === 0 &&
				monitor_any.latencyHistory.length > 0
			) {
				monitor.applyAutoDetectedProfile();
			}
