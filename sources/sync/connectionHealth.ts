...
export class ConnectionHealthMonitor {
	...
	setProfile(profileName: keyof typeof HEARTBEAT_PROFILES): void {
		if (!HEARTBEAT_PROFILES[profileName]) {
			console.warn(
				`Unknown profile '${profileName}', falling back to 'standard'`,
			);
	...
	private async performHealthCheck(): Promise<void> {
		...
		try {
			...
			if (
				this.latencyHistory.length % 10 === 0 &&
			...
			console.log(
				`🏥 ConnectionHealthMonitor: Ping successful - ${latency}ms (${this.status.quality})`,
			);
		} catch (error) {
			// Handle ping failure
			...
			if (
				...
				currentProfileData.maxConsecutiveFailures
			) {
		...
		if (this.lastDisconnectTime) {
			this.status.downtime =
				...
				(this.connectionStartTime
					? this.connectionStartTime - this.lastDisconnectTime
					: 0);
		}
	}
	...
	private getRecentFailures(): number {
		...
		return this.failureHistory.filter((f) => f.timestamp > tenMinutesAgo)
			.length;
	}
	...
	private setupSocketListeners(): void {
		...
		connectionStateMachine.addStateChangeListener((state, context, event) => {
			...
			console.log(
				`🏥 ConnectionHealthMonitor: State machine updated: ${state} (${event.type})`,
			);
			this.notifyListeners();
		});
