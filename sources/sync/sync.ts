...
class Sync {
	...
	async sendMessage(sessionId: string, text: string) {
		...
		if (sendSuccess) {
			...
			log.error(
				`Failed to send message for session ${sessionId}, message not added to local UI`,
			);
		}
	}
	...
	private fetchSessions = async () => {
		...
		for (const session of sessions) {
			if (session.dataEncryptionKey) {
				const decrypted = await this.encryption.decryptEncryptionKey(
	...
	public enableEnhancedConnectionManagement = () => {
		try {
			...
			import("./staleConnectionCleaner").then(
				({ startStaleConnectionCleanup }) => {
					startStaleConnectionCleanup();
			...
			import("./sessionStatePersistence").then(
				({ startSessionStatePersistence }) => {
					startSessionStatePersistence();
	...
	private fetchMessages = async (sessionId: string) => {
		...
		const decryptedMessages =
			await encryption.decryptMessages(messagesToDecrypt);
		...
		console.log(
			"Batch decrypted and normalized messages in",
			Date.now() - start,
			"ms",
		);
		...
		log.log(
			`💬 fetchMessages completed for session ${sessionId} - processed ${normalizedMessages.length} messages`,
		);
	};
	...
	fetchArtifactsList = async () => {
		...
		try {
			...
			for (const artifact of encryptedArtifacts) {
				try {
					// Decrypt header
					const decryptedHeader = await this.encryption.decryptRaw(
						artifact.header,
					);
					const header = JSON.parse(
						decryptedHeader,
					) as import("./artifactTypes").ArtifactHeader;
			...
			console.log(
				`📋 fetchArtifactsList completed - processed ${decryptedArtifacts.length} artifacts`,
			);
		} catch (error) {
	...
	fetchArtifactWithBody = async (
		artifactId: string,
	): Promise<import("./artifactTypes").DecryptedArtifact | null> => {
		if (!this.credentials) {
		...
		try {
			...
			const encryptedArtifact = await fetchArtifact(
				this.credentials,
				artifactId,
			);
			...
			if (encryptedArtifact.body) {
				const decryptedBody = await this.encryption.decryptRaw(
					encryptedArtifact.body,
				);
				const bodyData = JSON.parse(
					decryptedBody,
				) as import("./artifactTypes").ArtifactBody;
				body = bodyData.body;
			}
			...
			console.log(
				`📋 fetchArtifactWithBody completed for artifact ${artifactId}`,
			);
	...
	createArtifact = async (
		...
		try {
			...
			const encryptedHeader = await this.encryption.encryptRaw(
			...
			const encryptedBody = await this.encryption.encryptRaw(
			...
			const encryptedDataKey =
				await this.encryption.encryptEncryptionKey(dataKey);
