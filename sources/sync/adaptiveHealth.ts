...
export class AdaptiveHealthMonitor {
	...
	start(scheduleCallback: (interval: number) => void): void {
		if (this.isRunning) {
			console.log(
				"🔄 AdaptiveHealthMonitor: Already running, ignoring start call",
			);
			return;
		}
		...
		return {
			...
			successRate:
				this.pingHistory.length > 0
					? this.pingHistory.filter((r) => r.success).length /
	...
	private adaptPingInterval(): void {
		...
		if (this.consecutiveFailures >= 2 || stability < 0.7) {
			...
			console.log(
				`🔄 AdaptiveHealthMonitor: Network unstable - increasing frequency to ${this.currentInterval}ms`,
			);
		} else if (
			this.consecutiveSuccesses >= 5 &&
			stability > this.config.stabilityThreshold
		) {
			...
			console.log(
				`🔄 AdaptiveHealthMonitor: Network stable - reducing frequency to ${this.currentInterval}ms`,
			);
			...
			console.log(
				`🔄 AdaptiveHealthMonitor: Latency trending up - increasing frequency to ${this.currentInterval}ms`,
			);
			...
			console.log(
				`🔄 AdaptiveHealthMonitor: Latency improving - reducing frequency to ${this.currentInterval}ms`,
			);
		}
		...
		const changePercent =
			Math.abs(this.currentInterval - oldInterval) / oldInterval;
	...
	private calculateStability(): number {
		...
		const variance =
			latencies.reduce((acc, lat) => acc + Math.pow(lat - avgLatency, 2), 0) /
			latencies.length;

		// Calculate latency stability (lower variance = higher stability)
		const latencyStability =
			avgLatency > 0 ? Math.max(0, 1 - Math.sqrt(variance) / avgLatency) : 1.0;
	...
	getAnalytics(): {
		...
		const successfulPings = this.pingHistory.filter(
			(r) => r.success && r.latency !== undefined,
		);
		const avgLatency =
			successfulPings.length > 0
				? successfulPings.reduce((sum, r) => sum + r.latency!, 0) /
