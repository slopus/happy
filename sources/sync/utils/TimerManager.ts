	...
	setTimeout(
		...
		const wrappedCallback = () => {
			try {
				...
				log.error(
					`Timer callback error for ${id} - Source: ${this.source} - Error: ${err}`,
				);
		...
		if (options?.name) {
			log.log(
				`DEBUG: Set timeout '${options.name}' (${id}) for ${delay}ms - Source: ${this.source}`,
			);
		}
	...
	setInterval(
		...
		const wrappedCallback = () => {
			try {
				...
				const err = error instanceof Error ? error : new Error(String(error));
				log.error(
					`Interval callback error for ${id} - Source: ${this.source} - Error: ${err}`,
				);
		...
		if (options?.name) {
			log.log(
				`DEBUG: Set interval '${options.name}' (${id}) for ${interval}ms - Source: ${this.source}`,
			);
		}
