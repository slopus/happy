  ...
  static handleError(
    ...
    if (recovery) {
      try {
        ...
        log.error(`Recovery failed for ${context.operation} - Source: ${context.source} - Error: ${recoveryError as Error}`);
      }
    }
  ...
  static async withRetry<T>(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        log.log(`WARN: Retry ${attempt + 1}/${maxRetries} for ${context.operation} after ${delay}ms - Source: ${context.source}`);
