...
export async function machineStopDaemon(
	...
	try {
		log.log(
			`🛑 machineStopDaemon: Sending stop-daemon command to machine ${machineId}`,
		);
		const result = await apiSocket.machineRPC<
			{ message: string },
			Record<string, never>
		>(machineId, "stop-daemon", {});
		log.log(
			`✅ machineStopDaemon: Received response from machine ${machineId}: ${result.message}`,
		);
		return result;
	} catch (error) {
		const errorMsg = error instanceof Error ? error.message : "Unknown error";
		log.log(
			`❌ machineStopDaemon: Failed to stop daemon on machine ${machineId}: ${errorMsg}`,
		);
...
export async function sessionKill(
	...
	try {
		...
		if (response.success) {
			...
			log.log(
				`⚠️ sessionKill: Failed to kill session ${sessionId}: ${response.message}`,
			);
		}
		return response;
	} catch (error) {
		const errorMsg = error instanceof Error ? error.message : "Unknown error";
		log.log(
			`❌ sessionKill: Exception killing session ${sessionId}: ${errorMsg}`,
		);
		return {
			success: false,
			message: errorMsg,
