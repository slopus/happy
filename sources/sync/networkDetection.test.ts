...
describe("NetworkDetection", () => {
	...
	const mockNetInfoFetch = mockNetInfo.fetch as MockedFunction<
		typeof mockNetInfo.fetch
	>;
	const mockNetInfoAddEventListener =
		mockNetInfo.addEventListener as MockedFunction<
	...
	describe("Strategy Selection", () => {
		...
		it("should adjust strategy based on stability", () => {
			...
			expect(strategy.retryPolicy.maxAttempts).toBeGreaterThan(
				NETWORK_STRATEGIES["wifi-good"].retryPolicy.maxAttempts,
			);
			expect(strategy.heartbeatProfile).toBe("aggressive");
		});
		...
		it("should optimize strategy for high stability", () => {
			...
			expect(strategy.timeouts.heartbeat).toBeGreaterThan(
				NETWORK_STRATEGIES["wifi-good"].timeouts.heartbeat,
			);
		});
	});
	...
	describe("Cellular Generation Optimizations", () => {
		it("should adjust strategy for 3G networks", () => {
			...
			expect(strategy.timeouts.connection).toBeGreaterThan(
				NETWORK_STRATEGIES["cellular-good"].timeouts.connection,
			);
			expect(strategy.heartbeatProfile).toBe("aggressive");
		});
		...
		it("should optimize strategy for 5G networks", () => {
			...
			expect(strategy.timeouts.connection).toBeLessThan(
				NETWORK_STRATEGIES["cellular-excellent"].timeouts.connection,
			);
		});
	});
...
describe("NETWORK_STRATEGIES Configuration", () => {
	...
	it("should have consistent strategy structure", () => {
		Object.keys(NETWORK_STRATEGIES).forEach((key) => {
			...
			expect([
				...
				"corporate",
				"battery_saver",
			]).toContain(strategy.heartbeatProfile);
		});
	});
	...
	it("should have appropriate timeout progressions", () => {
		...
		expect(wifiExcellent.timeouts.connection).toBeLessThan(
			wifiPoor.timeouts.connection,
		);
		expect(wifiExcellent.retryPolicy.maxAttempts).toBeLessThanOrEqual(
			wifiPoor.retryPolicy.maxAttempts,
		);
	});
	...
	it("should use corporate profile for corporate-restricted", () => {
		expect(NETWORK_STRATEGIES["corporate-restricted"].heartbeatProfile).toBe(
			"corporate",
		);
	});
});
