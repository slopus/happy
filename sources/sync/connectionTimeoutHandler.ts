...
export class ConnectionTimeoutHandler {
	...
	async requestWithTimeout<T = any>(
		...
		for (let attempt = 1; attempt <= maxAttempts; attempt++) {
			try {
				console.log(
					`🌐 ConnectionTimeoutHandler: Request attempt ${attempt}/${maxAttempts} to ${url}`,
				);
				...
				if (this.shouldRetryStatus(response.status) && attempt < maxAttempts) {
					...
					console.warn(
						`🌐 ConnectionTimeoutHandler: Retrying after ${delay}ms due to status ${response.status}`,
					);
 ...
 */
export function createApiTimeoutHandler(
 ...
 */
export function createUploadTimeoutHandler(
	config?: Partial<ConnectionTimeoutConfig>,
): ConnectionTimeoutHandler {
	return new ConnectionTimeoutHandler({
 ...
 */
export function createCriticalTimeoutHandler(
	config?: Partial<ConnectionTimeoutConfig>,
): ConnectionTimeoutHandler {
	return new ConnectionTimeoutHandler({
