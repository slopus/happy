...
export async function stopDaemon(
	...
	try {
		...
		log.log(
			`✅ Daemon stopped successfully on machine ${machineId}: ${result.message}`,
		);
		return {
			success: true,
 ...
 */
export async function forceStopDaemon(
	machineId: string,
): Promise<DaemonControlResult> {
	try {
		...
		log.log(
			`🚨 Force stopping daemon on ${machineId}: found ${machineSessions.length} active sessions`,
		);
		...
		const killPromises = machineSessions.map(async (session) => {
			try {
				...
				log.log(
					`❌ Error killing session ${session.id}: ${error instanceof Error ? error.message : error}`,
				);
				return {
					success: false,
		...
		const successfulKills = killResults.filter(
 ...
 */
export async function removeSessionLocally(
	machineId: string,
): Promise<DaemonControlResult> {
	try {
		const sessions = storage.getState().sessions;
		...
		console.log(
			`Removing ${machineSessions.length} sessions locally for machine ${machineId}`,
		);
			...
			try {
				...
				if (machineEncryption) {
					...
					try {
						...
						console.warn(
							"Failed to refresh machines during cleanup:",
							refreshError,
						);
					}
				}
				...
				console.warn(
					"Failed to update machine metadata during cleanup:",
					metadataError,
				);
...
export function getDaemonStatus(
	machineId: string,
): "likely-alive" | "stopped" | "unknown" {
	const machine = storage.getState().machines[machineId];
	if (!machine) return "unknown";

	const metadata = machine.metadata as any;
	if (
		metadata?.daemonLastKnownStatus === "shutting-down" ||
		metadata?.daemonLastKnownStatus === "manually-cleaned"
	) {
		return "stopped";
	}
