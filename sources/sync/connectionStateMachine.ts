...
export class ConnectionStateMachine {
	...
	transition(event: StateTransitionEvent): boolean {
		...
		if (!newState || !this.isValidTransition(this.currentState, newState)) {
			console.warn(
				`🔀 ConnectionStateMachine: Invalid transition from ${this.currentState} with event ${event.type}`,
			);
			return false;
		}
		...
		console.log(
			`🔀 ConnectionStateMachine: ${previousState} → ${newState} (${event.type})`,
		);
		return true;
	}
		...
		switch (currentState) {
			...
			case ConnectionState.CONNECTING:
				...
				if (event.type === "failure" || event.type === "timeout") {
					return this.context.connectionAttempts >=
						this.config.maxConnectionAttempts
			...
			case ConnectionState.RECONNECTING:
				if (event.type === "connect") return ConnectionState.CONNECTED;
				if (event.type === "failure" || event.type === "timeout") {
					return this.context.consecutiveFailures >=
	...
	private updateContext(event: StateTransitionEvent): void {
			...
			case "timeout":
				...
				if (event.error) {
					...
					if (this.context.errorHistory.length > this.config.maxErrorHistory) {
						this.context.errorHistory = this.context.errorHistory.slice(
		...
		if (
			this.context.lastDisconnectedAt &&
	...
	private setupStateHandlers(): void {
		...
		this.stateEntryHandlers.set(
			ConnectionState.CONNECTING,
			(prevState, context, event) => {
	...
	private scheduleReconnection(): void {
		...
		console.log(
			`🔀 ConnectionStateMachine: Scheduled reconnection in ${delay}ms`,
		);
	}
	...
	private notifyStateChange(
		...
		for (const listener of this.stateChangeListeners) {
			try {
				...
				console.error(
					"🔀 ConnectionStateMachine: Error in state change listener:",
					error,
				);
			}
		}
	...
	getStateDescription(): string {
		...
		switch (this.currentState) {
			...
			case ConnectionState.RECONNECTING:
				return failures > 1
					? `Reconnecting (${failures} failures)`
					: "Reconnecting...";
			case ConnectionState.FAILED:
