...
class RevenueCatWeb implements RevenueCatInterface {
	...
	setLogLevel(level: LogLevel): void {
		...
		console.log(
			`RevenueCat log level set to ${LogLevel[level]} (not supported on web)`,
		);
	}
		...
		try {
			...
			const hasEntitlement =
				customerInfo.entitlements.all[
					options?.requiredEntitlementIdentifier || "pro"
				]?.isActive;
	...
	private transformCustomerInfo(webInfo: WebCustomerInfo): CustomerInfo {
		...
		const entitlements: Record<
			string,
			{ isActive: boolean; identifier: string }
		> = {};
		Object.entries(webInfo.entitlements.all || {}).forEach(
			([key, entitlement]) => {
				entitlements[key] = {
		...
		return {
			current: webOfferings.current
				? {
						...
						availablePackages: transformPackages(
							webOfferings.current.availablePackages,
						),
					}
				: null,
