...
try {
	...
	platformStorage = {
		...
		async multiGet(keys: string[]): Promise<[string, string | null][]> {
			if (typeof window !== "undefined" && window.localStorage) {
				return keys.map((key: string): [string, string | null] => [
					key,
...
export class SessionStatePersistence {
	...
	async clearAllBackups(): Promise<void> {
		...
		const storageKeys = keys.map(
			(sessionId) => `${STORAGE_PREFIX}${sessionId}`,
		);
	...
	private async backupCurrentState(): Promise<void> {
		try {
			...
			if (backupsToSave.length > 0) {
				const keyValuePairs = backupsToSave.map(
			...
			console.log(
				`💾 SessionStatePersistence: Backed up ${backupsToSave.length} sessions`,
			);
		} catch (error) {
			console.error(
				"💾 SessionStatePersistence: Failed to backup state:",
				error,
			);
		}
	}
	...
	private async loadBackupsFromStorage(): Promise<void> {
		try {
			...
			const backupKeys = allKeys.filter((key: string) =>
			...
			for (const [key, value] of backupData) {
				if (value) {
					try {
						...
						console.warn(
							`💾 SessionStatePersistence: Failed to parse backup for key ${key}:`,
							parseError,
						);
			...
			console.log(
				`💾 SessionStatePersistence: Loaded ${loadedCount} backups from storage`,
			);
		} catch (error) {
			console.error(
				"💾 SessionStatePersistence: Failed to load backups:",
				error,
			);
		}
	}
	...
	private async cleanupOldBackups(): Promise<void> {
		...
		if (toRemove.length > 0) {
			console.log(
				`💾 SessionStatePersistence: Cleaned up ${toRemove.length} old backups`,
			);
		}
	}
		...
		try {
			...
			for (const [sessionId, cachedBackup] of this.localStateCache.entries()) {
				...
				if (!currentSession) {
					...
					console.log(
						`💾 SessionStatePersistence: Restoring session ${sessionId} from backup`,
					);
			...
			if (reconciledSessions.length > 0) {
				...
				console.log(
					`💾 SessionStatePersistence: Reconciled ${reconciledSessions.length} sessions`,
				);
			}
		} catch (error) {
			console.error(
				"💾 SessionStatePersistence: Failed to reconcile state:",
				error,
			);
		}
	}
	...
	private async resolveSessionConflict(
		...
		try {
			switch (this.config.conflictResolution) {
				case "local":
					console.log(
						`💾 SessionStatePersistence: Using local state for session ${sessionId}`,
					);
					return localSession;

				case "remote":
					console.log(
						`💾 SessionStatePersistence: Using remote state for session ${sessionId}`,
					);
				...
				default: {
					...
					const merged: Session = {
						...
						updatedAt: Math.max(
							...
							remoteSession.updatedAt || 0,
						),
						activeAt: Math.max(
							localSession.activeAt || 0,
							remoteSession.activeAt || 0,
						),
						thinkingAt: Math.max(
							localSession.thinkingAt || 0,
							remoteSession.thinkingAt || 0,
						),
						...
						metadataVersion: Math.max(
							localSession.metadataVersion || 0,
							remoteSession.metadataVersion || 0,
						),
						...
						agentStateVersion: Math.max(
							localSession.agentStateVersion || 0,
							remoteSession.agentStateVersion || 0,
						),

						// Preserve local permission and model modes
						permissionMode:
							localSession.permissionMode || remoteSession.permissionMode,
						modelMode: localSession.modelMode || remoteSession.modelMode,

						// Prefer local thinking state if more recent
						thinking:
							localSession.thinkingAt &&
							remoteSession.thinkingAt &&
					...
					console.log(
						`💾 SessionStatePersistence: Merged local and remote state for session ${sessionId}`,
					);
					return merged;
				}
			}
			...
			console.error(
				`💾 SessionStatePersistence: Failed to resolve conflict for session ${sessionId}:`,
				error,
			);
	...
	private setupConnectionListeners(): void {
		...
		apiSocket.onReconnected(() => {
			console.log(
				"💾 SessionStatePersistence: Connection restored, starting reconciliation",
			);
