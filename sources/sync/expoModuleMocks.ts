...
export const TaskManagerMock = {
	defineTask: (taskName: string, task: TaskManagerTaskExecutor) => {
		console.warn(
			`[TaskManager Mock] defineTask called for: ${taskName}. Task registered but will not execute.`,
		);
	},

	unregisterTaskAsync: async (taskName: string) => {
		console.warn(
			`[TaskManager Mock] unregisterTaskAsync called for: ${taskName}`,
		);
	},
	...
	isTaskRegisteredAsync: async (taskName: string) => {
		console.warn(
			`[TaskManager Mock] isTaskRegisteredAsync called for: ${taskName}`,
		);
		return false;
	},
};
...
export const BackgroundFetchMock = {
	registerTaskAsync: async (
		...
		console.warn(
			`[BackgroundFetch Mock] registerTaskAsync called for: ${taskName}. Background fetch not available.`,
		);
	},

	unregisterTaskAsync: async (taskName: string) => {
		console.warn(
			`[BackgroundFetch Mock] unregisterTaskAsync called for: ${taskName}`,
		);
	},
	...
	setMinimumIntervalAsync: async (minimumInterval: number) => {
		console.warn(
			`[BackgroundFetch Mock] setMinimumIntervalAsync called with: ${minimumInterval}`,
		);
	},
...
export function getBackgroundFetch() {
	try {
		...
		console.warn(
			"expo-background-fetch not available, using mock implementation",
		);
