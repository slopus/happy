...
describe("connectionConfig", () => {
	...
	describe("LEGACY_CONNECTION_CONFIG", () => {
		it("should disable enhanced connection management", () => {
			expect(LEGACY_CONNECTION_CONFIG.enableEnhancedConnectionManagement).toBe(
				false,
			);
		});

		it("should disable all enhanced features", () => {
			expect(LEGACY_CONNECTION_CONFIG.enableConnectionHealthMonitoring).toBe(
				false,
			);
			expect(LEGACY_CONNECTION_CONFIG.enableStaleConnectionCleanup).toBe(false);
			expect(LEGACY_CONNECTION_CONFIG.enableSessionStatePersistence).toBe(
				false,
			);
		});

		it("should inherit other configuration from CONNECTION_CONFIG", () => {
			expect(LEGACY_CONNECTION_CONFIG.heartbeatInterval).toBe(
				CONNECTION_CONFIG.heartbeatInterval,
			);
			expect(LEGACY_CONNECTION_CONFIG.connectionTimeout).toBe(
				CONNECTION_CONFIG.connectionTimeout,
			);
			expect(LEGACY_CONNECTION_CONFIG.maxReconnectAttempts).toBe(
				CONNECTION_CONFIG.maxReconnectAttempts,
			);
		});
	});
	...
	describe("runtime configuration overrides", () => {
		...
		it("should clear runtime overrides", () => {
			...
			clearConnectionConfigOverride();
			expect(getEffectiveConnectionConfig().heartbeatInterval).toBe(
				CONNECTION_CONFIG.heartbeatInterval,
			);
		});
		...
		it("should handle multiple override calls", () => {
			...
			const config = getEffectiveConnectionConfig();
			expect(config.heartbeatInterval).toBe(
				CONNECTION_CONFIG.heartbeatInterval,
			); // overridden by second call
			expect(config.maxReconnectAttempts).toBe(10);
		});
	...
	describe("configuration consistency", () => {
		it("should have consistent timeout relationships", () => {
			...
			expect(config.heartbeatInterval).toBeLessThan(
				config.staleConnectionThreshold / 5,
			);

			// Cleanup interval should be less than stale threshold
			expect(config.cleanupInterval).toBeLessThan(
	...
	describe("type safety", () => {
		it("should handle partial configuration overrides correctly", () => {
			...
			expect(config.enableEnhancedConnectionManagement).toBe(
				CONNECTION_CONFIG.enableEnhancedConnectionManagement,
			);
	...
	describe("backward compatibility", () => {
		...
		it("should preserve legacy behavior for basic connection management", () => {
			...
			expect(config.maxReconnectAttempts).toBe(
				CONNECTION_CONFIG.maxReconnectAttempts,
			);
			expect(config.reconnectionDelay).toBe(
				CONNECTION_CONFIG.reconnectionDelay,
			);
		});
	});
});
