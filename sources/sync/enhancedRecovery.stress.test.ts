...
describe("Enhanced Session Recovery Stress Tests", () => {
	...
	describe("Large Queue Performance", () => {
		test("should handle 1000+ queued operations efficiently", async () => {
			...
			expect(result.processed).toBeGreaterThanOrEqual(
				Math.floor(numOperations * 0.95),
			);
			expect(result.failed).toBeLessThanOrEqual(
				Math.ceil(numOperations * 0.05),
			);
			expect(recovery.getQueueSize()).toBeLessThanOrEqual(
				Math.ceil(numOperations * 0.05),
			);
		...
		test("should maintain priority ordering under heavy load", async () => {
			...
			for (let i = 1; i < processedOrder.length; i++) {
				const prevPriority =
					priorityOrder[
						processedOrder[i - 1].priority as keyof typeof priorityOrder
					];
				const currPriority =
					priorityOrder[
						processedOrder[i].priority as keyof typeof priorityOrder
					];
		...
		test("should handle queue size limits efficiently with large datasets", async () => {
			...
			console.log(
				`- Final queue size: ${recovery.getQueueSize()}/${maxQueueSize}`,
			);
			console.log(
				`- Critical operations preserved: ${status.byPriority.critical}`,
			);
		});
	});
	...
	describe("Memory Usage Performance", () => {
		test("should maintain memory usage under 10MB for 1000 operations", () => {
			...
			console.log(
				`- Memory increase: ${memoryIncrease} bytes (${memoryInMB.toFixed(2)} MB)`,
			);
	...
	describe("Concurrent Operations Performance", () => {
		test("should handle rapid operation queuing", () => {
			...
			console.log(
				`Rapid queuing test: ${numOperations} operations in ${queueTime}ms`,
			);
		});
		...
		test("should maintain performance with mixed operation types under load", async () => {
			...
			console.log(`- Processing time: ${processTime}ms`);
			console.log(
				`- Operations per second: ${(numOperations / (processTime / 1000)).toFixed(0)}`,
			);
		...
		test("should handle burst queuing followed by processing cycles", async () => {
			...
			for (let burst = 0; burst < numBursts; burst++) {
				...
				console.log(
					`Burst ${burst}: queued in ${burstTime}ms, processed in ${processTime}ms`,
				);
			}
			...
			expect(maxProcessTime - minProcessTime).toBeLessThan(
				Math.max(avgProcessTime * 2, 50),
			); // Variance should be reasonable but allow more tolerance, with minimum 50ms

			console.log(`Burst processing summary:`);
			console.log(`- Average processing time: ${avgProcessTime.toFixed(1)}ms`);
			console.log(
				`- Min/Max processing time: ${minProcessTime}ms/${maxProcessTime}ms`,
			);
		});
	});
