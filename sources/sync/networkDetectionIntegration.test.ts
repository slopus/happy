...
describe("NetworkDetection Integration", () => {
	...
	describe("Network Change Detection and Strategy Adaptation", () => {
		it("should detect network changes and adapt strategies automatically", async () => {
			...
			const wifiStrategy = strategyChanges.find(
				(change) => change.profile.type === "wifi",
			);
			const cellularStrategy = strategyChanges.find(
				(change) => change.profile.type === "cellular",
			);
			...
			expect(wifiStrategy!.strategy.timeouts.connection).toBeLessThan(
				cellularStrategy!.strategy.timeouts.connection,
			);
		...
		it("should maintain connection through network transitions", async () => {
			...
			const listener: NetworkChangeListener = (profile, strategy) => {
				connectionEvents.push(
					`Network changed to ${profile.type}-${profile.quality}`,
				);
			...
			const strategiesAreDifferent =
				wifiStrategy.timeouts.connection !==
					cellularStrategy.timeouts.connection ||
				wifiStrategy.timeouts.heartbeat !==
					cellularStrategy.timeouts.heartbeat ||
				wifiStrategy.retryPolicy.maxAttempts !==
					cellularStrategy.retryPolicy.maxAttempts ||
			...
			expect(connectionEvents).toContain(
				"Network changed to cellular-excellent",
			);
		});
		...
		it("should handle offline scenarios", async () => {
			...
			const listener: NetworkChangeListener = (profile, strategy) => {
				connectionEvents.push(
					`Network: ${profile.type}, Reachable: ${profile.isInternetReachable}`,
				);
			};
	...
	describe("Error Handling and Resilience", () => {
		...
		it("should recover from temporary network issues", async () => {
			...
			expect(
				stateChanges.filter((s) => s === "wifi-excellent").length,
