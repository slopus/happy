...
export async function derivePasswordKey(
    ...
    try {
        ...
        const key = await Crypto.digestStringAsync(
            ...
            password + Array.from(salt).map(b => String.fromCharCode(b)).join(''),
...
export async function encryptWithPassword(
    ...
    try {
        ...
        return {
            encryptedData: Array.from(encryptedBytes).map(b =>
                b.toString(16).padStart(2, '0')).join(''),
            salt: Array.from(actualSalt).map(b =>
                b.toString(16).padStart(2, '0')).join(''),
        };
    } catch (error) {
...
export async function storePasswordHash(
    ...
    try {
        ...
        const hashHex = Array.from(hash).map(b =>
            b.toString(16).padStart(2, '0')).join('');
...
export async function verifyPassword(password: string): Promise<boolean> {
    try {
        ...
        const derivedHashHex = Array.from(derivedHash).map(b =>
            b.toString(16).padStart(2, '0')).join('');
 ...
 */
export async function testSecureStorage(): Promise<{ [key in SecurityLevel]: boolean }> {
