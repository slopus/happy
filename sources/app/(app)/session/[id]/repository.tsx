...
const SUPPORTED_FILE_EXTENSIONS = new Set([
  ...
  'json', 'xml', 'html', 'css', 'scss', 'sass', 'less', 'md',
  'yml', 'yaml', 'toml', 'ini', 'cfg', 'conf', 'sql', 'sh', 'bash',
]);

// File extensions for text preview (no syntax highlighting)
const TEXT_FILE_EXTENSIONS = new Set([
  'txt', 'log', 'conf', 'cfg', 'ini', 'env', 'gitignore', 'gitattributes',
  ...
  const loadFileContent = async () => {
    ...
    try {
      ...
      if (decodedContent && decodedContent.length > MAX_PREVIEW_SIZE) {
        setError(`File too large for preview (${Math.round(decodedContent.length / 1000)}KB). Maximum size: ${MAX_PREVIEW_SIZE / 1000}KB`);
        return;
      }
        ...
        <ScrollView style={styles.previewContent}>
          {isLoading ? (
            <View style={styles.previewLoadingContainer}>
              ...
              <Text style={[styles.previewLoadingText, { color: theme.colors.textSecondary }]}>
                                Loading file...
              </Text>
            </View>
          ) : error ? (
            ...
            <View style={styles.previewCodeContainer}>
              {shouldShowSyntaxHighlighting(file.fileName) ? (
                ...
                <Text
                  style={[styles.previewPlainText, { color: theme.colors.text }]}
                  selectable={true}
                >
                  {fileContent}
                </Text>
...
export default function RepositoryScreen() {
  ...
  const addToRecentFiles = React.useCallback((file: FileItem) => {
    setRecentFiles(prev => {
  ...
  const handleItemPress = React.useCallback((item: FileItem) => {
  ...
  const handleOpenInViewer = React.useCallback((item: FileItem) => {
    addToRecentFiles(item);
  ...
  const handleItemLongPress = React.useCallback((item: FileItem) => () => {
  ...
  const renderItemSubtitle = React.useCallback((item: FileItem) => {
  ...
  const renderFileItem = React.useCallback(({ item, index }: { item: FileItem; index: number }) => (
    <Item
      ...
      style={Platform.OS === 'web' && selectedIndex === index ?
        { backgroundColor: theme.colors.input.background } : undefined
      }
    />
  ), [renderItemSubtitle, renderItemIcon, handleItemPress, handleItemLongPress, filteredItems.length, selectedIndex, theme.colors.input.background]);

  // Key extractor for FlatList performance
  const keyExtractor = React.useCallback((item: FileItem, index: number) =>
    `${item.fullPath}-${index}`, []);
      ...
      {!searchQuery && currentPath === '' && recentFiles.length > 0 && (
        <View style={styles.recentFilesSection}>
          <View style={[styles.recentFilesHeader, { borderBottomColor: theme.colors.divider }]}>
            ...
            <Text style={[styles.recentFilesTitle, { color: theme.colors.text }]}>
                            Recent Files
            </Text>
          </View>
          ...
          <>
            ...
            {filteredItems.length > VIRTUALIZATION_THRESHOLD ? (
              <FlatList
                ...
                windowSize={10}
                getItemLayout={Platform.OS === 'ios' ? (data, index) => ({
