...
export default function FeaturesSettingsScreen() {
	...
	const cycleDefaultCoder = () => {
		const options: Array<"claude" | "codex" | "ask"> = [
			"claude",
			"codex",
			"ask",
		];
	...
	const getModelCapabilityIcon = (model: MCPModel) => {
		if (model.capabilities.codeGeneration && model.capabilities.codeReview)
		...
		<ItemList style={{ paddingTop: 0 }}>
					...
					<>
						{modelsToShow.map((model, index) => (
							<Item
								...
								icon={
									<Ionicons
										name={getModelCapabilityIcon(model)}
										size={29}
										color="#34C759"
									/>
								}
								...
								onPress={() => {
									router.push(
										`/settings/model-details?modelId=${encodeURIComponent(model.id)}`,
									);
								}}
								showChevron={true}
							/>
							...
							<Item
								title={
									showAllModels
										...
										: t(
												"settingsFeatures.showMoreModels",
			...
			>
				<Item
					...
					subtitle={
						anonymousMode
							? t("password.anonymousModeEnabled")
							: t("settingsFeatures.anonymousModeDisabled")
					}
					icon={<Ionicons name="person-outline" size={29} color="#FF9500" />}
					rightElement={
						<Switch value={anonymousMode} onValueChange={setAnonymousMode} />
					}
					showChevron={false}
				/>
			</ItemGroup>
