...
export default function MachineDetailScreen() {
	...
	const machineSessions = useMemo(() => {
		...
		sessions.forEach((item) => {
			if (
				item.type === "session" &&
				item.session.metadata?.machineId === machineId
			) {
							...
							if (result.success) {
								...
								showDaemonCleanupModal({
									...
									onRemoveSession: async () => {
										const cleanupResult = await removeSessionLocally(
											machineId!,
										);
										if (cleanupResult.success) {
											...
											throw new Error(
												cleanupResult.error || cleanupResult.message,
											);
										}
									},
									onCancel: () => {
									...
									onForceStop: async () => {
										...
										if (forceResult.success) {
											...
											Modal.alert(
												t("common.error"),
												forceResult.error || forceResult.message,
											);
										}
										await sync.refreshMachines();
									},
