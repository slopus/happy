...
export function CliManagement({ session }: CliManagementProps) {
	...
	const handleDevBranchSwitch = useCallback(async () => {
		...
		try {
			...
			const devCommand =
				"npm install -g git+https://github.com/jeffersonwarrior/happy-fork.git#main";
	...
	return (
		<ItemGroup>
			...
			<Item
				...
				subtitle={
					isOnDevBranch
						? "Development branch (jeffersonwarrior/happy-fork)"
						: "Stable release (npm)"
				}
				icon={
					<Ionicons
			...
			{isCliOutdated && !isOnDevBranch && (
				<Item
					title={
						isUpdating
							? t("sessionInfo.updateCliUpdating")
							: t("sessionInfo.updateCliAutomatic")
					}
					subtitle={
						isUpdating
							? "Please wait..."
							: `Update from v${session.metadata.version} to v${MINIMUM_CLI_VERSION}+`
					}
					icon={
						<Ionicons
			...
			{!isOnDevBranch && (
				<Item
					title={
						isSwitchingBranch
							? t("sessionInfo.updateCliDevUpdating")
							: t("sessionInfo.updateCliDev")
					}
					subtitle={
						isSwitchingBranch
							? "Installing from git..."
							: t("sessionInfo.updateCliDevDescription")
					}
					icon={
						<Ionicons
							name={
								isSwitchingBranch ? "refresh-outline" : "git-branch-outline"
							}
							size={29}
			...
			{isOnDevBranch && (
				<Item
					title={
						isSwitchingBranch
							? "Switching to stable..."
							: t("sessionInfo.updateCliStable")
					}
					subtitle={
						isSwitchingBranch
							? "Installing from npm..."
							: t("sessionInfo.updateCliStableDescription")
					}
					icon={
						<Ionicons
