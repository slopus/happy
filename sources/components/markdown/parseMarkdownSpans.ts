...
export function parseMarkdownSpans(markdown: string, header: boolean) {
	...
	while ((match = pattern.exec(markdown)) !== null) {
		...
		if (iterationCount > maxIterations) {
			console.error(
				"parseMarkdownSpans: Maximum iterations exceeded, breaking to prevent infinite loop",
			);
			break;
		}
		...
		if (match.index === lastIndex && match[0].length === 0) {
			console.error(
				"parseMarkdownSpans: Zero-length match detected, breaking to prevent infinite loop",
			);
