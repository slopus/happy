...
export const ru: TranslationStructure = {
  ...
  settingsFeatures: {
    ...
    aiModels: 'Модели ИИ',
    aiModelsDescription: 'Автоматически обнаруженные модели ИИ и их возможности. Нажмите для обновления списка моделей.',
  ...
  password: {
    ...
    incorrectPasswordMessage: ({ attempts }: { attempts: number }) => `Неверный пароль. ${plural({ count: attempts, one: 'Осталась 1 попытка', few: `Осталось ${attempts} попытки`, many: `Осталось ${attempts} попыток` })}.`,
    tooManyAttempts: 'Слишком много попыток',
    tooManyAttemptsMessage: 'Слишком много неудачных попыток ввода пароля. Попробуйте биометрическую аутентификацию или сбросьте приложение.',
    attemptsRemaining: ({ attempts }: { attempts: number }) => plural({ count: attempts, one: 'Осталась 1 попытка', few: `Осталось ${attempts} попытки`, many: `Осталось ${attempts} попыток` }),
    ...
    forgotPasswordMessage: 'При сбросе пароля приложение будет сброшено, и вам нужно будет восстановить аккаунт с помощью секретного ключа.',
    ...
    resetPasswordBiometric: 'Ваша личность подтверждена. Это действие удалит защиту паролем, сохранив данные ваших сессий в безопасности.',
    resetPassword: 'Сбросить пароль',
    resetAppTitle: 'Сброс приложения',
    resetAppWarning: 'Это удалит ВСЕ ваши данные, включая сессии, настройки и информацию об аккаунте. Это действие нельзя отменить.',
    resetAppDescription: 'Удалить все данные и начать заново',
    confirmReset: 'Вы абсолютно уверены?',
    confirmResetMessage: 'Это безвозвратно удалит всё. Вам потребуется восстановить аккаунт с помощью секретного ключа.',
  ...
  passwordMigration: {
    ...
    description: 'Добавьте защиту паролем для безопасности ваших сессий программирования. Ваши данные останутся конфиденциальными с возможностью биометрической разблокировки.',
