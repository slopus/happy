...
import { checkIfDaemonRunningAndCleanupStaleState, isDaemonRunningCurrentlyInstalledHappyVersion, stopDaemon, listDaemonSessions, stopDaemonSession } from '@/daemon/controlClient';
import { killRunawayHappyProcesses } from '@/daemon/doctor';
import { install } from '@/daemon/install';
import { uninstall } from '@/daemon/uninstall';
import { DEFAULT_GEMINI_MODEL, GEMINI_MODEL_ENV } from '@/gemini/constants';
import { logger, getLatestDaemonLog } from '@/ui/logger';
  ...
  if (subcommand === 'doctor') {
    ...
    return;
	    ...
	    try {
	      ...
	      const { startedBy, permissionMode, permissionModeUpdatedAt } = parseSessionStartArgsForCommand()
