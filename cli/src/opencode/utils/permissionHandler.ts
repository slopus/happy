 ...
 * Extends BasePermissionHandler with OpenCode-specific permission mode logic.
 */
...
/**
 * OpenCode-specific permission handler with permission mode support.
 * 
 * TODO: When OpenCode's yolo mode is released, add mapping here.
 * OpenCode currently supports 'build' (full access) and 'plan' (read-only) modes.
 */
export class OpencodePermissionHandler extends BasePermissionHandler {
    ...
    private shouldAutoApprove(toolName: string, toolCallId: string, input: unknown): boolean {
        ...
        // - OpencodeReasoning: Reasoning is just display of thinking process, not an action
        // - think: Thinking/saving memories is safe
        // - save_memory: Saving memories is safe
        const alwaysAutoApproveNames = ['change_title', 'happy__change_title', 'OpencodeReasoning', 'think', 'save_memory'];
        ...
        // Check by toolCallId (OpenCode may send change_title with different patterns)
        ...
        switch (this.currentPermissionMode) {
            case 'yolo':
                ...
                // TODO: Map to OpenCode's yolo mode when it's released
                return true;
            case 'safe-yolo':
            ...
            case 'read-only':
                ...
                const writeTools = ['write', 'edit', 'create', 'delete', 'patch', 'fs-edit', 'bash', 'shell'];
