    ...
    b.onMessage((msg: AgentMessage) => {
      if (loadingSession) {
        if (msg.type === 'status' && msg.status === 'error') {
          turnAborted = true;
          params.session.sendAgentMessage('opencode', { type: 'turn_aborted', id: randomUUID() });
        }
        return;
      }
      switch (msg.type) {
        ...
        case 'tool-call': {
          ...
          params.session.sendAgentMessage('opencode', {
            type: 'tool-call',
            callId: msg.callId,
            name: msg.toolName,
            input: msg.args,
        ...
        case 'permission-request': {
          forwardAcpPermissionRequest({ msg, session: params.session, agent: 'opencode' });
          break;
        }
        ...
        case 'event': {
          ...
          if (name === 'thinking') {
            ...
            if (text) {
              params.session.sendAgentMessage('opencode', { type: 'thinking', text });
            }
          }
          break;
        }
  ...
  return {
    ...
    async reset(): Promise<void> {
      ...
      if (backend) {
        try {
          ...
          logger.debug('[OpenCodeACP] Failed to dispose backend (non-fatal)', e);
        }
        backend = null;
      }
    },
    ...
    async startOrLoad(opts: { resumeId?: string | null }): Promise<string> {
      ...
      if (resumeId) {
        ...
        if (!loadSession && !loadWithReplay) {
          throw new Error('OpenCode ACP backend does not support loading sessions');
        }
        ...
        if (replay) {
          importAcpReplayHistoryV1({
            ...
            logger.debug('[OpenCodeACP] Failed to import replay history (non-fatal)', e);
          });
        }
      } else {
    ...
    flushTurn(): void {
      if (accumulatedResponse.trim()) {
        params.session.sendAgentMessage('opencode', { type: 'message', message: accumulatedResponse });
      }

      if (!turnAborted) {
        params.session.sendAgentMessage('opencode', { type: 'task_complete', id: randomUUID() });
      }
