# PRD-001: Agent Display Name Integration
...
Make the session list display agent names from `.arc.yaml` instead of path-based names.
...
3. When a session has `.arc.yaml` with `agent.name: "Emila"`, the session list shows "Emila" instead of the path

## Current State

- `AgentConfigProvider` exists at `sources/arc/agent/context.tsx`
- Provider is wired into `_layout.tsx`
- `getDisplayName(sessionId, fallback)` method available
...
- Connect to a session with `.arc.yaml` - should show agent name
